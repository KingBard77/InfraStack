...
const ArchitectureVnetAzureModelCore = require('../templates/content/tools/azure/architecture-vnet-azure/assets/bin/model-core.js');
...
test('inferFromPrompt extracts Azure Container Apps, region, NAT mode, and PostgreSQL deterministically', function () {
    ...
    const result = ArchitectureVnetAzureModelCore.inferFromPrompt(prompt, presets[1]);
...
test('inferFromPrompt keeps preset CIDR when prompt CIDR is invalid', function () {
    ...
    const result = ArchitectureVnetAzureModelCore.inferFromPrompt(prompt, presets[0]);
...
test('buildExportPayload preserves tool identity, version, and state payload shape', function () {
    ...
    const payload = ArchitectureVnetAzureModelCore.buildExportPayload(
        spec,
    ...
    assert.equal(payload.tool, 'architecture-vnet-azure');
...
test('buildImportedPayloadState rejects invalid tool id and invalid CIDR', function () {
    const wrongTool = ArchitectureVnetAzureModelCore.buildImportedPayloadState({
        tool: 'wrong-tool',
        version: '1.0.0',
        diagram: {}
    }, presets);

    assert.equal(wrongTool.error, 'The imported JSON is not an Azure VNet Architecture workspace export.');

    const invalidCidr = ArchitectureVnetAzureModelCore.buildImportedPayloadState({
        tool: 'architecture-vnet-azure',
        version: '1.0.0',
        diagram: {
...
test('buildImportedPayloadState restores a valid exported payload', function () {
    ...
    const restored = ArchitectureVnetAzureModelCore.buildImportedPayloadState(payload, presets);
