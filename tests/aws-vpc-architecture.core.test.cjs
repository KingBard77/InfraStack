...
const AwsVpcModelCore = require('../templates/content/tools/aws/aws-vpc-architecture/assets/bin/model-core.js');
...
test('inferFromPrompt extracts ECS Fargate, region, NAT mode, and Aurora deterministically', function () {
    ...
    const result = AwsVpcModelCore.inferFromPrompt(prompt, presets[1]);
...
test('inferFromPrompt keeps preset CIDR when prompt CIDR is invalid', function () {
    ...
    const result = AwsVpcModelCore.inferFromPrompt(prompt, presets[0]);
...
test('buildExportPayload preserves tool identity, version, and state payload shape', function () {
    ...
    const payload = AwsVpcModelCore.buildExportPayload(
        spec,
    ...
    assert.equal(payload.tool, 'aws-vpc-architecture');
...
test('buildImportedPayloadState rejects invalid tool id and invalid CIDR', function () {
    const wrongTool = AwsVpcModelCore.buildImportedPayloadState({
        tool: 'wrong-tool',
        version: '1.0.0',
        diagram: {}
    }, presets);
    ...
    const invalidCidr = AwsVpcModelCore.buildImportedPayloadState({
        tool: 'aws-vpc-architecture',
        version: '1.0.0',
        diagram: {
...
test('buildImportedPayloadState restores a valid exported payload', function () {
    ...
    const restored = AwsVpcModelCore.buildImportedPayloadState(payload, presets);
