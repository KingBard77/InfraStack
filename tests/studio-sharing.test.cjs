...
test('Studio Share creates a persisted read-only project instead of sharing the editor URL', () => {
    ...
    assert.match(page, /id="studio-share-copy-link"/);
    ...
    assert.match(page, /More sharing options/);
    assert.match(page, /id="studio-share-reddit"/);
    assert.match(page, /id="studio-share-copy-link-more"/);
    assert.match(page, /id="studio-share-open-embed"/);
    assert.match(page, /id="studio-embed-close"/);
    assert.match(publisher, /elements\.shareReddit\.href/);
    assert.match(publisher, /elements\.shareOpenEmbed\.addEventListener/);
...
test('share and embed pages use the shared read-only renderer', () => {
    ...
    assert.equal(embedCss.replace('/* embed.css */', '').trim(), shareCss.replace('/* share.css */', '').trim());
