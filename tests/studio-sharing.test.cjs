...
test('Studio Share creates a persisted read-only project instead of sharing the editor URL', () => {
    ...
    const graphAdapter = read('assets/js/studio/library/studio-maxgraph.js');
    ...
    assert.match(page, /id="studio-mode-share"/);
    assert.match(page, /id="studio-share-mode-preview"/);
    ...
    assert.match(publisher, /previewImage: renderShareImage/);
    assert.match(publisher, /openShare: function \(\) \{ return createPublishedSnapshot\('share'\); \}/);
    assert.match(publisher, /openEmbed: function \(\) \{ return createPublishedSnapshot\('embed'\); \}/);
    assert.match(publisher, /downloadPng/);
    ...
    assert.match(imageRenderer, /import \{ StudioMaxGraphAdapter \} from '\.\.\/library\/studio-maxgraph\.js'/);
    assert.match(imageRenderer, /new StudioMaxGraphAdapter\(graph, outline\)/);
    assert.match(imageRenderer, /adapter\.exportSvg\(diagram\.width, diagram\.height\)/);
    assert.match(imageRenderer, /context\.drawImage\(diagramImage, diagram\.x, diagram\.y, diagram\.width, diagram\.height\)/);
    assert.doesNotMatch(imageRenderer, /connection\.label \|\| connection\.protocol/);
    assert.doesNotMatch(imageRenderer, /connection\.routing/);
