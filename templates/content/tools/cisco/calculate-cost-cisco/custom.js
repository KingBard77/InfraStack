...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const sortOptionInputs = Array.from(document.querySelectorAll('input[name="calculateCostCiscoSortOption"]'));
        ...
        if (query.includeCompute) {
            if (query.computeInstance === 'custom') {
                ...
                if (query.overrides.computeHourly === null) {
                    return 'Set device monthly override in Advanced when using Custom profile.';
                }
    ...
    function buildSummaryHtml(result) {
        ...
        return `
            <section class="calculate-cost-cisco-overview" aria-live="polite">
                <article class="calculate-cost-cisco-spend-card">
                    <div class="calculate-cost-cisco-spend-ring" id="calculateCostCiscoSpendRing" style="--progress-angle: ${escapeHtml(String(progressAngle))}deg;" aria-label="Estimated monthly Cisco run rate ${escapeHtml(formatCurrency(result.totals.monthlyTotal))}">
                        <div class="calculate-cost-cisco-spend-chart" id="calculateCostCiscoSpendChart" aria-hidden="true"></div>
                        <div class="calculate-cost-cisco-spend-value">
                            <strong>${escapeHtml(formatCurrency(result.totals.monthlyTotal))}</strong>
                            <span>Monthly</span>
                        </div>
                    </div>
                    <h3 class="calculate-cost-cisco-spend-label">Estimated Cisco run rate</h3>
                    <p class="calculate-cost-cisco-spend-copy">Built from current workload assumptions, not from your billing account. The math is visible so the argument can be too.</p>
                </article>

                <section class="calculate-cost-cisco-summary-panel" aria-label="Cisco estimate summary">
                    <div class="calculate-cost-cisco-badge-row">
                        <span class="calculate-cost-cisco-badge">${escapeHtml(presetLabel)}</span>
                        <span class="calculate-cost-cisco-badge">Services ${escapeHtml(String(result.totals.activeServices))}</span>
                        <span class="calculate-cost-cisco-badge">Overrides ${escapeHtml(String(overrideCount))}</span>
                        <span class="calculate-cost-cisco-badge calculate-cost-cisco-badge-muted">Updated ${escapeHtml(generatedAtText)}</span>
                    </div>

                    <div class="calculate-cost-cisco-summary-route">
                        <span class="calculate-cost-cisco-summary-route-label">Top driver</span>
                        <span class="calculate-cost-cisco-summary-route-value">${escapeHtml(result.totals.topDriver)}</span>
                    </div>

                    <div class="calculate-cost-cisco-stat-grid">
                        <div class="calculate-cost-cisco-stat-card">
                            <span class="calculate-cost-cisco-stat-label">Annual</span>
                            <span class="calculate-cost-cisco-stat-value">${escapeHtml(formatCurrency(result.totals.annualTotal))}</span>
                            <span class="calculate-cost-cisco-stat-note">Projected from the current monthly run rate.</span>
                        </div>

                        <div class="calculate-cost-cisco-stat-card">
                            <span class="calculate-cost-cisco-stat-label">Daily</span>
                            <span class="calculate-cost-cisco-stat-value">${escapeHtml(formatCurrency(result.totals.dailyTotal))}</span>
                            <span class="calculate-cost-cisco-stat-note">Thirty-day planning average.</span>
                        </div>

                        <div class="calculate-cost-cisco-stat-card">
                            <span class="calculate-cost-cisco-stat-label">Hourly</span>
                            <span class="calculate-cost-cisco-stat-value">${escapeHtml(formatCurrency(result.totals.hourlyTotal))}</span>
                            <span class="calculate-cost-cisco-stat-note">Useful for sanity-checking long-running workloads.</span>
                        </div>

                        <div class="calculate-cost-cisco-stat-card">
                            <span class="calculate-cost-cisco-stat-label">Transfer</span>
                            <span class="calculate-cost-cisco-stat-value">${escapeHtml(formatNumber(result.totals.combinedEgressGb, 1))} GB</span>
                            <span class="calculate-cost-cisco-stat-note">Combined licensing telemetry, management response, and shared outbound traffic.</span>
                        </div>
                    </div>
                </section>
            </section>
