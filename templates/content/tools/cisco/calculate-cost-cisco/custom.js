...
installInfraStackResultSummaryNormalizer('calculate-cost-cisco');
...
// ns:start family._base.workspace.01_input-brief
    const form = document.getElementById('calculateCostCiscoForm');
    const labelInput = document.getElementById('calculateCostCiscoLabel');
...
// ns:end family._base.workspace.02_basic-settings
    ...
    const resetButton = document.getElementById('calculateCostCiscoReset');

// ns:start family._base.workspace.03_custom-settings
    const includeComputeInput = document.getElementById('calculateCostCiscoIncludeCompute');
    const includeDiskInput = document.getElementById('calculateCostCiscoIncludeDisk');
    const includeBlobInput = document.getElementById('calculateCostCiscoIncludeBlob');
    const includeFunctionsInput = document.getElementById('calculateCostCiscoIncludeFunctions');
    ...
    const computeCustomVcpuInput = document.getElementById('calculateCostCiscoComputeCustomVcpu');
    const computeCustomMemoryGiBInput = document.getElementById('calculateCostCiscoComputeCustomMemoryGiB');
    const computeCountInput = document.getElementById('calculateCostCiscoComputeCount');
    const computeHoursInput = document.getElementById('calculateCostCiscoComputeHours');

    const diskStorageGbInput = document.getElementById('calculateCostCiscoDiskStorageGb');
    const diskIopsInput = document.getElementById('calculateCostCiscoDiskIops');
    const diskThroughputInput = document.getElementById('calculateCostCiscoDiskThroughput');

    const blobStorageGbInput = document.getElementById('calculateCostCiscoBlobStorageGb');
    const blobEgressGbInput = document.getElementById('calculateCostCiscoBlobEgressGb');
    const blobGetRequestsKInput = document.getElementById('calculateCostCiscoBlobGetRequestsK');
    const blobPutRequestsKInput = document.getElementById('calculateCostCiscoBlobPutRequestsK');

    const functionsRequestsMillionInput = document.getElementById('calculateCostCiscoFunctionsRequestsMillion');
    const functionsDurationMsInput = document.getElementById('calculateCostCiscoFunctionsDurationMs');
    ...
    const sharedEgressGbInput = document.getElementById('calculateCostCiscoSharedEgressGb');
    const supportPctInput = document.getElementById('calculateCostCiscoSupportPct');
    const contingencyPctInput = document.getElementById('calculateCostCiscoContingencyPct');
    const manualAdjustmentInput = document.getElementById('calculateCostCiscoManualAdjustment');

    const freeTierFunctionsInput = document.getElementById('calculateCostCiscoFreeTierFunctions');
    const freeTierApiInput = document.getElementById('calculateCostCiscoFreeTierApi');
    const freeTierEgressInput = document.getElementById('calculateCostCiscoFreeTierEgress');
    const regionalUpliftPctInput = document.getElementById('calculateCostCiscoRegionalUpliftPct');

    const computeHourlyOverrideInput = document.getElementById('calculateCostCiscoComputeHourlyOverride');
    const diskStorageRateInput = document.getElementById('calculateCostCiscoDiskStorageRate');
    const diskIopsRateInput = document.getElementById('calculateCostCiscoDiskIopsRate');
    const diskThroughputRateInput = document.getElementById('calculateCostCiscoDiskThroughputRate');
    const blobStorageRateInput = document.getElementById('calculateCostCiscoBlobStorageRate');
    const blobGetRateInput = document.getElementById('calculateCostCiscoBlobGetRate');
    const blobPutRateInput = document.getElementById('calculateCostCiscoBlobPutRate');
    const functionsRequestRateInput = document.getElementById('calculateCostCiscoFunctionsRequestRate');
    const functionsGbSecondRateInput = document.getElementById('calculateCostCiscoFunctionsGbSecondRate');
    const apiHttpFirstRateInput = document.getElementById('calculateCostCiscoApiHttpFirstRate');
    const apiHttpNextRateInput = document.getElementById('calculateCostCiscoApiHttpNextRate');
    const apiRestRateInput = document.getElementById('calculateCostCiscoApiRestRate');
    const egressRateInput = document.getElementById('calculateCostCiscoEgressRate');

// ns:end family._base.workspace.03_custom-settings
    const resultEmpty = document.getElementById('calculateCostCiscoResultEmpty');
    const resultContent = document.getElementById('calculateCostCiscoResultContent');
    const resultError = document.getElementById('calculateCostCiscoResultError');
    const resultSummary = document.getElementById('calculateCostCiscoResultSummary');
    const breakdownTableBody = document.getElementById('calculateCostCiscoBreakdownTableBody');
    const serviceTableBody = document.getElementById('calculateCostCiscoServiceTableBody');
    const assumptionTableBody = document.getElementById('calculateCostCiscoAssumptionTableBody');
    const recommendationsWrap = document.getElementById('calculateCostCiscoRecommendations');
    const methodologyWrap = document.getElementById('calculateCostCiscoMethodology');
    const jsonOutput = document.getElementById('calculateCostCiscoJsonOutput');

    const sortInput = document.getElementById('calculateCostCiscoSort');
    const sortSummary = document.getElementById('calculateCostCiscoSortSummary');
    const sortOptionButtons = Array.from(document.querySelectorAll('.calculate-cost-cisco-sort-option'));
    const sortSelect = document.getElementById('calculateCostCiscoSortSelect');
    const exportPdfButton = document.getElementById('calculateCostCiscoExportPdf');
    const downloadCsvButton = document.getElementById('calculateCostCiscoDownloadCsv');
    const copyJsonButton = document.getElementById('calculateCostCiscoCopyJson');
    const downloadJsonButton = document.getElementById('calculateCostCiscoDownloadJson');
    const importJsonButton = document.getElementById('calculateCostCiscoImportJsonButton');
    ...
    const PURCHASE_MODE_LABELS = {
        onDemand: 'Monthly allocation',
        spot: 'Lab or spare allocation',
        savings1: 'EA 1yr',
        savings3: 'EA 3yr',
        reserved1: 'Smart Net 1yr',
        reserved3: 'Smart Net 3yr'
    };
        ...
        'd4s-v5': {
            label: 'Catalyst 9500 distribution',
            family: 'General purpose',
            vcpu: 4,
            memoryGiB: 16,
            rates: {
    ...
    const NUMERIC_INPUT_DECORATORS = {
        calculateCostCiscoComputeCustomVcpu: {
            suffix: 'units'
        },
        calculateCostCiscoComputeCustomMemoryGiB: {
            suffix: 'GiB'
        },
        calculateCostCiscoComputeCount: {
            suffix: 'units'
        },
        calculateCostCiscoComputeHours: {
            suffix: 'hours'
        },
        calculateCostCiscoDiskStorageGb: {
            suffix: 'units'
        },
        calculateCostCiscoDiskIops: {
            suffix: 'units'
        },
        calculateCostCiscoDiskThroughput: {
            suffix: 'Mbps'
        },
        calculateCostCiscoBlobStorageGb: {
            suffix: 'units'
        },
        calculateCostCiscoBlobEgressGb: {
            suffix: 'GB'
        },
        calculateCostCiscoBlobGetRequestsK: {
            suffix: '1k'
        },
        calculateCostCiscoBlobPutRequestsK: {
            suffix: '1k'
        },
        calculateCostCiscoFunctionsRequestsMillion: {
            suffix: '1M'
        },
        calculateCostCiscoFunctionsDurationMs: {
            suffix: 'ms'
        },
        calculateCostCiscoFunctionsMemoryMb: {
            suffix: 'MB'
        },
        calculateCostCiscoApiRequestsMillion: {
            suffix: '1M'
        },
        calculateCostCiscoApiResponseKb: {
            suffix: 'KB'
        },
        calculateCostCiscoSharedEgressGb: {
            suffix: 'GB'
        },
        calculateCostCiscoSupportPct: {
            suffix: '%'
        },
        calculateCostCiscoContingencyPct: {
            suffix: '%'
        },
        calculateCostCiscoManualAdjustment: {
            prefix: '$'
        },
        calculateCostCiscoRegionalUpliftPct: {
            suffix: '%'
        },
        calculateCostCiscoComputeHourlyOverride: {
            prefix: '$',
            suffix: 'unit-hour'
        },
        calculateCostCiscoDiskStorageRate: {
            prefix: '$',
            suffix: 'unit-month'
        },
        calculateCostCiscoDiskIopsRate: {
            prefix: '$',
            suffix: 'unit-month'
        },
        calculateCostCiscoDiskThroughputRate: {
            prefix: '$',
            suffix: 'Mbps-month'
        },
        calculateCostCiscoBlobStorageRate: {
            prefix: '$',
            suffix: 'unit-month'
        },
        calculateCostCiscoBlobGetRate: {
            prefix: '$',
            suffix: '1k checks'
        },
        calculateCostCiscoBlobPutRate: {
            prefix: '$',
            suffix: '1k changes'
        },
        calculateCostCiscoFunctionsRequestRate: {
            prefix: '$',
            suffix: '1M jobs'
        },
        calculateCostCiscoFunctionsGbSecondRate: {
            prefix: '$',
            suffix: 'GB-s'
        },
        calculateCostCiscoApiHttpFirstRate: {
            prefix: '$',
            suffix: '1M'
        },
        calculateCostCiscoApiHttpNextRate: {
            prefix: '$',
            suffix: '1M'
        },
        calculateCostCiscoApiRestRate: {
            prefix: '$',
            suffix: '1M'
        },
        calculateCostCiscoEgressRate: {
            prefix: '$',
            suffix: 'GB'
        }
    };
    ...
    function buildInputAddon(text, position) {
        ...
        addon.className = `calculate-cost-cisco-input-addon calculate-cost-cisco-input-addon--${position}`;
    ...
    function decorateNumericInput(input, decorator) {
        if (!input || input.dataset.calculateCostCiscoDecorated === '1') {
            return;
        }
        ...
        wrapper.className = 'calculate-cost-cisco-input-group';
        input.classList.add('calculate-cost-cisco-input-group-control');
        input.dataset.calculateCostCiscoDecorated = '1';
    ...
    function initMarkdownCopyButtons() {
        const promptBlocks = Array.from(document.querySelectorAll('.markdown-content pre.calculate-cost-cisco-prompt-pre'));
        const promptCopyButtons = document.querySelectorAll('.calculate-cost-cisco-prompt-copy-btn');
        ...
        promptCopyButtons.forEach((button) => {
            ...
            if (button.dataset.calculateCostCiscoCopyBound === '1') {
                return;
            }

            button.dataset.calculateCostCiscoCopyBound = '1';
        ...
        const codeBlocks = document.querySelectorAll('.markdown-content pre:not(.calculate-cost-cisco-prompt-pre)');
        ...
        codeBlocks.forEach((pre) => {
            ...
            if (button && button.dataset.calculateCostCiscoCopyBound === '1') {
                return;
            }
    ...
    function loadECharts() {
        ...
        if (!echartsPromise) {
            echartsPromise = new Promise((resolve) => {
                const existingScript = document.querySelector('script[data-calculate-cost-cisco-echarts]');
                ...
                script.async = true;
                script.dataset.calculateCostCiscoEcharts = 'true';
    ...
    function slugify(value) {
        return String(value || 'cisco-cost-estimate')
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/^-+|-+$/g, '') || 'cisco-cost-estimate';
    }
    ...
    function buildQuery() {
        return {
            label: String(labelInput.value || '').trim() || 'Cisco network estimate',
        ...
        if (query.includeCompute) {
            if (query.computeInstance === 'custom') {
                if (query.computeCustomVcpu <= 0 || query.computeCustomMemoryGiB <= 0) {
                    return 'Set custom planning units and management footprint when using Custom profile.';
                }

                if (query.overrides.computeHourly === null) {
                    return 'Set device monthly override in Custom when using Custom profile.';
                }
            } else if (!COMPUTE_CATALOG[query.computeInstance]) {
                return 'Choose a supported Switch platform.';
            }

            if (!PURCHASE_MODE_LABELS[query.computePurchase]) {
                return 'Choose a supported purchase model.';
            }
        }

        if (!['http', 'rest'].includes(query.apiType)) {
            return 'Choose a supported Cloud management type.';
        }
    ...
    function updateCustomInstanceLabel() {
        ...
        const sizeLabel = customVcpu > 0 && customMemoryGiB > 0
            ? `${formatNumber(customVcpu, 0)} planning units - ${formatNumber(customMemoryGiB, customMemoryGiB % 1 === 0 ? 0 : 1)} GiB footprint`
            : 'user defined';

        computeCustomOptionLabel.textContent = customLabel
            ? `${customLabel} - ${sizeLabel}`
            : `Custom profile - ${sizeLabel}`;
    }
    ...
    function syncServiceState() {
        Object.values(serviceRefs).forEach((serviceRef) => {
            ...
            Array.from(serviceRef.body.querySelectorAll('.calculate-cost-cisco-select')).forEach((selectElement) => {
    ...
    function calculateCompute(query, assumptions) {
        if (!query.includeCompute || query.computeCount <= 0 || query.computeHours <= 0) {
            assumptions.push({
                name: 'Switching hardware pricing basis',
                value: 'Disabled',
                note: 'No switching hardware cost is included in this estimate.'
            });
        ...
        const instance = query.computeInstance === 'custom'
            ? {
                label: query.computeCustomLabel || 'Custom switching profile',
        ...
        assumptions.push({
            ...
            note: `${resolvedRate.source}. ${instance.vcpu} planning units, ${instance.memoryGiB} GiB management footprint.${query.computeInstance === 'custom' ? ' Custom size is priced from the device monthly override.' : ''}`
        });

        return {
            lineItems: [
                {
                    service: 'Switching hardware',
                    component: `${instance.label} ${PURCHASE_MODE_LABELS[query.computePurchase]}`,
                    usage: `${query.computeCount} unit(s) x ${formatNumber(query.computeHours, 0)} h`,
                    unitBasis: `${formatRate(resolvedRate.value, 4)} / unit-hour`,
                    monthly: monthlyCost,
                    annual: monthlyCost * 12,
                    copyValue: `${instance.label} ${PURCHASE_MODE_LABELS[query.computePurchase]} | ${query.computeCount} units | ${query.computeHours} hours | ${formatCurrency(monthlyCost)}`
                }
            ]
        };
    }
    ...
    function calculateDisk(query, assumptions) {
        if (!query.includeDisk || query.diskStorageGb <= 0) {
            assumptions.push({
                name: 'Wireless access pricing basis',
                value: 'Disabled',
                note: 'No wireless access cost is included in this estimate.'
            });
        ...
        assumptions.push({
            name: 'Wireless access pricing basis',
            value: 'Wireless access starter model',
            note: `Baseline 3000 controller units and 125 Mbps included. Access point ${storageRate.source}, controller capacity ${iopsRate.source}, throughput ${throughputRate.source}.`
        });

        return {
            lineItems: [
                {
                    service: 'Wireless access',
                    component: 'Wireless access units',
                    usage: `${formatNumber(query.diskStorageGb, 0)} unit(s)`,
                    unitBasis: `${formatRate(storageRate.value, 4)} / unit-month`,
                    monthly: storageCost,
                    annual: storageCost * 12,
                    copyValue: `Wireless AP allocation | ${query.diskStorageGb} units | ${formatCurrency(storageCost)}`
                },
                {
                    service: 'Wireless access',
                    component: 'Additional controller capacity',
                    usage: `${formatNumber(additionalIops, 0)} billable units`,
                    unitBasis: `${formatRate(iopsRate.value, 4)} / unit-month`,
                    monthly: iopsCost,
                    annual: iopsCost * 12,
                    copyValue: `Wireless controller capacity | ${additionalIops} billable units | ${formatCurrency(iopsCost)}`
                },
                {
                    service: 'Wireless access',
                    component: 'Additional throughput allowance',
                    usage: `${formatNumber(additionalThroughput, 0)} billable Mbps`,
                    unitBasis: `${formatRate(throughputRate.value, 4)} / Mbps-month`,
                    monthly: throughputCost,
                    annual: throughputCost * 12,
                    copyValue: `Wireless throughput allowance | ${additionalThroughput} Mbps | ${formatCurrency(throughputCost)}`
                }
    ...
    function calculateBlob(query, assumptions) {
        if (
            ...
            assumptions.push({
                name: 'Licensing pricing basis',
                value: 'Disabled',
                note: 'No Licensing and subscription cost is included in this estimate.'
            });
        ...
        assumptions.push({
            name: 'Licensing pricing basis',
            value: 'Licensing starter model',
            note: `License unit ${storageRate.source}. Activity rates use editable starter assumptions for policy checks and configuration changes.`
        });
        ...
        return {
            lineItems: [
                {
                    service: 'Licensing',
                    component: 'License units',
                    usage: `${formatNumber(query.blobStorageGb, 0)} unit-month`,
                    unitBasis: `${formatRate(storageRate.value, 4)} / unit-month`,
                    monthly: storageCost,
                    annual: storageCost * 12,
                    copyValue: `Licensing | ${query.blobStorageGb} units | ${formatCurrency(storageCost)}`
                },
                {
                    service: 'Licensing',
                    component: 'Policy checks',
                    usage: `${formatNumber(query.blobGetRequestsK, 0)} x 1k checks`,
                    unitBasis: `${formatRate(getRate.value, 4)} / 1k checks`,
                    monthly: getCost,
                    annual: getCost * 12,
                    copyValue: `License entitlement checks | ${query.blobGetRequestsK} x 1k | ${formatCurrency(getCost)}`
                },
                {
                    service: 'Licensing',
                    component: 'Config changes',
                    usage: `${formatNumber(query.blobPutRequestsK, 0)} x 1k changes`,
                    unitBasis: `${formatRate(putRate.value, 4)} / 1k changes`,
                    monthly: putCost,
                    annual: putCost * 12,
                    copyValue: `License change events | ${query.blobPutRequestsK} x 1k | ${formatCurrency(putCost)}`
                }
            ].filter((item) => item.monthly > 0),
    ...
    function calculateFunctions(query, assumptions) {
        if (!query.includeFunctions || query.functionsRequestsMillion <= 0) {
            assumptions.push({
                name: 'Controller automation pricing basis',
                value: 'Disabled',
                note: 'No controller automation job or duration cost is included in this estimate.'
            });
        ...
        assumptions.push({
            ...
            note: `Job pricing ${requestRate.source}. Runtime pricing ${gbSecondRate.source}. Worker memory set to ${formatNumber(query.functionsMemoryMb, 0)} MB.`
        });

        return {
            lineItems: [
                {
                    service: 'Controller automation',
                    component: 'Job charges',
                    usage: `${formatNumber(requestCount, 0)} jobs (${formatNumber(billableRequests, 0)} billable)`,
                    unitBasis: `${formatRate(requestRate.value, 2)} / 1M jobs`,
                    monthly: requestCost,
                    annual: requestCost * 12,
                    copyValue: `Controller automation jobs | ${requestCount} jobs | ${formatCurrency(requestCost)}`
                },
                {
                    service: 'Controller automation',
                    component: 'Runtime duration',
                    ...
                    copyValue: `Controller automation runtime | ${formatNumber(totalGbSeconds, 0)} GB-s | ${formatCurrency(computeCost)}`
                }
            ].filter((item) => item.monthly > 0)
        };
    }
    ...
    function calculateApiManagement(query, assumptions) {
        if (!query.includeApi || query.apiRequestsMillion <= 0) {
            assumptions.push({
                name: 'Cloud management pricing basis',
                value: 'Disabled',
                note: 'No cloud management call cost is included in this estimate.'
            });
        ...
        assumptions.push({
            name: 'Cloud management pricing basis',
            value: query.apiType === 'http' ? 'Essentials management starter model' : 'Advantage management starter model',
            ...
            lineItems: [
                {
                    ...
                    component: query.apiType === 'http' ? 'Essentials management calls' : 'Advantage management calls',
                    usage: `${formatNumber(requestCount, 0)} calls (${formatNumber(billableRequests, 0)} billable)`,
                    unitBasis,
                    monthly: requestCost,
                    annual: requestCost * 12,
                    copyValue: `Cloud management ${query.apiType.toUpperCase()} calls | ${requestCount} calls | ${formatCurrency(requestCost)}`
                }
            ].filter((item) => item.monthly > 0),
            egressGb
        };
    ...
    function calculateNetwork(query, combinedEgressGb, assumptions) {
        ...
        assumptions.push({
            name: 'Shared transfer basis',
            value: query.freeTierEgress ? 'Shared 100 GB transfer allowance applied' : 'No transfer allowance applied',
            note: `${egressRate.source}. Transfer includes licensing telemetry, API responses, and shared outbound traffic.`
        });
            ...
            lineItems: [
                {
                    ...
                    copyValue: `Shared transfer | ${formatNumber(combinedEgressGb, 1)} GB | ${formatCurrency(egressCost)}`
                }
            ]
        };
    }
    ...
    function buildRecommendations(query, serviceRows, totals, combinedEgressGb, overridesUsed) {
        ...
        if (query.includeCompute && query.computePurchase === 'onDemand' && query.computeHours >= 500 && query.computeCount > 0) {
            recommendations.push({
                title: 'Steady switching allocation',
                copy: 'Switching hardware is modeled as a steady monthly allocation. Compare that same shape against enterprise agreement or reserved coverage before anyone signs a budget with a brave face.'
            });
        }

        if (combinedEgressGb >= 500) {
            recommendations.push({
                title: 'Transfer is material',
                copy: 'Network transfer is large enough to deserve its own review. Telemetry filtering, caching, or reducing unnecessary exports may move the estimate more than hardware tuning.'
            });
        }
        ...
        if (query.includeFunctions && query.functionsRequestsMillion >= 3 && query.functionsDurationMs >= 250) {
            recommendations.push({
                title: 'Controller automation runtime is noticeable',
                copy: 'Duration-heavy controller automation can creep from elegant to expensive. Review worker sizing, run frequency, and whether any management path belongs on a steadier service.'
            });
        }
        ...
        if (overridesUsed === 0) {
            recommendations.push({
                ...
                copy: 'You are using the bundled starter rates without manual overrides. That is fine for planning, but it is the moment to validate the expensive lines in Cisco commercial pricing before the estimate becomes policy.'
            });
        }
    ...
    function groupServiceRows(lineItems, monthlyTotal) {
        ...
        return Array.from(totalsByService.values())
            ...
            .map((row) => {
                ...
                let signal = 'Light';
                let signalClass = 'calculate-cost-cisco-signal-light';

                if (sharePct >= 40) {
                    signal = 'Heavy';
                    signalClass = 'calculate-cost-cisco-signal-heavy';
                } else if (sharePct >= 15) {
                    signal = 'Medium';
                    signalClass = 'calculate-cost-cisco-signal-medium';
                }
    ...
    function buildEstimate(query) {
        const assumptions = [
            {
                name: 'Model basis',
                value: 'Cisco starter network estimate',
    ...
    function renderSpendRingChart(result) {
        const chartElement = document.getElementById('calculateCostCiscoSpendChart');
        const ringElement = document.getElementById('calculateCostCiscoSpendRing');
    ...
    function buildSummaryHtml(result) {
        ...
        return `
            <header class="calculate-cost-cisco-result-header" aria-label="Result summary header">
                <div class="calculate-cost-cisco-result-header-main">
                    <span class="calculate-cost-cisco-result-header-icon" aria-hidden="true"><i class="bi bi-calculator"></i></span>
                    <div class="calculate-cost-cisco-result-header-copy">
                        <h2 class="calculate-cost-cisco-result-header-title">Result Summary</h2>
                        <p>Overview of the current Cisco estimate result and key metrics</p>
                    </div>
                </div>
                <div class="calculate-cost-cisco-result-header-meta" aria-label="Result summary status">
                    <span class="calculate-cost-cisco-result-header-chip calculate-cost-cisco-result-chip-ready"><span class="calculate-cost-cisco-result-chip-icon" aria-hidden="true"><i class="bi bi-circle-fill"></i></span><span>Estimated</span></span>
                    <span class="calculate-cost-cisco-result-header-chip calculate-cost-cisco-result-chip-updated"><span class="calculate-cost-cisco-result-chip-icon" aria-hidden="true"><i class="bi bi-calendar3"></i></span><span>${escapeHtml(generatedAtText)}</span></span>
                </div>
            </header>

            <div class="calculate-cost-cisco-result-hero-grid" aria-live="polite">
                <article class="calculate-cost-cisco-result-card calculate-cost-cisco-result-card-primary" data-result-visual="ring" aria-label="Primary estimate result">
                    <div class="calculate-cost-cisco-result-primary-heading calculate-cost-cisco-result-visual-copy calculate-cost-cisco-result-visual-copy-top"><div class="calculate-cost-cisco-result-kicker">Primary Result</div><h3 class="calculate-cost-cisco-result-title calculate-cost-cisco-result-title-center">Estimated run rate</h3></div>
                    <div class="calculate-cost-cisco-result-primary-visual" id="calculateCostCiscoResultVisual" aria-label="Primary estimate result visual">
                        <span class="calculate-cost-cisco-result-card-icon calculate-cost-cisco-result-card-icon-primary" aria-hidden="true"><i class="bi bi-cash-coin"></i></span>
                        <div class="calculate-cost-cisco-result-ring calculate-cost-cisco-spend-ring" id="calculateCostCiscoSpendRing" style="--calculate-cost-cisco-result-progress: ${escapeHtml(String(progressAngle))}deg; --progress-angle: ${escapeHtml(String(progressAngle))}deg; --calculate-cost-cisco-result-value-chars: ${escapeHtml(String(primaryValueChars))};" aria-label="Estimated monthly Cisco run rate ${escapeHtml(monthlyTotal)}">
                            <div class="calculate-cost-cisco-spend-chart" id="calculateCostCiscoSpendChart" aria-hidden="true"></div>
                            <div class="calculate-cost-cisco-result-ring-center calculate-cost-cisco-spend-value">
                                <strong class="calculate-cost-cisco-result-ring-value">${escapeHtml(monthlyTotal)}</strong>
                                <span class="calculate-cost-cisco-result-ring-unit">Monthly</span>
                            </div>
                        </div>
                    </div>
                    <div class="calculate-cost-cisco-result-visual-copy">
                        <p class="calculate-cost-cisco-result-copy calculate-cost-cisco-result-copy-center">Modeled monthly total from the current assumptions.</p>
                    </div>
                    <span class="calculate-cost-cisco-result-card-divider" aria-hidden="true"></span>
                    <div class="calculate-cost-cisco-result-chip-row calculate-cost-cisco-result-chip-row-center" aria-label="Primary result outcome">
                        <span class="calculate-cost-cisco-result-chip calculate-cost-cisco-result-chip-outcome calculate-cost-cisco-result-chip-ready"><span class="calculate-cost-cisco-result-chip-icon" aria-hidden="true"><i class="bi bi-cash-coin"></i></span><span>Monthly Estimate</span></span>
                    </div>
                </article>

                <article class="calculate-cost-cisco-result-card calculate-cost-cisco-result-card-summary" aria-label="Estimate summary">
                    <div class="calculate-cost-cisco-result-summary-intro">
                        <span class="calculate-cost-cisco-result-card-icon calculate-cost-cisco-result-card-icon-summary" aria-hidden="true"><i class="bi bi-receipt"></i></span>
                        <div class="calculate-cost-cisco-result-summary-copy">
                            <div class="calculate-cost-cisco-result-kicker">Descriptive Summary</div>
                            <h3 class="calculate-cost-cisco-result-title">Cisco network run-rate estimate</h3>
                            <p class="calculate-cost-cisco-result-copy">Top driver: ${escapeHtml(topDriver)}. Totals are derived from the current assumptions, visible overrides, and generated service mix.</p>
                        </div>
                    </div>
                    <span class="calculate-cost-cisco-result-card-divider" aria-hidden="true"></span>
                    <div class="calculate-cost-cisco-result-chip-grid" aria-label="Estimate state">
                        <span class="calculate-cost-cisco-result-chip calculate-cost-cisco-result-chip-ready"><span class="calculate-cost-cisco-result-chip-icon" aria-hidden="true"><i class="bi bi-check2-circle"></i></span><span>${escapeHtml(presetLabel)}</span></span>
                        <span class="calculate-cost-cisco-result-chip calculate-cost-cisco-result-chip-baseline"><span class="calculate-cost-cisco-result-chip-icon" aria-hidden="true"><i class="bi bi-grid-3x3-gap"></i></span><span>${escapeHtml(String(activeServices))} services</span></span>
                        <span class="calculate-cost-cisco-result-chip calculate-cost-cisco-result-chip-${overrideTone}"><span class="calculate-cost-cisco-result-chip-icon" aria-hidden="true"><i class="bi bi-sliders"></i></span><span>${escapeHtml(String(overrideCount))} overrides</span></span>
                        <span class="calculate-cost-cisco-result-chip calculate-cost-cisco-result-chip-baseline"><span class="calculate-cost-cisco-result-chip-icon" aria-hidden="true"><i class="bi bi-graph-up-arrow"></i></span><span>Top driver: ${escapeHtml(topDriver)}</span></span>
                        <span class="calculate-cost-cisco-result-chip calculate-cost-cisco-result-chip-warning"><span class="calculate-cost-cisco-result-chip-icon" aria-hidden="true"><i class="bi bi-hdd-network"></i></span><span>Egress: ${escapeHtml(combinedEgress)}</span></span>
                    </div>
                </article>
            </div>

            <div class="calculate-cost-cisco-result-metric-grid" aria-label="Estimate metrics">
                <article class="calculate-cost-cisco-result-metric-card calculate-cost-cisco-result-metric-success">
                    <span class="calculate-cost-cisco-result-metric-icon" aria-hidden="true"><i class="bi bi-calendar2-check"></i></span>
                    <span class="calculate-cost-cisco-result-metric-label">Annual</span>
                    <strong class="calculate-cost-cisco-result-metric-value">${escapeHtml(formatCurrency(result.totals.annualTotal))}</strong>
                    <span class="calculate-cost-cisco-result-metric-copy">Projected from the current monthly run rate.</span>
                    <span class="calculate-cost-cisco-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="calculate-cost-cisco-result-metric-card calculate-cost-cisco-result-metric-info">
                    <span class="calculate-cost-cisco-result-metric-icon" aria-hidden="true"><i class="bi bi-sun"></i></span>
                    <span class="calculate-cost-cisco-result-metric-label">Daily</span>
                    <strong class="calculate-cost-cisco-result-metric-value">${escapeHtml(formatCurrency(result.totals.dailyTotal))}</strong>
                    <span class="calculate-cost-cisco-result-metric-copy">Thirty-day planning average.</span>
                    <span class="calculate-cost-cisco-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="calculate-cost-cisco-result-metric-card calculate-cost-cisco-result-metric-accent-tone">
                    <span class="calculate-cost-cisco-result-metric-icon" aria-hidden="true"><i class="bi bi-clock"></i></span>
                    <span class="calculate-cost-cisco-result-metric-label">Hourly</span>
                    <strong class="calculate-cost-cisco-result-metric-value">${escapeHtml(formatCurrency(result.totals.hourlyTotal))}</strong>
                    <span class="calculate-cost-cisco-result-metric-copy">Useful for long-running workload checks.</span>
                    <span class="calculate-cost-cisco-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="calculate-cost-cisco-result-metric-card calculate-cost-cisco-result-metric-warning">
                    <span class="calculate-cost-cisco-result-metric-icon" aria-hidden="true"><i class="bi bi-hdd-network"></i></span>
                    <span class="calculate-cost-cisco-result-metric-label">Egress</span>
                    <strong class="calculate-cost-cisco-result-metric-value">${escapeHtml(combinedEgress)}</strong>
                    <span class="calculate-cost-cisco-result-metric-copy">Combined modeled outbound traffic.</span>
                    <span class="calculate-cost-cisco-result-metric-accent" aria-hidden="true"></span>
                </article>
            </div>
        `;
    ...
    function buildServiceRow(row, index) {
        return `
            <tr>
                ...
                <td class="calculate-cost-cisco-currency">${escapeHtml(formatCurrency(row.monthly))}</td>
    ...
    function buildRecommendationsHtml(recommendations) {
        return recommendations.map((recommendation) => `
            <section class="calculate-cost-cisco-recommendation-card">
                <span class="calculate-cost-cisco-basis-label">Recommendation</span>
                <strong>${escapeHtml(recommendation.title)}</strong>
                <span class="calculate-cost-cisco-recommendation-copy">${escapeHtml(recommendation.copy)}</span>
            </section>
        `).join('');
    }
    ...
    function buildMethodologyHtml(result) {
        return `
            <section class="calculate-cost-cisco-method-card">
                <span class="calculate-cost-cisco-method-label">Model Scope</span>
                <span class="calculate-cost-cisco-method-title">Included spend drivers</span>
                <span class="calculate-cost-cisco-method-copy">This model covers switching hardware, wireless access, licensing, controller automation, cloud management, shared transfer, support uplift, contingency, and any manual monthly adjustment you applied.</span>
            </section>
            <section class="calculate-cost-cisco-method-card">
                <span class="calculate-cost-cisco-method-label">Model Gaps</span>
                <span class="calculate-cost-cisco-method-title">What is not priced automatically</span>
                <span class="calculate-cost-cisco-method-copy">Taxes, cabling, optics, rack work, WAN circuits, professional services, observability, backups, support plan minimums, and private discount agreements are not automatically derived. Add them as explicit overrides or manual adjustments when they matter.</span>
            </section>
        `;
    }
    ...
    function renderResult(result) {
        destroySpendRingChart();
        resultSummary.classList.add('calculate-cost-cisco-result-summary');
    ...
    form.addEventListener('submit', function (event) {
        ...
        setLoadingState('Estimating Cisco monthly planning spend...');
...
(function setupCalculateCostCiscoTableOutputStandard() {
    const rootSelector = '.calculate-cost-cisco-tool';
    const tableSelector = '.tool-result-table tbody tr, .calculate-cost-cisco-table tbody tr';
    const tbodySelector = '.tool-result-table tbody, .calculate-cost-cisco-table tbody';
    const clampClass = 'calculate-cost-cisco-table-cell-text';
    const cellClampClass = 'calculate-cost-cisco-cell-clamp';
    const statusColumnClass = 'calculate-cost-cisco-table-status-cell';
    ...
    function applyStatusAlignment(root) {
        root.querySelectorAll('.tool-result-table, .calculate-cost-cisco-table').forEach(function alignStatusTable(table) {
