...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-overview">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-compass" aria-hidden="true"></i><span>Overview</span></h2>
  <p><strong>Cisco Campus Network Architecture</strong> is an InfraStack prompt-driven workspace for turning a short Cisco campus network brief into a first-pass architecture diagram. The workspace reads explicit terms such as <span class="architecture-campus-network-cisco-term-accent">site</span>, <span class="architecture-campus-network-cisco-term-accent">building or access block</span>, <span class="architecture-campus-network-cisco-term-accent">campus IP plan</span>, ingress services, workload tier, data tier, egress mode, observability controls, and hybrid links, then uses that normalized model to render the stage, technical inventory, prompt notes, and export state.</p>
  <p><a id="architecture-campus-network-cisco-cite-primary" class="architecture-campus-network-cisco-citation-link" href="#architecture-campus-network-cisco-ref-primary"><span class="architecture-campus-network-cisco-citation-inline">Cisco (2024)</span></a> documents campus network architecture as a structured foundation for wired, wireless, WAN edge, management, and high-availability design. The workspace uses that source-backed boundary as the anchor for placement, review, and restore behavior.</p>
...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-technical">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-diagram-3" aria-hidden="true"></i><span>Technical Details</span></h2>
  <p>The workspace builds one normalized architecture model from the prompt, selected preset, and current inspector state. That same model drives the visual stage, technical inventory, prompt notes, score or status summary, and JSON export. <a id="architecture-campus-network-cisco-cite-technical-primary" class="architecture-campus-network-cisco-citation-link" href="#architecture-campus-network-cisco-ref-primary"><span class="architecture-campus-network-cisco-citation-inline">Cisco (2024)</span></a> documents campus network architecture as a structured foundation for wired, wireless, WAN edge, management, and high-availability design, so the generated boundary is treated as a planning container rather than a decorative frame. <a id="architecture-campus-network-cisco-cite-method" class="architecture-campus-network-cisco-citation-link" href="#architecture-campus-network-cisco-ref-method"><span class="architecture-campus-network-cisco-citation-inline">Cisco (n.d.)</span></a> describes HSRP as a first-hop redundancy protocol for transparent gateway failover, which is why egress and private-access choices are exposed as reviewable architecture intent instead of hidden assumptions.</p>
...
<h3 class="architecture-campus-network-cisco-technical-step-heading">8. Limits and review points</h3>
<p><a id="architecture-campus-network-cisco-cite-review" class="architecture-campus-network-cisco-citation-link" href="#architecture-campus-network-cisco-ref-review"><span class="architecture-campus-network-cisco-citation-inline">Cisco Meraki (n.d.)</span></a> documents cloud-managed network licensing considerations that can affect operating model and platform selection, so this page treats the generated output as review material rather than validation evidence. The workspace does not certify security, resilience, compliance, production readiness, cost accuracy, service availability, or implementation completeness. It also does not perform detailed subnet math, policy analysis, failover simulation, performance sizing, or change-control approval. A useful first pass can still be wrong when the prompt is vague, the selected preset is a poor fit, or the real environment has constraints that the browser-side model cannot see.</p>

<h3 class="architecture-campus-network-cisco-technical-step-heading">9. Practical review workflow</h3>
...
<h3 class="architecture-campus-network-cisco-technical-step-heading">10. Handoff checklist</h3>
<p>A clean handoff includes the diagram, the inventory rows, the prompt notes, and the JSON state. The diagram explains the visible model, but the notes explain how the workspace interpreted the prompt. The inventory gives reviewers a compact list of components, placements, and purposes. The JSON keeps the model editable so another reviewer can reopen the same state instead of recreating it from memory. For Cisco Campus Network Architecture, the handoff should also state which choices came from the prompt, which choices came from the preset, and which choices were corrected after generation. That distinction matters because a diagram that looks polished can still hide a weak assumption if the team does not know where a value came from.</p>
...
<h3 class="architecture-campus-network-cisco-technical-step-heading">11. What the export does not prove</h3>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Boundary</td><td>Renders the campus fabric as the main ownership and placement container.</td><td>Confirm the boundary matches the intended trust and routing model.</td></tr>
    <tr><td>Ingress</td><td>Places edge services such as internet edge and WAN edge before workloads when enabled.</td><td>Review public entry, filtering, and load-balancing assumptions.</td></tr>
...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-examples">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-chat-square-text" aria-hidden="true"></i><span>Example Prompts</span></h2>
...
<pre class="architecture-campus-network-cisco-prompt-pre"><code>Create a Cisco Campus Network Architecture in headquarters campus with campus IP plan 10.70.0.0/16. Use redundant core and distribution layers, access switch blocks, VLANs with SVI gateways, HSRP, EtherChannel uplinks, wireless, WAN edge, Cisco Secure Firewall, DHCP, DNS, monitoring, and syslog.</code></pre>

<details class="architecture-campus-network-cisco-prompt-note">
  <summary>
    <span class="architecture-campus-network-cisco-prompt-note-label architecture-campus-network-cisco-prompt-note-label-closed">Show prompt use</span>
    <span class="architecture-campus-network-cisco-prompt-note-label architecture-campus-network-cisco-prompt-note-label-open">Hide prompt use</span>
    <button type="button" class="architecture-campus-network-cisco-prompt-copy-btn" data-prompt-copy-index="0">
...
<pre class="architecture-campus-network-cisco-prompt-pre"><code>Build a Cisco campus architecture for a three-building site with redundant core switches, distribution pairs, access blocks, VLANs for users, voice, wireless, and servers, HSRP gateway redundancy, EtherChannel uplinks, wireless controllers, WAN edge, Cisco Secure Firewall, DHCP, DNS, monitoring, and syslog.</code></pre>

<details class="architecture-campus-network-cisco-prompt-note">
  <summary>
    <span class="architecture-campus-network-cisco-prompt-note-label architecture-campus-network-cisco-prompt-note-label-closed">Show prompt use</span>
    <span class="architecture-campus-network-cisco-prompt-note-label architecture-campus-network-cisco-prompt-note-label-open">Hide prompt use</span>
    <button type="button" class="architecture-campus-network-cisco-prompt-copy-btn" data-prompt-copy-index="1">
      <i class="bi bi-clipboard" aria-hidden="true"></i>
      <span>Copy prompt</span>
    </button>
  </summary>
...
<pre class="architecture-campus-network-cisco-prompt-pre"><code>Draft a Cisco campus network for a branch site with collapsed core, two access switch stacks, guest wireless, voice VLAN, WAN edge, firewall, site-to-site VPN, DHCP, DNS, monitoring, and a clear management network.</code></pre>

<details class="architecture-campus-network-cisco-prompt-note">
  <summary>
    <span class="architecture-campus-network-cisco-prompt-note-label architecture-campus-network-cisco-prompt-note-label-closed">Show prompt use</span>
    <span class="architecture-campus-network-cisco-prompt-note-label architecture-campus-network-cisco-prompt-note-label-open">Hide prompt use</span>
    <button type="button" class="architecture-campus-network-cisco-prompt-copy-btn" data-prompt-copy-index="2">
      <i class="bi bi-clipboard" aria-hidden="true"></i>
      <span>Copy prompt</span>
    </button>
  </summary>
...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-prompt-tips">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-lightbulb" aria-hidden="true"></i><span>Prompt Tips</span></h2>
  <p>The generator works best when the prompt names the important Cisco campus network decisions directly. Write for the parser and the reviewer, not for a marketing brief.</p>
...
<details class="tool-guidance-item" open>
  ...
  <div class="tool-guidance-answer">
    ...
    <ul>
      <li>Name the site, building or access block count, and campus IP plan when those values matter.</li>
<li>Describe the main entry path with terms such as <code>internet edge</code>, <code>WAN edge</code>, <code>Cisco Secure Firewall</code>.</li>
...
<details class="tool-guidance-item">
  ...
  <div class="tool-guidance-answer">
    <p>Clean prompts isolate one scenario and use exact Cisco campus network terms the parser can map consistently.</p>
    <ul>
...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-how-to">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-list-check" aria-hidden="true"></i><span>How To Use</span></h2>
...
<details class="tool-guidance-item" open>
  ...
  <div class="tool-guidance-answer">
    <p>Select a preset when you want a stable Cisco campus network pattern, or write a focused brief that names scope, placement, connectivity, and controls.</p>
  </div>
</details>
...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-export">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-box-arrow-down" aria-hidden="true"></i><span>Export Notes</span></h2>
...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-faq">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-question-circle" aria-hidden="true"></i><span>FAQ</span></h2>
...
<details class="faq-item" open>
  ...
  <div class="faq-answer"><p>No. The workspace uses deterministic Cisco campus network-specific prompt extraction and local browser-side state handling.</p></div>
</details>
...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-acronyms">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-type" aria-hidden="true"></i><span>Acronyms</span></h2>
  ...
  <table>
    ...
    <tbody>
      <tr><td>VLAN</td><td>Virtual Local Area Network</td><td>Represents logical segmentation across campus access blocks.</td></tr>
      <tr><td>SVI</td><td>Switched Virtual Interface</td><td>Represents Layer 3 gateway intent for a VLAN.</td></tr>
      <tr><td>HSRP</td><td>Hot Standby Router Protocol</td><td>Represents first-hop gateway redundancy.</td></tr>
      <tr><td>WLAN</td><td>Wireless Local Area Network</td><td>Represents wireless access and controller placement.</td></tr>
      <tr><td>WAN</td><td>Wide Area Network</td><td>Represents external or intersite connectivity.</td></tr>
      <tr><td>AAA</td><td>Authentication, Authorization, and Accounting</td><td>Represents identity and access-control infrastructure.</td></tr>
...
<div class="content-card architecture-campus-network-cisco-markdown-card architecture-campus-network-cisco-markdown-card-citations">
  <h2 class="architecture-campus-network-cisco-section-heading"><i class="bi bi-journal-text" aria-hidden="true"></i><span>References</span></h2>
  ...
  <table class="architecture-campus-network-cisco-citation-table">
    <thead>
      <tr>
        <th>Source type</th>
    ...
    <tbody>
      <tr id="architecture-campus-network-cisco-ref-primary"><td>Website</td><td><a class="architecture-campus-network-cisco-citation-backlink" href="#architecture-campus-network-cisco-cite-primary"><span class="architecture-campus-network-cisco-citation-inline">(Cisco, 2024)</span></a></td><td>Cisco. (2024). <em>Campus LAN and wireless LAN design guide</em>. Cisco Design Zone. Retrieved May 17, 2026, from <a href="https://www.cisco.com/c/en/us/solutions/enterprise/design-zone-campus/index.html">https://www.cisco.com/c/en/us/solutions/enterprise/design-zone-campus/index.html</a></td></tr>
      <tr id="architecture-campus-network-cisco-ref-method"><td>Website</td><td><a class="architecture-campus-network-cisco-citation-backlink" href="#architecture-campus-network-cisco-cite-method"><span class="architecture-campus-network-cisco-citation-inline">(Cisco, n.d.)</span></a></td><td>Cisco. (n.d.). <em>Hot Standby Router Protocol features and functionality</em>. Cisco Support. Retrieved May 17, 2026, from <a href="https://www.cisco.com/c/en/us/support/docs/ip/hot-standby-router-protocol-hsrp/13780-6.html">https://www.cisco.com/c/en/us/support/docs/ip/hot-standby-router-protocol-hsrp/13780-6.html</a></td></tr>
      <tr id="architecture-campus-network-cisco-ref-review"><td>Website</td><td><a class="architecture-campus-network-cisco-citation-backlink" href="#architecture-campus-network-cisco-cite-review"><span class="architecture-campus-network-cisco-citation-inline">(Cisco Meraki, n.d.)</span></a></td><td>Cisco Meraki. (n.d.). <em>Meraki licensing</em>. Retrieved May 17, 2026, from <a href="https://documentation.meraki.com/General_Administration/Licensing">https://documentation.meraki.com/General_Administration/Licensing</a></td></tr>
    </tbody>
  </table>
</div>
