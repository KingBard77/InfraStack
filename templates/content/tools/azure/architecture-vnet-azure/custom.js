...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function buildSvgMarkup(spec, layoutOverrides, connectorOverrides) {
        ...
        return {
            svgMarkup: [
                ...
                '.diagram-marquee-selection{fill:rgba(6,182,212,0.1);stroke:#ffffff;stroke-width:3;stroke-dasharray:8 6;pointer-events:none;filter:drop-shadow(0 6px 14px rgba(15,23,42,0.24));}',
                ...
                '.diagram-connector-anchor-handle{fill:#ffffff;stroke:#2563EB;stroke-width:3;cursor:grab;pointer-events:all;filter:drop-shadow(0 4px 8px rgba(37,99,235,0.24));}',
                '.diagram-connector-anchor-handle:active{cursor:grabbing;}',
                '.diagram-connector-bend-handle{fill:#2563EB;stroke:#ffffff;stroke-width:3;cursor:move;pointer-events:all;filter:drop-shadow(0 4px 8px rgba(37,99,235,0.28));}',
                ...
                ']]></style>',
                '<marker id="architectureVnetAzureArrow" markerWidth="10" markerHeight="10" refX="8" refY="5" orient="auto" markerUnits="strokeWidth">',
                '<path d="M 0 0 L 10 5 L 0 10 z" fill="#111827"></path>',
                '</marker>',
                '<marker id="architectureVnetAzureArrowActive" markerWidth="10" markerHeight="10" refX="8" refY="5" orient="auto" markerUnits="strokeWidth">',
                '<path d="M 0 0 L 10 5 L 0 10 z" fill="#111827"></path>',
                '</marker>',
                '</defs>',
    ...
    function buildExportPayload(spec, inventory, layoutOverrides, connectorOverrides) {
        return ArchitectureVnetAzureModelCore.buildExportPayload(spec, inventory, layoutOverrides, connectorOverrides, buildNotePayload(spec));
    }
    ...
    function renderOutputScore(spec) {
        ...
        outputStatus.innerHTML = [
            '<div class="architecture-vnet-azure-score-ring-card architecture-vnet-azure-score-ring-card-' + escapeHtml(scorePayload.badgeTone) + '" tabindex="0" role="group" aria-label="Architecture score ' + escapeHtml(String(scorePayload.score)) + ' out of 100. ' + escapeHtml(scorePayload.ringLabel) + '">',
            '<div class="architecture-vnet-azure-score-value" style="--progress-angle: ' + escapeHtml(String(ringProgressAngle)) + 'deg;" aria-label="Architecture score ' + escapeHtml(String(scorePayload.score)) + ' out of 100">',
            '<svg class="architecture-vnet-azure-score-ring" viewBox="0 0 140 140" aria-hidden="true" focusable="false">',
            '<circle class="architecture-vnet-azure-score-ring-track" cx="70" cy="70" r="' + escapeHtml(String(ringRadius)) + '"></circle>',
            '<circle class="architecture-vnet-azure-score-ring-glow" cx="70" cy="70" r="' + escapeHtml(String(ringRadius)) + '" transform="rotate(-90 70 70)" stroke-dasharray="' + escapeHtml(String(ringProgressLength)) + ' ' + escapeHtml(String(ringGapLength)) + '"></circle>',
            '<circle class="architecture-vnet-azure-score-ring-fill" cx="70" cy="70" r="' + escapeHtml(String(ringRadius)) + '" transform="rotate(-90 70 70)" stroke-dasharray="' + escapeHtml(String(ringProgressLength)) + ' ' + escapeHtml(String(ringGapLength)) + '"></circle>',
            '</svg>',
            '<div class="architecture-vnet-azure-score-center">',
            '<span class="architecture-vnet-azure-score-value-number">' + escapeHtml(String(scorePayload.score)) + '</span>',
            '<span class="architecture-vnet-azure-score-caption">/100</span>',
            '</div>',
            '</div>',
            '<span class="architecture-vnet-azure-score-label">',
            '<span class="architecture-vnet-azure-score-label-orb" aria-hidden="true"><i class="' + escapeHtml(scorePayload.badgeIcon) + '"></i></span>',
            '<span class="architecture-vnet-azure-score-label-text">' + escapeHtml(scorePayload.ringLabel) + '</span>',
            '</span>',
            '</div>',
            '<div class="architecture-vnet-azure-score-copy">',
            '<div class="architecture-vnet-azure-score-kicker">Architecture Score</div>',
            '<div class="architecture-vnet-azure-score-summary">' + escapeHtml(scorePayload.band) + '</div>',
            '<div class="architecture-vnet-azure-score-detail">' + escapeHtml(scorePayload.detail) + '</div>',
            '<div class="architecture-vnet-azure-score-tags">',
            [
                '<span class="architecture-vnet-azure-score-tag architecture-vnet-azure-score-tag-status architecture-vnet-azure-score-tag-status-' + escapeHtml(scorePayload.badgeTone) + '">',
            ...
            scorePayload.tags.map(function (item) {
                return [
                    '<span class="architecture-vnet-azure-score-tag architecture-vnet-azure-score-tag-' + escapeHtml(item.tone) + '">',
