(function (root, factory) {
    ...
    root.CalculateCostAzureModelCore = core;
}(typeof self !== 'undefined' ? self : this, function () {
    'use strict';

    const TOOL_ID = 'calculate-cost-azure';
    const VERSION = '1.0.0';
    const HOURS_PER_MONTH = 730;

    const computeProfiles = {
        'b2s': {
            label: 'B2s - 2 vCPU - 4 GiB',
            hourly: 0.0416,
            vcpu: 2,
            memoryGiB: 4
        },
        'd2s-v5': {
            label: 'D2s v5 - 2 vCPU - 8 GiB',
            hourly: 0.096,
            vcpu: 2,
            memoryGiB: 8
        },
        'd4s-v5': {
            label: 'D4s v5 - 4 vCPU - 16 GiB',
            hourly: 0.192,
            vcpu: 4,
            memoryGiB: 16
        },
        'd8s-v5': {
            label: 'D8s v5 - 8 vCPU - 32 GiB',
            hourly: 0.384,
            vcpu: 8,
            memoryGiB: 32
        },
        'f4s-v2': {
            label: 'F4s v2 - 4 vCPU - 8 GiB',
            hourly: 0.169,
            vcpu: 4,
            memoryGiB: 8
        }
    ...
    const databaseTiers = {
        ...
        'sql-standard': {
            label: 'Azure SQL Standard S0',
            monthly: 15
        },
        'sql-business': {
            label: 'Azure SQL Business Critical starter',
            monthly: 280
        },
        'postgres-flex': {
            label: 'Azure Database for PostgreSQL flexible starter',
            monthly: 160
        }
    };
    ...
    /**
     * Normalizes raw calculator input into the Azure cost model.
     *
    ...
    function normalizeState(input) {
        ...
        return {
            ...
            region: chooseString(source.region, defaultState.region, ['eastus', 'eastus2', 'westus3', 'centralus', 'westeurope', 'southeastasia']),
    ...
    /**
     * Builds a full Azure estimate from normalized or raw state.
     *
    ...
    function buildEstimate(input) {
        ...
        if (state.services.compute.include && state.services.compute.count > 0) {
            ...
            addLine(
                lines,
                'Compute',
                'Azure Virtual Machines - ' + profile.label,
        ...
        if (state.services.storage.include && state.services.storage.storageGb > 0) {
            ...
            addLine(lines, 'Storage', 'Azure Blob Storage capacity', state.services.storage.storageGb, 'GB-month', storageRate, storageMonthly, 'Hot LRS starter assumption');
            addLine(lines, 'Storage', 'Blob read/write operations', state.services.storage.transactionsMillion, 'million operations', opsRate, opsMonthly, 'Request-volume planning input');
        }
        ...
        if (state.services.serverless.include) {
            ...
            addLine(lines, 'Serverless', 'Azure Functions requests', state.services.serverless.requestsMillion, 'million requests', requestRate, requestMonthly, 'Consumption-style request estimate');
            addLine(lines, 'Serverless', 'Azure Functions execution', state.services.serverless.gbSeconds, 'GB-seconds', gbSecondRate, executionMonthly, 'Runtime and memory estimate');
        }
        ...
        if (state.services.network.include) {
            ...
            if (state.services.network.edgeMonthly > 0) {
                addLine(lines, 'Network', 'Application Gateway or Front Door allowance', 1, 'monthly allowance', state.services.network.edgeMonthly, state.services.network.edgeMonthly, 'Visible edge-service placeholder');
            }
        }
    ...
    function buildAssumptions(state) {
        return [
            ...
            ['Support uplift', state.assumptions.supportPct + '%', 'Planning allowance, not a selected Microsoft support plan.'],
    ...
    function buildRecommendations(state, lines, total) {
        ...
        if (!driver) {
            recommendations.push('Add at least one included Azure service before using the estimate in review.');
        } else {
        ...
        if (state.assumptions.commitmentDiscountPct === 0 && state.services.compute.include) {
            recommendations.push('Compare pay-as-you-go with Azure savings plans or reservations when VM runtime is steady.');
        }
        ...
        recommendations.push('Validate material values in Azure Pricing Calculator, Retail Prices API data, billing exports, or agreement pricing before committing.');

        return recommendations;
    }

    /**
     * Builds an exportable JSON payload for the current Azure estimate.
     *
     * @param {object} input Raw or normalized estimate state.
    ...
    /**
     * Builds restored state from an imported Azure calculator payload.
     *
    ...
    function buildImportedPayloadState(payload) {
        if (!payload || payload.tool !== TOOL_ID || typeof payload.state !== 'object') {
            return {
                error: 'Invalid Azure cost calculator JSON.'
            };
        }
