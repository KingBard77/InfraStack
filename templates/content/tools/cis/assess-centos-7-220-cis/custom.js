...
installInfraStackResultSummaryNormalizer('assess-centos-7-220-cis');
// ns:end family._base.workspace.05_result-summary

const assessCentos7220CisData = {{ include('content/tools/cis/assess-centos-7-220-cis/assets/custom.json.twig')|raw }};
const assessCentos7220CisScriptEndpoint = '{{ path('app_cis_assess_centos_7_220_cis_script') }}';

// ns:start family._base.workspace.00_shell
document.addEventListener('DOMContentLoaded', function () {
    const benchmark = assessCentos7220CisData.benchmark || {};
    const families = Array.isArray(assessCentos7220CisData.families) ? assessCentos7220CisData.families.slice() : [];
    const sections = Array.isArray(assessCentos7220CisData.sections) ? assessCentos7220CisData.sections.slice() : [];
    const controls = Array.isArray(assessCentos7220CisData.controls) ? assessCentos7220CisData.controls.slice() : [];
// ns:start family._base.workspace.01_input-brief
    const form = document.getElementById('assessCentos7220CisForm');
    const queryInput = document.getElementById('assessCentos7220CisQuery');
    const submitButton = document.getElementById('assessCentos7220CisSubmit');
    const resetButton = document.getElementById('assessCentos7220CisReset');
// ns:end family._base.workspace.01_input-brief
// ns:start family._base.workspace.02_basic-settings
    const familyInput = document.getElementById('assessCentos7220CisFamily');
    const sectionPathInput = document.getElementById('assessCentos7220CisSectionPath');
    const criticalityInput = document.getElementById('assessCentos7220CisCriticality');
    const selectedControlInput = document.getElementById('assessCentos7220CisSelectedControl');
    const selectedControlTrigger = document.getElementById('assessCentos7220CisSelectedControlTrigger');
    const selectedControlLabel = document.getElementById('assessCentos7220CisSelectedControlLabel');
    const selectedControlPanel = document.getElementById('assessCentos7220CisControlPickerPanel');
    const selectedControlSearch = document.getElementById('assessCentos7220CisControlPickerSearch');
    const selectedControlOptions = document.getElementById('assessCentos7220CisControlPickerOptions');
// ns:end family._base.workspace.02_basic-settings
    const sortInput = document.getElementById('assessCentos7220CisSort');
    const sortSummary = document.getElementById('assessCentos7220CisSortSummary');
    const sortSelect = document.getElementById('assessCentos7220CisSortSelect');
    const sortOptionsContainer = document.getElementById('assessCentos7220CisSortOptions');
    const rowLimitInput = document.getElementById('assessCentos7220CisRowLimit');
    const criticalityRow = document.getElementById('assessCentos7220CisCriticalityRow');
    const resultEmpty = document.getElementById('assessCentos7220CisResultEmpty');
    const resultError = document.getElementById('assessCentos7220CisResultError');
    const resultContent = document.getElementById('assessCentos7220CisResultContent');
    const resultSummary = document.getElementById('assessCentos7220CisResultSummary');
    const toolbarMeta = document.getElementById('assessCentos7220CisToolbarMeta');
    const controlsTableBody = document.getElementById('assessCentos7220CisControlsTableBody');
    const sectionsTableBody = document.getElementById('assessCentos7220CisSectionsTableBody');
    const scriptMeta = document.getElementById('assessCentos7220CisScriptMeta');
    const scriptOutput = document.getElementById('assessCentos7220CisScriptOutput');
    const jsonOutput = document.getElementById('assessCentos7220CisJsonOutput');
    const copyScriptButton = document.getElementById('assessCentos7220CisCopyScript');
    const downloadScriptButton = document.getElementById('assessCentos7220CisDownloadScript');
    const fullScriptButton = document.getElementById('assessCentos7220CisFullScript');
    const bundleModal = document.getElementById('assessCentos7220CisBundleModal');
    const bundleCloseButton = document.getElementById('assessCentos7220CisBundleClose');
    const bundleCancelButton = document.getElementById('assessCentos7220CisBundleCancel');
    const bundleDetails = document.getElementById('assessCentos7220CisBundleDetails');
    const bundleRequestText = document.getElementById('assessCentos7220CisBundleRequestText');
    const bundleEmailLink = document.getElementById('assessCentos7220CisBundleEmail');
    const bundleCopyButton = document.getElementById('assessCentos7220CisBundleCopy');
    const exportPdfButton = document.getElementById('assessCentos7220CisExportPdf');
    const downloadCsvButton = document.getElementById('assessCentos7220CisDownloadCsv');
    const copyJsonButton = document.getElementById('assessCentos7220CisCopyJson');
    const downloadJsonButton = document.getElementById('assessCentos7220CisDownloadJson');
    const importJsonButton = document.getElementById('assessCentos7220CisImportJsonButton');
    const importJsonInput = document.getElementById('assessCentos7220CisImportJson');
    const tabButtons = Array.from(document.querySelectorAll('.assess-centos-7-220-cis-tab-btn'));
    const tabPanels = Array.from(document.querySelectorAll('.assess-centos-7-220-cis-tab-panel'));
...
// ns:end family._base.workspace.03_custom-settings
    ...
    function initMarkdownCopyButtons() {
        const promptBlocks = Array.from(document.querySelectorAll('.markdown-content pre.assess-centos-7-220-cis-prompt-pre'));
        const promptCopyButtons = document.querySelectorAll('.assess-centos-7-220-cis-prompt-copy-btn');
        ...
        const commandBlocks = Array.from(document.querySelectorAll('.markdown-content pre.assess-centos-7-220-cis-command-example-pre'));
        const commandCopyButtons = document.querySelectorAll('.assess-centos-7-220-cis-command-copy-btn');
        ...
        const codeBlocks = document.querySelectorAll('.markdown-content pre:not(.assess-centos-7-220-cis-prompt-pre):not(.assess-centos-7-220-cis-command-example-pre)');
    ...
    function renderSortOptions(container, options, selectedValue) {
        container.innerHTML = options.map(function (option) {
            ...
            return `
                <button type="button" class="assess-centos-7-220-cis-sort-option${isActive ? ' is-active' : ''}" data-sort-value="${escapeHtml(option.value)}" aria-pressed="${isActive ? 'true' : 'false'}"${metaAttribute}>
    ...
    function renderControlPickerButtons(options, selectedValue) {
        if (!options.length) {
            selectedControlOptions.innerHTML = '<div class="assess-centos-7-220-cis-control-picker-empty">No matched scripts.</div>';
            return;
        }
        ...
        selectedControlOptions.innerHTML = options.map(function (option) {
            ...
            return '<button type="button" class="assess-centos-7-220-cis-control-picker-option' + (isActive ? ' is-active' : '') + '" data-control-value="' + escapeHtml(option.value) + '" role="option" aria-selected="' + (isActive ? 'true' : 'false') + '"><strong>' + escapeHtml(option.title) + '</strong>' + meta + '</button>';
        }).join('');
    }
    ...
    function renderCriticalityOverview() {
        ...
        criticalityRow.innerHTML = overviewItems.map(function (item) {
            return `<span class="assess-centos-7-220-cis-criticality-pill" data-tone="${criticalityTone(item.value)}"><strong>${counts[item.value] || 0}</strong><span>${item.label}</span></span>`;
        }).join('');
    }
    ...
    function buildSummaryMarkup(result) {
        ...
        return `
            <header class="assess-centos-7-220-cis-result-header" aria-label="Result summary header">
                <div class="assess-centos-7-220-cis-result-header-main">
                    <span class="assess-centos-7-220-cis-result-header-icon" aria-hidden="true"><i class="bi bi-bar-chart-line"></i></span>
                    <div class="assess-centos-7-220-cis-result-header-copy">
                        <h2 class="assess-centos-7-220-cis-result-header-title">Result Summary</h2>
                        <p>Overview of the current assessment results and key metrics</p>
                    </div>
                </div>
                <div class="assess-centos-7-220-cis-result-header-meta" aria-label="Result summary status">
                    <span class="assess-centos-7-220-cis-result-header-chip assess-centos-7-220-cis-result-chip-ready"><span class="assess-centos-7-220-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-circle-fill"></i></span><span>Generated</span></span>
                    <span class="assess-centos-7-220-cis-result-header-chip assess-centos-7-220-cis-result-chip-updated"><span class="assess-centos-7-220-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-calendar3"></i></span><span>${escapeHtml(updatedText)}</span></span>
                </div>
            </header>

            <div class="assess-centos-7-220-cis-result-hero-grid" aria-live="polite">
                <article class="assess-centos-7-220-cis-result-card assess-centos-7-220-cis-result-card-primary" data-result-visual="number" aria-label="Primary result">
                    <div class="assess-centos-7-220-cis-result-primary-heading assess-centos-7-220-cis-result-visual-copy assess-centos-7-220-cis-result-visual-copy-top"><div class="assess-centos-7-220-cis-result-kicker">Primary Result</div><h3 class="assess-centos-7-220-cis-result-title assess-centos-7-220-cis-result-title-center">${escapeHtml(visibleRows)} visible rows</h3></div>
                    <div class="assess-centos-7-220-cis-result-primary-visual" id="assessCentos7220CisResultVisual" aria-label="Primary assessment result">
                        <span class="assess-centos-7-220-cis-result-card-icon assess-centos-7-220-cis-result-card-icon-primary" aria-hidden="true"><i class="bi bi-clipboard-check"></i></span>
                        <div class="assess-centos-7-220-cis-result-primary-number">
                            <strong class="assess-centos-7-220-cis-result-primary-number-value">${escapeHtml(visibleRows)}</strong>
                            <span class="assess-centos-7-220-cis-result-primary-number-unit">visible rows</span>
                        </div>
                    </div>
                    <div class="assess-centos-7-220-cis-result-visual-copy">
                        <p class="assess-centos-7-220-cis-result-copy assess-centos-7-220-cis-result-copy-center">Selected: ${escapeHtml(selectedMeta)}</p>
                    </div>
                    <span class="assess-centos-7-220-cis-result-card-divider" aria-hidden="true"></span>
                    <div class="assess-centos-7-220-cis-result-chip-row assess-centos-7-220-cis-result-chip-row-center" aria-label="Primary result source">
                        <span class="assess-centos-7-220-cis-result-chip assess-centos-7-220-cis-result-chip-outcome assess-centos-7-220-cis-result-chip-ready"><span class="assess-centos-7-220-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-shield-check"></i></span><span>Benchmark: ${escapeHtml(benchmark.name || 'CentOS 7 CIS')}</span></span>
                    </div>
                </article>

                <article class="assess-centos-7-220-cis-result-card assess-centos-7-220-cis-result-card-summary" aria-label="Assessment summary">
                    <div class="assess-centos-7-220-cis-result-summary-intro">
                        <span class="assess-centos-7-220-cis-result-card-icon assess-centos-7-220-cis-result-card-icon-summary" aria-hidden="true"><i class="bi bi-clipboard-data"></i></span>
                        <div class="assess-centos-7-220-cis-result-summary-copy">
                            <div class="assess-centos-7-220-cis-result-kicker">Descriptive Summary</div>
                            <h3 class="assess-centos-7-220-cis-result-title">Filtered CIS control set</h3>
                            <p class="assess-centos-7-220-cis-result-copy">The table, section rollups, selected script body, JSON, and exports are all derived from the current filters.</p>
                        </div>
                    </div>
                    <span class="assess-centos-7-220-cis-result-card-divider" aria-hidden="true"></span>
                    <div class="assess-centos-7-220-cis-result-chip-grid" aria-label="Assessment state">
                        <span class="assess-centos-7-220-cis-result-chip assess-centos-7-220-cis-result-chip-ready"><span class="assess-centos-7-220-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-shield-check"></i></span><span>Benchmark: ${escapeHtml(benchmark.name || 'CentOS 7 CIS')}</span></span>
                        <span class="assess-centos-7-220-cis-result-chip assess-centos-7-220-cis-result-chip-baseline"><span class="assess-centos-7-220-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-diagram-3"></i></span><span>Family: ${escapeHtml(familyLabel)}</span></span>
                        <span class="assess-centos-7-220-cis-result-chip assess-centos-7-220-cis-result-chip-baseline"><span class="assess-centos-7-220-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-folder2-open"></i></span><span>Section: ${escapeHtml(sectionLabel)}</span></span>
                        <span class="assess-centos-7-220-cis-result-chip assess-centos-7-220-cis-result-chip-baseline"><span class="assess-centos-7-220-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-flag"></i></span><span>Criticality: ${escapeHtml(criticalityLabel)}</span></span>
                        <span class="assess-centos-7-220-cis-result-chip assess-centos-7-220-cis-result-chip-warning"><span class="assess-centos-7-220-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-funnel"></i></span><span>${escapeHtml(queryLabel)}</span></span>
                    </div>
                </article>
            </div>

            <div class="assess-centos-7-220-cis-result-metric-grid" aria-label="Assessment metrics">
                <article class="assess-centos-7-220-cis-result-metric-card assess-centos-7-220-cis-result-metric-success">
                    <span class="assess-centos-7-220-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-file-earmark-text"></i></span>
                    <span class="assess-centos-7-220-cis-result-metric-label">Matched Scripts</span>
                    <strong class="assess-centos-7-220-cis-result-metric-value">${escapeHtml(matchedScripts)}</strong>
                    <span class="assess-centos-7-220-cis-result-metric-copy">Controls matching the selected filters.</span>
                    <span class="assess-centos-7-220-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-centos-7-220-cis-result-metric-card assess-centos-7-220-cis-result-metric-info">
                    <span class="assess-centos-7-220-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-eye"></i></span>
                    <span class="assess-centos-7-220-cis-result-metric-label">Visible Rows</span>
                    <strong class="assess-centos-7-220-cis-result-metric-value">${escapeHtml(visibleRows)}</strong>
                    <span class="assess-centos-7-220-cis-result-metric-copy">Rows rendered in the controls table.</span>
                    <span class="assess-centos-7-220-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-centos-7-220-cis-result-metric-card assess-centos-7-220-cis-result-metric-accent-tone">
                    <span class="assess-centos-7-220-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-layers"></i></span>
                    <span class="assess-centos-7-220-cis-result-metric-label">Sections</span>
                    <strong class="assess-centos-7-220-cis-result-metric-value">${escapeHtml(visibleSections)}</strong>
                    <span class="assess-centos-7-220-cis-result-metric-copy">Section rollups from the filtered set.</span>
                    <span class="assess-centos-7-220-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-centos-7-220-cis-result-metric-card assess-centos-7-220-cis-result-metric-warning">
                    <span class="assess-centos-7-220-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-code"></i></span>
                    <span class="assess-centos-7-220-cis-result-metric-label">Selected Script</span>
                    <strong class="assess-centos-7-220-cis-result-metric-value">${escapeHtml(selectedScriptLines)}</strong>
                    <span class="assess-centos-7-220-cis-result-metric-copy">${escapeHtml(selectedScriptCopy)}</span>
                    <span class="assess-centos-7-220-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
            </div>
        `;
    ...
    function renderToolbarMeta(result) {
        ...
        toolbarMeta.innerHTML = `
            <span class="assess-centos-7-220-cis-toolbar-pill">Family scope: ${result.familyScopedControls.length}</span>
            <span class="assess-centos-7-220-cis-toolbar-pill">Section scope: ${result.sectionScopedControls.length}</span>
            <span class="assess-centos-7-220-cis-toolbar-pill">Criticality scope: ${result.criticalityScopedControls.length}</span>
            <span class="assess-centos-7-220-cis-toolbar-pill">${escapeHtml(rowLimitLabel)}</span>
            <span class="assess-centos-7-220-cis-toolbar-pill">Selected: ${escapeHtml(selectedLabel)}</span>
        `;
    }
    ...
    function renderControlsTable(result) {
        if (result.visibleControls.length === 0) {
            controlsTableBody.innerHTML = `
                <tr class="assess-centos-7-220-cis-empty-row">
        ...
        controlsTableBody.innerHTML = result.visibleControls.map(function (control, index) {
            ...
            return `
                <tr data-control-path="${escapeHtml(control.script_path)}" class="${isSelected ? 'assess-centos-7-220-cis-control-row-selected' : ''}">
                  <td class="tool-generated-rownum-cell">${index + 1}</td>
                  <td><span class="assess-centos-7-220-cis-control-id">${escapeHtml(control.id)}</span></td>
                  <td><span class="assess-centos-7-220-cis-title-text">${escapeHtml(control.title)}</span></td>
                  <td>
                    <div class="assess-centos-7-220-cis-section-cell">
                      <span class="assess-centos-7-220-cis-path-text" title="${escapeHtml(control.section_path)}">${escapeHtml(sectionDisplay)}</span>
                      <span class="assess-centos-7-220-cis-section-family">${escapeHtml(control.family_label)}</span>
                    </div>
                  </td>
                  <td><span class="assess-centos-7-220-cis-criticality-badge" data-tone="${escapeHtml(criticalityTone(control.criticality))}">${escapeHtml(criticalityLabel)}</span></td>
                  <td><span class="assess-centos-7-220-cis-script-text" title="${escapeHtml(control.script_path)}">${escapeHtml(scriptDisplay)}</span></td>
                  <td class="assess-centos-7-220-cis-table-action-cell tool-table-action-cell">
                    <button type="button" class="assess-centos-7-220-cis-row-copy" data-control-copy-row="${index}" aria-label="Copy control row ${index + 1}" title="Copy control row">
        ...
        sectionsTableBody.innerHTML = result.visibleSections.map(function (section, index) {
            ...
            return `
                <tr>
                  ...
                  <td>
                    <div class="assess-centos-7-220-cis-section-cell">
                      <span class="assess-centos-7-220-cis-path-text">${escapeHtml(section.path)}</span>
                      <span class="assess-centos-7-220-cis-section-family">${escapeHtml(section.full_label)}</span>
                    </div>
                  </td>
                  <td>${section.depth}</td>
                  <td>${section.visible_control_count}</td>
                  <td>${section.child_count}</td>
                  <td><span class="assess-centos-7-220-cis-range-text">${escapeHtml(rangeText)}</span></td>
                </tr>
            `;
        }).join('');
    }
    ...
    function renderScriptState(control, statusText, scriptContent) {
        if (!control) {
            scriptMeta.innerHTML = `
                <div class="assess-centos-7-220-cis-script-chip">
                  <span class="assess-centos-7-220-cis-script-chip-label">Selection</span>
                  <span class="assess-centos-7-220-cis-script-chip-value">No matched script</span>
                </div>
            `;
        ...
        const statusMarkup = statusText
            ? `
            <div class="assess-centos-7-220-cis-script-chip">
              <span class="assess-centos-7-220-cis-script-chip-label">Status</span>
              <span class="assess-centos-7-220-cis-script-chip-value">${escapeHtml(statusText)}</span>
            </div>
        `
            : '';
        ...
        scriptMeta.innerHTML = `
            <div class="assess-centos-7-220-cis-script-chip">
              <span class="assess-centos-7-220-cis-script-chip-label">Control ID</span>
              <span class="assess-centos-7-220-cis-script-chip-value">${escapeHtml(control.id)}</span>
            </div>
            <div class="assess-centos-7-220-cis-script-chip">
              <span class="assess-centos-7-220-cis-script-chip-label">Title</span>
              <span class="assess-centos-7-220-cis-script-chip-value">${escapeHtml(control.title)}</span>
            </div>
            <div class="assess-centos-7-220-cis-script-chip">
              <span class="assess-centos-7-220-cis-script-chip-label">Criticality</span>
              <span class="assess-centos-7-220-cis-script-chip-value">${escapeHtml(criticalityLabel)}</span>
            </div>
            <div class="assess-centos-7-220-cis-script-chip">
              <span class="assess-centos-7-220-cis-script-chip-label">Section path</span>
              <span class="assess-centos-7-220-cis-script-chip-value">${escapeHtml(control.section_path)}</span>
            </div>
            <div class="assess-centos-7-220-cis-script-chip">
              <span class="assess-centos-7-220-cis-script-chip-label">Script path</span>
              <span class="assess-centos-7-220-cis-script-chip-value">${escapeHtml(control.script_path)}</span>
            </div>
            <div class="assess-centos-7-220-cis-script-chip">
              <span class="assess-centos-7-220-cis-script-chip-label">Lines</span>
              <span class="assess-centos-7-220-cis-script-chip-value">${control.script_line_count}</span>
            </div>
            ${statusMarkup}
        `;
    ...
    async function requestSelectedScript(scriptPath) {
        const response = await fetch(assessCentos7220CisScriptEndpoint, {
            method: 'POST',
            headers: {
    ...
    function renderBundleModal(result) {
        ...
        bundleDetails.innerHTML = bundleRequest.detailRows.map(function (row) {
            return `
                <div class="assess-centos-7-220-cis-bundle-detail">
    ...
    function openBundleModal() {
        ...
        document.body.classList.add('assess-centos-7-220-cis-modal-open');
        bundleCloseButton.focus();
    }

    function closeBundleModal() {
        bundleModal.hidden = true;
        document.body.classList.remove('assess-centos-7-220-cis-modal-open');
...
// ns:end family._base.workspace.08_json-restore
    ...
    function renderResult(result) {
        ...
        showResultState();
        resultSummary.classList.add('assess-centos-7-220-cis-result-summary');
    ...
    selectedControlOptions.addEventListener('click', function (event) {
        ...
        renderExplorer();
        if (latestResult) {
            activateTab('assessCentos7220CisScriptPanel');
        }
    });
    ...
    resetButton.addEventListener('click', function () {
        ...
        showEmptyState();
        activateTab('assessCentos7220CisControlsPanel');
    ...
    controlsTableBody.addEventListener('click', function (event) {
        const target = event.target instanceof HTMLElement ? event.target : null;
        const copyButton = target ? target.closest('.assess-centos-7-220-cis-row-copy') : null;
    ...
    document.addEventListener('click', function (event) {
        const fullScriptTrigger = event.target instanceof HTMLElement ? event.target.closest('#assessCentos7220CisFullScript') : null;
...
// ns:start family._base.workspace.06_output-toolbar
    exportPdfButton.addEventListener('click', function () {
        ...
        exportResultShellAsPdf('assess-centos-7-220-cis', resultContent);
    ...
    downloadCsvButton.addEventListener('click', function () {
        ...
        downloadFile('assess-centos-7-220-cis.csv', latestResult.csv, 'text/csv;charset=utf-8');
    ...
    downloadJsonButton.addEventListener('click', function () {
        ...
        downloadFile('assess-centos-7-220-cis.json', JSON.stringify(latestResult.jsonPayload, null, 2), 'application/json;charset=utf-8');
...
(function setupAssessCentos7220CisTableOutputStandard() {
    const rootSelector = '.assess-centos-7-220-cis-tool';
    const tableSelector = '.tool-result-table tbody tr, .assess-centos-7-220-cis-table tbody tr';
    const tbodySelector = '.tool-result-table tbody, .assess-centos-7-220-cis-table tbody';
    const clampClass = 'assess-centos-7-220-cis-table-cell-text';
    const cellClampClass = 'assess-centos-7-220-cis-cell-clamp';
    const statusColumnClass = 'assess-centos-7-220-cis-table-status-cell';
    ...
    function applyStatusAlignment(root) {
        root.querySelectorAll('.tool-result-table, .assess-centos-7-220-cis-table').forEach(function alignStatusTable(table) {
