...
function check {
    ...
    if [[ -n "$LINE" ]] && echo "$LINE" | grep -Eq "(^|[[:space:]])$REQUIRED([[:space:]]|$)"; then
        ...
        if echo "$LINE" | grep -Eq '(^|[[:space:]])nullok([[:space:]]|$)'; then STATUS="Fail: pam_unix.so includes forbidden argument nullok"; fi
...
function fix {
    ...
    sed -i -E '/pam_unix\.so/ s/(^|[[:space:]])nullok([[:space:]]|$)/ /g' /etc/pam.d/common-password
