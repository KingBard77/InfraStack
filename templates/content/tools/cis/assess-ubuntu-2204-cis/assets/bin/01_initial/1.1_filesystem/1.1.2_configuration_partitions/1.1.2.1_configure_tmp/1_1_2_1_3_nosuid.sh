...
CRITICALITY=1
TITLE="Ensure nosuid options set on /tmp partition"
...
function check {
	...
	if mount | grep '/tmp' | grep "nosuid"  > /dev/null ; then
        ...
        STATUS="No options set nosuid is set on /tmp partition"
    fi
...
function fix {
    ...
    if [ "$CURRENT_OPTIONS" == "$DESIRED_OPTIONS" ]; then
        echo "Options nosuid on /tmp is already configured correctly."
    else
