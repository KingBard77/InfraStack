...
installInfraStackResultSummaryNormalizer('assess-nginx-300-cis');
// ns:end family._base.workspace.05_result-summary

const assessNginx300CisData = {{ include('content/tools/cis/assess-nginx-300-cis/assets/custom.json.twig')|raw }};
const assessNginx300CisScriptEndpoint = '{{ path('app_cis_assess_nginx_300_cis_script') }}';

// ns:start family._base.workspace.00_shell
document.addEventListener('DOMContentLoaded', function () {
    const benchmark = assessNginx300CisData.benchmark || {};
    const families = Array.isArray(assessNginx300CisData.families) ? assessNginx300CisData.families.slice() : [];
    const sections = Array.isArray(assessNginx300CisData.sections) ? assessNginx300CisData.sections.slice() : [];
    const controls = Array.isArray(assessNginx300CisData.controls) ? assessNginx300CisData.controls.slice() : [];
// ns:start family._base.workspace.01_input-brief
    const form = document.getElementById('assessNginx300CisForm');
    const queryInput = document.getElementById('assessNginx300CisQuery');
    const submitButton = document.getElementById('assessNginx300CisSubmit');
    const resetButton = document.getElementById('assessNginx300CisReset');
// ns:end family._base.workspace.01_input-brief
// ns:start family._base.workspace.02_basic-settings
    const familyInput = document.getElementById('assessNginx300CisFamily');
    const sectionPathInput = document.getElementById('assessNginx300CisSectionPath');
    const criticalityInput = document.getElementById('assessNginx300CisCriticality');
    const selectedControlInput = document.getElementById('assessNginx300CisSelectedControl');
    const selectedControlTrigger = document.getElementById('assessNginx300CisSelectedControlTrigger');
    const selectedControlLabel = document.getElementById('assessNginx300CisSelectedControlLabel');
    const selectedControlPanel = document.getElementById('assessNginx300CisControlPickerPanel');
    const selectedControlSearch = document.getElementById('assessNginx300CisControlPickerSearch');
    const selectedControlOptions = document.getElementById('assessNginx300CisControlPickerOptions');
// ns:end family._base.workspace.02_basic-settings
    const sortInput = document.getElementById('assessNginx300CisSort');
    const sortSummary = document.getElementById('assessNginx300CisSortSummary');
    const sortSelect = document.getElementById('assessNginx300CisSortSelect');
    const sortOptionsContainer = document.getElementById('assessNginx300CisSortOptions');
    const rowLimitInput = document.getElementById('assessNginx300CisRowLimit');
    const criticalityRow = document.getElementById('assessNginx300CisCriticalityRow');
    const resultEmpty = document.getElementById('assessNginx300CisResultEmpty');
    const resultError = document.getElementById('assessNginx300CisResultError');
    const resultContent = document.getElementById('assessNginx300CisResultContent');
    const resultSummary = document.getElementById('assessNginx300CisResultSummary');
    const toolbarMeta = document.getElementById('assessNginx300CisToolbarMeta');
    const controlsTableBody = document.getElementById('assessNginx300CisControlsTableBody');
    const sectionsTableBody = document.getElementById('assessNginx300CisSectionsTableBody');
    const scriptMeta = document.getElementById('assessNginx300CisScriptMeta');
    const scriptOutput = document.getElementById('assessNginx300CisScriptOutput');
    const jsonOutput = document.getElementById('assessNginx300CisJsonOutput');
    const copyScriptButton = document.getElementById('assessNginx300CisCopyScript');
    const downloadScriptButton = document.getElementById('assessNginx300CisDownloadScript');
    const fullScriptButton = document.getElementById('assessNginx300CisFullScript');
    const bundleModal = document.getElementById('assessNginx300CisBundleModal');
    const bundleCloseButton = document.getElementById('assessNginx300CisBundleClose');
    const bundleCancelButton = document.getElementById('assessNginx300CisBundleCancel');
    const bundleDetails = document.getElementById('assessNginx300CisBundleDetails');
    const bundleRequestText = document.getElementById('assessNginx300CisBundleRequestText');
    const bundleEmailLink = document.getElementById('assessNginx300CisBundleEmail');
    const bundleCopyButton = document.getElementById('assessNginx300CisBundleCopy');
    const exportPdfButton = document.getElementById('assessNginx300CisExportPdf');
    const downloadCsvButton = document.getElementById('assessNginx300CisDownloadCsv');
    const copyJsonButton = document.getElementById('assessNginx300CisCopyJson');
    const downloadJsonButton = document.getElementById('assessNginx300CisDownloadJson');
    const importJsonButton = document.getElementById('assessNginx300CisImportJsonButton');
    const importJsonInput = document.getElementById('assessNginx300CisImportJson');
    const tabButtons = Array.from(document.querySelectorAll('.assess-nginx-300-cis-tab-btn'));
    const tabPanels = Array.from(document.querySelectorAll('.assess-nginx-300-cis-tab-panel'));
...
// ns:end family._base.workspace.03_custom-settings
    ...
    function initMarkdownCopyButtons() {
        const promptBlocks = Array.from(document.querySelectorAll('.markdown-content pre.assess-nginx-300-cis-prompt-pre'));
        const promptCopyButtons = document.querySelectorAll('.assess-nginx-300-cis-prompt-copy-btn');
        ...
        const commandBlocks = Array.from(document.querySelectorAll('.markdown-content pre.assess-nginx-300-cis-command-example-pre'));
        const commandCopyButtons = document.querySelectorAll('.assess-nginx-300-cis-command-copy-btn');
        ...
        const codeBlocks = document.querySelectorAll('.markdown-content pre:not(.assess-nginx-300-cis-prompt-pre):not(.assess-nginx-300-cis-command-example-pre)');
    ...
    function renderSortOptions(container, options, selectedValue) {
        container.innerHTML = options.map(function (option) {
            ...
            return `
                <button type="button" class="assess-nginx-300-cis-sort-option${isActive ? ' is-active' : ''}" data-sort-value="${escapeHtml(option.value)}" aria-pressed="${isActive ? 'true' : 'false'}"${metaAttribute}>
    ...
    function renderControlPickerButtons(options, selectedValue) {
        if (!options.length) {
            selectedControlOptions.innerHTML = '<div class="assess-nginx-300-cis-control-picker-empty">No matched scripts.</div>';
            return;
        }
        ...
        selectedControlOptions.innerHTML = options.map(function (option) {
            ...
            return '<button type="button" class="assess-nginx-300-cis-control-picker-option' + (isActive ? ' is-active' : '') + '" data-control-value="' + escapeHtml(option.value) + '" role="option" aria-selected="' + (isActive ? 'true' : 'false') + '"><strong>' + escapeHtml(option.title) + '</strong>' + meta + '</button>';
        }).join('');
    }
    ...
    function renderCriticalityOverview() {
        ...
        criticalityRow.innerHTML = overviewItems.map(function (item) {
            return `<span class="assess-nginx-300-cis-criticality-pill" data-tone="${criticalityTone(item.value)}"><strong>${counts[item.value] || 0}</strong><span>${item.label}</span></span>`;
        }).join('');
    }
    ...
    function buildSummaryMarkup(result) {
        ...
        return `
            <header class="assess-nginx-300-cis-result-header" aria-label="Result summary header">
                <div class="assess-nginx-300-cis-result-header-main">
                    <span class="assess-nginx-300-cis-result-header-icon" aria-hidden="true"><i class="bi bi-bar-chart-line"></i></span>
                    <div class="assess-nginx-300-cis-result-header-copy">
                        <h2 class="assess-nginx-300-cis-result-header-title">Result Summary</h2>
                        <p>Overview of the current assessment results and key metrics</p>
                    </div>
                </div>
                <div class="assess-nginx-300-cis-result-header-meta" aria-label="Result summary status">
                    <span class="assess-nginx-300-cis-result-header-chip assess-nginx-300-cis-result-chip-ready"><span class="assess-nginx-300-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-circle-fill"></i></span><span>Generated</span></span>
                    <span class="assess-nginx-300-cis-result-header-chip assess-nginx-300-cis-result-chip-updated"><span class="assess-nginx-300-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-calendar3"></i></span><span>${escapeHtml(updatedText)}</span></span>
                </div>
            </header>

            <div class="assess-nginx-300-cis-result-hero-grid" aria-live="polite">
                <article class="assess-nginx-300-cis-result-card assess-nginx-300-cis-result-card-primary" data-result-visual="number" aria-label="Primary result">
                    <div class="assess-nginx-300-cis-result-primary-heading assess-nginx-300-cis-result-visual-copy assess-nginx-300-cis-result-visual-copy-top"><div class="assess-nginx-300-cis-result-kicker">Primary Result</div><h3 class="assess-nginx-300-cis-result-title assess-nginx-300-cis-result-title-center">${escapeHtml(visibleRows)} visible rows</h3></div>
                    <div class="assess-nginx-300-cis-result-primary-visual" id="assessNginx300CisResultVisual" aria-label="Primary assessment result">
                        <span class="assess-nginx-300-cis-result-card-icon assess-nginx-300-cis-result-card-icon-primary" aria-hidden="true"><i class="bi bi-clipboard-check"></i></span>
                        <div class="assess-nginx-300-cis-result-primary-number">
                            <strong class="assess-nginx-300-cis-result-primary-number-value">${escapeHtml(visibleRows)}</strong>
                            <span class="assess-nginx-300-cis-result-primary-number-unit">visible rows</span>
                        </div>
                    </div>
                    <div class="assess-nginx-300-cis-result-visual-copy">
                        <p class="assess-nginx-300-cis-result-copy assess-nginx-300-cis-result-copy-center">Selected: ${escapeHtml(selectedMeta)}</p>
                    </div>
                    <span class="assess-nginx-300-cis-result-card-divider" aria-hidden="true"></span>
                    <div class="assess-nginx-300-cis-result-chip-row assess-nginx-300-cis-result-chip-row-center" aria-label="Primary result source">
                        <span class="assess-nginx-300-cis-result-chip assess-nginx-300-cis-result-chip-outcome assess-nginx-300-cis-result-chip-ready"><span class="assess-nginx-300-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-shield-check"></i></span><span>Benchmark: ${escapeHtml(benchmark.name || 'NGINX CIS')}</span></span>
                    </div>
                </article>

                <article class="assess-nginx-300-cis-result-card assess-nginx-300-cis-result-card-summary" aria-label="Assessment summary">
                    <div class="assess-nginx-300-cis-result-summary-intro">
                        <span class="assess-nginx-300-cis-result-card-icon assess-nginx-300-cis-result-card-icon-summary" aria-hidden="true"><i class="bi bi-clipboard-data"></i></span>
                        <div class="assess-nginx-300-cis-result-summary-copy">
                            <div class="assess-nginx-300-cis-result-kicker">Descriptive Summary</div>
                            <h3 class="assess-nginx-300-cis-result-title">Filtered CIS control set</h3>
                            <p class="assess-nginx-300-cis-result-copy">The table, section rollups, selected script body, JSON, and exports are all derived from the current filters.</p>
                        </div>
                    </div>
                    <span class="assess-nginx-300-cis-result-card-divider" aria-hidden="true"></span>
                    <div class="assess-nginx-300-cis-result-chip-grid" aria-label="Assessment state">
                        <span class="assess-nginx-300-cis-result-chip assess-nginx-300-cis-result-chip-ready"><span class="assess-nginx-300-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-shield-check"></i></span><span>Benchmark: ${escapeHtml(benchmark.name || 'NGINX CIS')}</span></span>
                        <span class="assess-nginx-300-cis-result-chip assess-nginx-300-cis-result-chip-baseline"><span class="assess-nginx-300-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-diagram-3"></i></span><span>Family: ${escapeHtml(familyLabel)}</span></span>
                        <span class="assess-nginx-300-cis-result-chip assess-nginx-300-cis-result-chip-baseline"><span class="assess-nginx-300-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-folder2-open"></i></span><span>Section: ${escapeHtml(sectionLabel)}</span></span>
                        <span class="assess-nginx-300-cis-result-chip assess-nginx-300-cis-result-chip-baseline"><span class="assess-nginx-300-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-flag"></i></span><span>Criticality: ${escapeHtml(criticalityLabel)}</span></span>
                        <span class="assess-nginx-300-cis-result-chip assess-nginx-300-cis-result-chip-warning"><span class="assess-nginx-300-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-funnel"></i></span><span>${escapeHtml(queryLabel)}</span></span>
                    </div>
                </article>
            </div>

            <div class="assess-nginx-300-cis-result-metric-grid" aria-label="Assessment metrics">
                <article class="assess-nginx-300-cis-result-metric-card assess-nginx-300-cis-result-metric-success">
                    <span class="assess-nginx-300-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-file-earmark-text"></i></span>
                    <span class="assess-nginx-300-cis-result-metric-label">Matched Scripts</span>
                    <strong class="assess-nginx-300-cis-result-metric-value">${escapeHtml(matchedScripts)}</strong>
                    <span class="assess-nginx-300-cis-result-metric-copy">Controls matching the selected filters.</span>
                    <span class="assess-nginx-300-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-nginx-300-cis-result-metric-card assess-nginx-300-cis-result-metric-info">
                    <span class="assess-nginx-300-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-eye"></i></span>
                    <span class="assess-nginx-300-cis-result-metric-label">Visible Rows</span>
                    <strong class="assess-nginx-300-cis-result-metric-value">${escapeHtml(visibleRows)}</strong>
                    <span class="assess-nginx-300-cis-result-metric-copy">Rows rendered in the controls table.</span>
                    <span class="assess-nginx-300-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-nginx-300-cis-result-metric-card assess-nginx-300-cis-result-metric-accent-tone">
                    <span class="assess-nginx-300-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-layers"></i></span>
                    <span class="assess-nginx-300-cis-result-metric-label">Sections</span>
                    <strong class="assess-nginx-300-cis-result-metric-value">${escapeHtml(visibleSections)}</strong>
                    <span class="assess-nginx-300-cis-result-metric-copy">Section rollups from the filtered set.</span>
                    <span class="assess-nginx-300-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-nginx-300-cis-result-metric-card assess-nginx-300-cis-result-metric-warning">
                    <span class="assess-nginx-300-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-code"></i></span>
                    <span class="assess-nginx-300-cis-result-metric-label">Selected Script</span>
                    <strong class="assess-nginx-300-cis-result-metric-value">${escapeHtml(selectedScriptLines)}</strong>
                    <span class="assess-nginx-300-cis-result-metric-copy">${escapeHtml(selectedScriptCopy)}</span>
                    <span class="assess-nginx-300-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
            </div>
        `;
    ...
    function renderToolbarMeta(result) {
        ...
        toolbarMeta.innerHTML = `
            <span class="assess-nginx-300-cis-toolbar-pill">Family scope: ${result.familyScopedControls.length}</span>
            <span class="assess-nginx-300-cis-toolbar-pill">Section scope: ${result.sectionScopedControls.length}</span>
            <span class="assess-nginx-300-cis-toolbar-pill">Criticality scope: ${result.criticalityScopedControls.length}</span>
            <span class="assess-nginx-300-cis-toolbar-pill">${escapeHtml(rowLimitLabel)}</span>
            <span class="assess-nginx-300-cis-toolbar-pill">Selected: ${escapeHtml(selectedLabel)}</span>
        `;
    }
    ...
    function renderControlsTable(result) {
        if (result.visibleControls.length === 0) {
            controlsTableBody.innerHTML = `
                <tr class="assess-nginx-300-cis-empty-row">
        ...
        controlsTableBody.innerHTML = result.visibleControls.map(function (control, index) {
            ...
            return `
                <tr data-control-path="${escapeHtml(control.script_path)}" class="${isSelected ? 'assess-nginx-300-cis-control-row-selected' : ''}">
                  <td class="tool-generated-rownum-cell">${index + 1}</td>
                  <td><span class="assess-nginx-300-cis-control-id">${escapeHtml(control.id)}</span></td>
                  <td><span class="assess-nginx-300-cis-title-text">${escapeHtml(control.title)}</span></td>
                  <td>
                    <div class="assess-nginx-300-cis-section-cell">
                      <span class="assess-nginx-300-cis-path-text" title="${escapeHtml(control.section_path)}">${escapeHtml(sectionDisplay)}</span>
                      <span class="assess-nginx-300-cis-section-family">${escapeHtml(control.family_label)}</span>
                    </div>
                  </td>
                  <td><span class="assess-nginx-300-cis-criticality-badge" data-tone="${escapeHtml(criticalityTone(control.criticality))}">${escapeHtml(criticalityLabel)}</span></td>
                  <td><span class="assess-nginx-300-cis-script-text" title="${escapeHtml(control.script_path)}">${escapeHtml(scriptDisplay)}</span></td>
                  <td class="assess-nginx-300-cis-table-action-cell tool-table-action-cell">
                    <button type="button" class="assess-nginx-300-cis-row-copy" data-control-copy-row="${index}" aria-label="Copy control row ${index + 1}" title="Copy control row">
        ...
        sectionsTableBody.innerHTML = result.visibleSections.map(function (section, index) {
            ...
            return `
                <tr>
                  ...
                  <td>
                    <div class="assess-nginx-300-cis-section-cell">
                      <span class="assess-nginx-300-cis-path-text">${escapeHtml(section.path)}</span>
                      <span class="assess-nginx-300-cis-section-family">${escapeHtml(section.full_label)}</span>
                    </div>
                  </td>
                  <td>${section.depth}</td>
                  <td>${section.visible_control_count}</td>
                  <td>${section.child_count}</td>
                  <td><span class="assess-nginx-300-cis-range-text">${escapeHtml(rangeText)}</span></td>
                </tr>
            `;
        }).join('');
    }
    ...
    function renderScriptState(control, statusText, scriptContent) {
        if (!control) {
            scriptMeta.innerHTML = `
                <div class="assess-nginx-300-cis-script-chip">
                  <span class="assess-nginx-300-cis-script-chip-label">Selection</span>
                  <span class="assess-nginx-300-cis-script-chip-value">No matched script</span>
                </div>
            `;
        ...
        const statusMarkup = statusText
            ? `
            <div class="assess-nginx-300-cis-script-chip">
              <span class="assess-nginx-300-cis-script-chip-label">Status</span>
              <span class="assess-nginx-300-cis-script-chip-value">${escapeHtml(statusText)}</span>
            </div>
        `
            : '';
        ...
        scriptMeta.innerHTML = `
            <div class="assess-nginx-300-cis-script-chip">
              <span class="assess-nginx-300-cis-script-chip-label">Control ID</span>
              <span class="assess-nginx-300-cis-script-chip-value">${escapeHtml(control.id)}</span>
            </div>
            <div class="assess-nginx-300-cis-script-chip">
              <span class="assess-nginx-300-cis-script-chip-label">Title</span>
              <span class="assess-nginx-300-cis-script-chip-value">${escapeHtml(control.title)}</span>
            </div>
            <div class="assess-nginx-300-cis-script-chip">
              <span class="assess-nginx-300-cis-script-chip-label">Criticality</span>
              <span class="assess-nginx-300-cis-script-chip-value">${escapeHtml(criticalityLabel)}</span>
            </div>
            <div class="assess-nginx-300-cis-script-chip">
              <span class="assess-nginx-300-cis-script-chip-label">Section path</span>
              <span class="assess-nginx-300-cis-script-chip-value">${escapeHtml(control.section_path)}</span>
            </div>
            <div class="assess-nginx-300-cis-script-chip">
              <span class="assess-nginx-300-cis-script-chip-label">Script path</span>
              <span class="assess-nginx-300-cis-script-chip-value">${escapeHtml(control.script_path)}</span>
            </div>
            <div class="assess-nginx-300-cis-script-chip">
              <span class="assess-nginx-300-cis-script-chip-label">Lines</span>
              <span class="assess-nginx-300-cis-script-chip-value">${control.script_line_count}</span>
            </div>
            ${statusMarkup}
        `;
    ...
    async function requestSelectedScript(scriptPath) {
        const response = await fetch(assessNginx300CisScriptEndpoint, {
            method: 'POST',
            headers: {
    ...
    function renderBundleModal(result) {
        ...
        bundleDetails.innerHTML = bundleRequest.detailRows.map(function (row) {
            return `
                <div class="assess-nginx-300-cis-bundle-detail">
    ...
    function openBundleModal() {
        ...
        document.body.classList.add('assess-nginx-300-cis-modal-open');
        bundleCloseButton.focus();
    }

    function closeBundleModal() {
        bundleModal.hidden = true;
        document.body.classList.remove('assess-nginx-300-cis-modal-open');
...
// ns:end family._base.workspace.08_json-restore
    ...
    function renderResult(result) {
        ...
        showResultState();
        resultSummary.classList.add('assess-nginx-300-cis-result-summary');
    ...
    selectedControlOptions.addEventListener('click', function (event) {
        ...
        renderExplorer();
        if (latestResult) {
            activateTab('assessNginx300CisScriptPanel');
        }
    });
    ...
    resetButton.addEventListener('click', function () {
        ...
        showEmptyState();
        activateTab('assessNginx300CisControlsPanel');
    ...
    controlsTableBody.addEventListener('click', function (event) {
        const target = event.target instanceof HTMLElement ? event.target : null;
        const copyButton = target ? target.closest('.assess-nginx-300-cis-row-copy') : null;
    ...
    document.addEventListener('click', function (event) {
        const fullScriptTrigger = event.target instanceof HTMLElement ? event.target.closest('#assessNginx300CisFullScript') : null;
...
// ns:start family._base.workspace.06_output-toolbar
    exportPdfButton.addEventListener('click', function () {
        ...
        exportResultShellAsPdf('assess-nginx-300-cis', resultContent);
    ...
    downloadCsvButton.addEventListener('click', function () {
        ...
        downloadFile('assess-nginx-300-cis.csv', latestResult.csv, 'text/csv;charset=utf-8');
    ...
    downloadJsonButton.addEventListener('click', function () {
        ...
        downloadFile('assess-nginx-300-cis.json', JSON.stringify(latestResult.jsonPayload, null, 2), 'application/json;charset=utf-8');
...
(function setupAssessNginx300CisTableOutputStandard() {
    const rootSelector = '.assess-nginx-300-cis-tool';
    const tableSelector = '.tool-result-table tbody tr, .assess-nginx-300-cis-table tbody tr';
    const tbodySelector = '.tool-result-table tbody, .assess-nginx-300-cis-table tbody';
    const clampClass = 'assess-nginx-300-cis-table-cell-text';
    const cellClampClass = 'assess-nginx-300-cis-cell-clamp';
    const statusColumnClass = 'assess-nginx-300-cis-table-status-cell';
    ...
    function applyStatusAlignment(root) {
        root.querySelectorAll('.tool-result-table, .assess-nginx-300-cis-table').forEach(function alignStatusTable(table) {
