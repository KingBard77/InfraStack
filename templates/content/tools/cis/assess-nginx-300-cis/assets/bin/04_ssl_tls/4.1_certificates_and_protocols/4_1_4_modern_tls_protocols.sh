...
function check {
    ...
    PROTOCOLS="$(printf '%s\n' "$NGINX_CONFIG" | awk 'tolower($1) == "ssl_protocols" { $1=""; gsub(";", ""); print; exit }')"

    if [[ -z "$PROTOCOLS" ]] || printf '%s\n' "$PROTOCOLS" | grep -Eq 'SSLv|TLSv1([^.]|$)|TLSv1\.1'; then
        STATUS="Fail: ssl_protocols does not restrict to TLSv1.2/TLSv1.3"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*ssl_protocols[[:space:]]+' "$CIS_BLOCK"
}
