...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq '^[[:space:]]*ssl_stapling[[:space:]]+on;' || ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq '^[[:space:]]*ssl_stapling_verify[[:space:]]+on;'; then
        STATUS="Fail: OCSP stapling and verification are not enabled"
    fi
