...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq 'ssl_session_cache[[:space:]]+shared:' || ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq 'ssl_session_tickets[[:space:]]+off;'; then
        STATUS="Fail: secure TLS session resumption settings were not found"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*(ssl_session_cache|ssl_session_timeout|ssl_session_tickets)[[:space:]]+' "$CIS_BLOCK"
}
