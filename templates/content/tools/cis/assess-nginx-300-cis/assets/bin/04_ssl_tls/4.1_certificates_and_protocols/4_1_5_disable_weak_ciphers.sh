...
function check {
    ...
    CIPHERS="$(printf '%s\n' "$NGINX_CONFIG" | awk 'tolower($1) == "ssl_ciphers" { $1=""; gsub(";", ""); print; exit }')"

    if [[ -z "$CIPHERS" ]] || printf '%s\n' "$CIPHERS" | grep -Eiq '(NULL|EXPORT|DES|3DES|RC4|MD5|aNULL|eNULL)'; then
        STATUS="Fail: ssl_ciphers is missing or contains weak cipher tokens"
    fi
...
function fix {
    ...
    printf -v CIS_BLOCK '%s\n' 'ssl_ciphers '"'"'HIGH:!aNULL:!MD5:!3DES:!DES:!RC4:!EXPORT:!NULL'"'"';' 'ssl_prefer_server_ciphers on;'

    nginx_apply_hardening_block '^[[:space:]]*(ssl_ciphers|ssl_prefer_server_ciphers)[[:space:]]+' "$CIS_BLOCK"
}
