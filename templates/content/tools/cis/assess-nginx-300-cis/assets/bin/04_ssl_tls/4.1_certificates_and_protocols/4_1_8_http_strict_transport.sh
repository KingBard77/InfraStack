...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq 'add_header[[:space:]]+Strict-Transport-Security'; then
        STATUS="Fail: HSTS header is not configured"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*add_header[[:space:]]+Strict-Transport-Security[[:space:]]+' "$CIS_BLOCK"
}
