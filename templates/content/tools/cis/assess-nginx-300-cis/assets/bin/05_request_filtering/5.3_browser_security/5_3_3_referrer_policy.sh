...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq 'add_header[[:space:]]+Referrer-Policy'; then
        STATUS="Fail: Referrer-Policy header is not configured"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*add_header[[:space:]]+Referrer-Policy[[:space:]]+' "$CIS_BLOCK"
}
