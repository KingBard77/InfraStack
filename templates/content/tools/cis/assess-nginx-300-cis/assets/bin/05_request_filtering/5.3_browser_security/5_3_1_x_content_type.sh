...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq 'add_header[[:space:]]+X-Content-Type-Options'; then
        STATUS="Fail: X-Content-Type-Options header is not configured"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*add_header[[:space:]]+X-Content-Type-Options[[:space:]]+' "$CIS_BLOCK"
}
