...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eq 'log_format' || ! printf '%s\n' "$NGINX_CONFIG" | grep -Eq '\$remote_addr|\$request|\$status'; then
        STATUS="Fail: detailed log_format with request context was not found"
    fi
...
function fix {
    ...
    printf -v CIS_BLOCK '%s\n' 'log_format cis '"'"'$remote_addr - $remote_user [$time_local] "$request" $status $body_bytes_sent "$http_referer" "$http_user_agent"'"'"';'

    nginx_apply_hardening_block '^[[:space:]]*log_format[[:space:]]+cis[[:space:]]+' "$CIS_BLOCK"
}
