...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq '^[[:space:]]*access_log[[:space:]]+' || printf '%s\n' "$NGINX_CONFIG" | grep -Eiq '^[[:space:]]*access_log[[:space:]]+off;'; then
        STATUS="Fail: access logging is disabled or not configured"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*access_log[[:space:]]+' "$CIS_BLOCK"
}
