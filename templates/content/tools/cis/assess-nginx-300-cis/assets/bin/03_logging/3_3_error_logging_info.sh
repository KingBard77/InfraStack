...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq '^[[:space:]]*error_log[[:space:]]+[^;]+[[:space:]]+info;'; then
        STATUS="Fail: error_log is not set to info level"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*error_log[[:space:]]+' "$CIS_BLOCK"
}
