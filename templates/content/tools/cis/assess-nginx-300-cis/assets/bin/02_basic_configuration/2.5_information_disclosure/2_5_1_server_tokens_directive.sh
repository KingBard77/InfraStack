...
function check {
    ...
    if ! printf '%s\n' "$NGINX_CONFIG" | grep -Eiq '^[[:space:]]*server_tokens[[:space:]]+off;'; then
        STATUS="Fail: server_tokens is not set to off"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*server_tokens[[:space:]]+' "$CIS_BLOCK"
}
