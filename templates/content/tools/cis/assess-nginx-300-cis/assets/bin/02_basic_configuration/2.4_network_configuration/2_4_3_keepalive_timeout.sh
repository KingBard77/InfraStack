...
function check {
    ...
    VALUE="$(printf '%s\n' "$NGINX_CONFIG" | awk 'tolower($1) == "keepalive_timeout" { gsub(";", "", $2); print $2; exit }')"
    VALUE="${VALUE%s}"

    if [[ ! "$VALUE" =~ ^[0-9]+$ ]] || [[ "$VALUE" -eq 0 ]] || [[ "$VALUE" -gt 10 ]]; then
        STATUS="Fail: keepalive_timeout is not set to 1-10 seconds"
    fi
...
function fix {
    ...
    nginx_apply_hardening_block '^[[:space:]]*keepalive_timeout[[:space:]]+' "$CIS_BLOCK"
}
