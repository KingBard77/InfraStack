...
function check {
    ...
    for CLUSTER in $(aws ecs list-clusters --query 'clusterArns[]' --output text 2>/dev/null); do
        aws ecs describe-clusters --clusters "$CLUSTER" --include SETTINGS --query 'clusters[?settings[?name==`containerInsights` && (value==`enabled` || value==`enhanced`)]].clusterArn' --output text 2>/dev/null | grep -q . || STATUS="Fail: ECS cluster monitoring is not enabled: $CLUSTER"
    done
