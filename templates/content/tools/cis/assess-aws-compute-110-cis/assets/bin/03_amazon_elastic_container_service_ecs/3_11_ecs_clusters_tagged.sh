...
function check {
    ...
    for CLUSTER in $(aws ecs list-clusters --query 'clusterArns[]' --output text 2>/dev/null); do
        aws ecs describe-clusters --clusters "$CLUSTER" --include TAGS --query 'clusters[?!tags || length(tags)==`0`].clusterArn' --output text 2>/dev/null | grep -q . && STATUS="Fail: ECS cluster has no tags: $CLUSTER"
    done
