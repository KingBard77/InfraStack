...
function check {
    ...
    for ARN in $(aws ecs list-task-definitions --query 'taskDefinitionArns[]' --output text 2>/dev/null); do
        aws ecs describe-task-definition --task-definition "$ARN" --query 'taskDefinition.containerDefinitions[].environment[].name' --output text 2>/dev/null | tr '\t' '\n' | grep -Eiq '(password|secret|token|key)' && STATUS="Fail: possible secret environment variable in ECS task definition: $ARN"
    done
