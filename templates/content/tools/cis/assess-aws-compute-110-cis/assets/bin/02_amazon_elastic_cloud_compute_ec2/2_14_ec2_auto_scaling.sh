...
function check {
    ...
    for REGION in $(aws ec2 describe-regions --query 'Regions[].RegionName' --output text); do
        FAILED="$(aws autoscaling describe-auto-scaling-groups --region "$REGION" --query 'AutoScalingGroups[?Tags[?PropagateAtLaunch==`false`]].AutoScalingGroupName' --output text 2>/dev/null)"
        [[ -n "$FAILED" ]] && STATUS="Fail: ASG tags not propagating in $REGION: $FAILED"
    done
