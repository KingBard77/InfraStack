...
function check {
    ...
    for REGION in $(aws ec2 describe-regions --query 'Regions[].RegionName' --output text); do
        FAILED="$(aws ec2 describe-snapshots --region "$REGION" --owner-ids self --query 'Snapshots[?Encrypted==`false`].SnapshotId' --output text 2>/dev/null)"
        [[ -n "$FAILED" ]] && STATUS="Fail: unencrypted EBS snapshots found in $REGION: $FAILED"
    done
