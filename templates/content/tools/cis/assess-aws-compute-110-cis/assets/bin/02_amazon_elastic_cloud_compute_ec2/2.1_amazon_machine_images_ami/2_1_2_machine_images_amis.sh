...
function check {
    ...
    for REGION in $(aws ec2 describe-regions --query 'Regions[].RegionName' --output text); do
        FAILED="$(aws ec2 describe-images --region "$REGION" --owners self --query 'Images[?BlockDeviceMappings[?Ebs.Encrypted==`false`]].ImageId' --output text 2>/dev/null)"
        [[ -n "$FAILED" ]] && STATUS="Fail: unencrypted AMIs found in $REGION: $FAILED"
    done
