...
function check {
    ...
    FAILED="$(aws ec2 describe-volumes --filters Name=status,Values=available --query 'Volumes[].VolumeId' --output text 2>/dev/null)"

    if [[ -n "$FAILED" ]]; then
        STATUS="Fail: unused EBS volumes found: $FAILED"
    fi
