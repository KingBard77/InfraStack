...
function check {
    ...
    for REGION in $(aws ec2 describe-regions --query 'Regions[].RegionName' --output text); do
        for SNAPSHOT in $(aws ec2 describe-snapshots --region "$REGION" --owner-ids self --query 'Snapshots[].SnapshotId' --output text 2>/dev/null); do
            aws ec2 describe-snapshot-attribute --region "$REGION" --snapshot-id "$SNAPSHOT" --attribute createVolumePermission --query 'CreateVolumePermissions[?Group==`all`]' --output text 2>/dev/null | grep -q . && STATUS="Fail: public EBS snapshot found in $REGION: $SNAPSHOT"
        done
    done
