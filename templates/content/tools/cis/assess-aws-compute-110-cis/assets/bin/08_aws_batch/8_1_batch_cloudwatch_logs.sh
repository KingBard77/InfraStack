...
function check {
    ...
    FAILED="$(aws batch describe-job-definitions --status ACTIVE --query 'jobDefinitions[?containerProperties.logConfiguration.logDriver!=`awslogs`].jobDefinitionName' --output text 2>/dev/null)"

    if [[ -n "$FAILED" ]]; then
        STATUS="Fail: active AWS Batch job definitions without awslogs found: $FAILED"
    fi
