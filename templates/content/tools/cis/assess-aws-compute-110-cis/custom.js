...
installInfraStackResultSummaryNormalizer('assess-aws-compute-110-cis');
// ns:end family._base.workspace.05_result-summary

const assessAwsCompute110CisData = {{ include('content/tools/cis/assess-aws-compute-110-cis/assets/custom.json.twig')|raw }};
const assessAwsCompute110CisScriptEndpoint = '{{ path('app_cis_assess_aws_compute_110_cis_script') }}';

// ns:start family._base.workspace.00_shell
document.addEventListener('DOMContentLoaded', function () {
    const benchmark = assessAwsCompute110CisData.benchmark || {};
    const families = Array.isArray(assessAwsCompute110CisData.families) ? assessAwsCompute110CisData.families.slice() : [];
    const sections = Array.isArray(assessAwsCompute110CisData.sections) ? assessAwsCompute110CisData.sections.slice() : [];
    const controls = Array.isArray(assessAwsCompute110CisData.controls) ? assessAwsCompute110CisData.controls.slice() : [];
// ns:start family._base.workspace.01_input-brief
    const form = document.getElementById('assessAwsCompute110CisForm');
    const queryInput = document.getElementById('assessAwsCompute110CisQuery');
    const submitButton = document.getElementById('assessAwsCompute110CisSubmit');
    const resetButton = document.getElementById('assessAwsCompute110CisReset');
// ns:end family._base.workspace.01_input-brief
// ns:start family._base.workspace.02_basic-settings
    const familyInput = document.getElementById('assessAwsCompute110CisFamily');
    const sectionPathInput = document.getElementById('assessAwsCompute110CisSectionPath');
    const criticalityInput = document.getElementById('assessAwsCompute110CisCriticality');
    const selectedControlInput = document.getElementById('assessAwsCompute110CisSelectedControl');
    const selectedControlTrigger = document.getElementById('assessAwsCompute110CisSelectedControlTrigger');
    const selectedControlLabel = document.getElementById('assessAwsCompute110CisSelectedControlLabel');
    const selectedControlPanel = document.getElementById('assessAwsCompute110CisControlPickerPanel');
    const selectedControlSearch = document.getElementById('assessAwsCompute110CisControlPickerSearch');
    const selectedControlOptions = document.getElementById('assessAwsCompute110CisControlPickerOptions');
// ns:end family._base.workspace.02_basic-settings
    const sortInput = document.getElementById('assessAwsCompute110CisSort');
    const sortSummary = document.getElementById('assessAwsCompute110CisSortSummary');
    const sortSelect = document.getElementById('assessAwsCompute110CisSortSelect');
    const sortOptionsContainer = document.getElementById('assessAwsCompute110CisSortOptions');
    const rowLimitInput = document.getElementById('assessAwsCompute110CisRowLimit');
    const criticalityRow = document.getElementById('assessAwsCompute110CisCriticalityRow');
    const resultEmpty = document.getElementById('assessAwsCompute110CisResultEmpty');
    const resultError = document.getElementById('assessAwsCompute110CisResultError');
    const resultContent = document.getElementById('assessAwsCompute110CisResultContent');
    const resultSummary = document.getElementById('assessAwsCompute110CisResultSummary');
    const toolbarMeta = document.getElementById('assessAwsCompute110CisToolbarMeta');
    const controlsTableBody = document.getElementById('assessAwsCompute110CisControlsTableBody');
    const sectionsTableBody = document.getElementById('assessAwsCompute110CisSectionsTableBody');
    const scriptMeta = document.getElementById('assessAwsCompute110CisScriptMeta');
    const scriptOutput = document.getElementById('assessAwsCompute110CisScriptOutput');
    const jsonOutput = document.getElementById('assessAwsCompute110CisJsonOutput');
    const copyScriptButton = document.getElementById('assessAwsCompute110CisCopyScript');
    const downloadScriptButton = document.getElementById('assessAwsCompute110CisDownloadScript');
    const fullScriptButton = document.getElementById('assessAwsCompute110CisFullScript');
    const bundleModal = document.getElementById('assessAwsCompute110CisBundleModal');
    const bundleCloseButton = document.getElementById('assessAwsCompute110CisBundleClose');
    const bundleCancelButton = document.getElementById('assessAwsCompute110CisBundleCancel');
    const bundleDetails = document.getElementById('assessAwsCompute110CisBundleDetails');
    const bundleRequestText = document.getElementById('assessAwsCompute110CisBundleRequestText');
    const bundleEmailLink = document.getElementById('assessAwsCompute110CisBundleEmail');
    const bundleCopyButton = document.getElementById('assessAwsCompute110CisBundleCopy');
    const exportPdfButton = document.getElementById('assessAwsCompute110CisExportPdf');
    const downloadCsvButton = document.getElementById('assessAwsCompute110CisDownloadCsv');
    const copyJsonButton = document.getElementById('assessAwsCompute110CisCopyJson');
    const downloadJsonButton = document.getElementById('assessAwsCompute110CisDownloadJson');
    const importJsonButton = document.getElementById('assessAwsCompute110CisImportJsonButton');
    const importJsonInput = document.getElementById('assessAwsCompute110CisImportJson');
    const tabButtons = Array.from(document.querySelectorAll('.assess-aws-compute-110-cis-tab-btn'));
    const tabPanels = Array.from(document.querySelectorAll('.assess-aws-compute-110-cis-tab-panel'));
    ...
    const sortCatalog = [
        ...
        },
        {
            value: 'criticality',
            title: 'Criticality',
            meta: 'Show higher declared criticality before lower levels'
        },
        {
            value: 'script',
            title: 'Script',
...
// ns:end family._base.workspace.03_custom-settings
    ...
    function initMarkdownCopyButtons() {
        const promptBlocks = Array.from(document.querySelectorAll('.markdown-content pre.assess-aws-compute-110-cis-prompt-pre'));
        const promptCopyButtons = document.querySelectorAll('.assess-aws-compute-110-cis-prompt-copy-btn');
        ...
        const commandBlocks = Array.from(document.querySelectorAll('.markdown-content pre.assess-aws-compute-110-cis-command-example-pre'));
        const commandCopyButtons = document.querySelectorAll('.assess-aws-compute-110-cis-command-copy-btn');
        ...
        const codeBlocks = document.querySelectorAll('.markdown-content pre:not(.assess-aws-compute-110-cis-prompt-pre):not(.assess-aws-compute-110-cis-command-example-pre)');
    ...
    function formatCriticalityLabel(value, fallbackLabel) {
        ...
        return fallbackLabel || labels[Number(value)] || `Level ${value}`;
    }
    ...
    function criticalityTone(value) {
        ...
        return tones[Number(value)] || 'unspecified';
    }
    ...
    function renderSortOptions(container, options, selectedValue) {
        container.innerHTML = options.map(function (option) {
            ...
            return `
                <button type="button" class="assess-aws-compute-110-cis-sort-option${isActive ? ' is-active' : ''}" data-sort-value="${escapeHtml(option.value)}" aria-pressed="${isActive ? 'true' : 'false'}"${metaAttribute}>
    ...
    function renderControlPickerButtons(options, selectedValue) {
        if (!options.length) {
            selectedControlOptions.innerHTML = '<div class="assess-aws-compute-110-cis-control-picker-empty">No matched scripts.</div>';
            return;
        }
        ...
        selectedControlOptions.innerHTML = options.map(function (option) {
            ...
            return '<button type="button" class="assess-aws-compute-110-cis-control-picker-option' + (isActive ? ' is-active' : '') + '" data-control-value="' + escapeHtml(option.value) + '" role="option" aria-selected="' + (isActive ? 'true' : 'false') + '"><strong>' + escapeHtml(option.title) + '</strong>' + meta + '</button>';
        }).join('');
    }
    ...
    function renderCriticalityOverview() {
        const counts = controls.reduce(function (accumulator, control) {
            const key = Number(control.criticality || 0);
            accumulator[key] = (accumulator[key] || 0) + 1;
            return accumulator;
        }, {});
        ...
        const overviewItems = [
            ...
            return item.value === 0 || counts[item.value];
        });

        criticalityRow.innerHTML = overviewItems.map(function (item) {
            return `<span class="assess-aws-compute-110-cis-criticality-pill" data-tone="${criticalityTone(item.value)}"><strong>${counts[item.value] || 0}</strong><span>${item.label}</span></span>`;
        }).join('');
    }
    ...
    function buildSummaryMarkup(result) {
        ...
        return `
            <header class="assess-aws-compute-110-cis-result-header" aria-label="Result summary header">
                <div class="assess-aws-compute-110-cis-result-header-main">
                    <span class="assess-aws-compute-110-cis-result-header-icon" aria-hidden="true"><i class="bi bi-bar-chart-line"></i></span>
                    <div class="assess-aws-compute-110-cis-result-header-copy">
                        <h2 class="assess-aws-compute-110-cis-result-header-title">Result Summary</h2>
                        <p>Overview of the current assessment results and key metrics</p>
                    </div>
                </div>
                <div class="assess-aws-compute-110-cis-result-header-meta" aria-label="Result summary status">
                    <span class="assess-aws-compute-110-cis-result-header-chip assess-aws-compute-110-cis-result-chip-ready"><span class="assess-aws-compute-110-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-circle-fill"></i></span><span>Generated</span></span>
                    <span class="assess-aws-compute-110-cis-result-header-chip assess-aws-compute-110-cis-result-chip-updated"><span class="assess-aws-compute-110-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-calendar3"></i></span><span>${escapeHtml(updatedText)}</span></span>
                </div>
            </header>

            <div class="assess-aws-compute-110-cis-result-hero-grid" aria-live="polite">
                <article class="assess-aws-compute-110-cis-result-card assess-aws-compute-110-cis-result-card-primary" data-result-visual="number" aria-label="Primary result">
                    <div class="assess-aws-compute-110-cis-result-primary-heading assess-aws-compute-110-cis-result-visual-copy assess-aws-compute-110-cis-result-visual-copy-top"><div class="assess-aws-compute-110-cis-result-kicker">Primary Result</div><h3 class="assess-aws-compute-110-cis-result-title assess-aws-compute-110-cis-result-title-center">${escapeHtml(visibleRows)} visible rows</h3></div>
                    <div class="assess-aws-compute-110-cis-result-primary-visual" id="assessAwsCompute110CisResultVisual" aria-label="Primary assessment result">
                        <span class="assess-aws-compute-110-cis-result-card-icon assess-aws-compute-110-cis-result-card-icon-primary" aria-hidden="true"><i class="bi bi-clipboard-check"></i></span>
                        <div class="assess-aws-compute-110-cis-result-primary-number">
                            <strong class="assess-aws-compute-110-cis-result-primary-number-value">${escapeHtml(visibleRows)}</strong>
                            <span class="assess-aws-compute-110-cis-result-primary-number-unit">visible rows</span>
                        </div>
                    </div>
                    <div class="assess-aws-compute-110-cis-result-visual-copy">
                        <p class="assess-aws-compute-110-cis-result-copy assess-aws-compute-110-cis-result-copy-center">Selected: ${escapeHtml(selectedMeta)}</p>
                    </div>
                    <span class="assess-aws-compute-110-cis-result-card-divider" aria-hidden="true"></span>
                    <div class="assess-aws-compute-110-cis-result-chip-row assess-aws-compute-110-cis-result-chip-row-center" aria-label="Primary result source">
                        <span class="assess-aws-compute-110-cis-result-chip assess-aws-compute-110-cis-result-chip-outcome assess-aws-compute-110-cis-result-chip-ready"><span class="assess-aws-compute-110-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-shield-check"></i></span><span>Benchmark: ${escapeHtml(benchmark.name || 'AWS Compute CIS')}</span></span>
                    </div>
                </article>

                <article class="assess-aws-compute-110-cis-result-card assess-aws-compute-110-cis-result-card-summary" aria-label="Assessment summary">
                    <div class="assess-aws-compute-110-cis-result-summary-intro">
                        <span class="assess-aws-compute-110-cis-result-card-icon assess-aws-compute-110-cis-result-card-icon-summary" aria-hidden="true"><i class="bi bi-clipboard-data"></i></span>
                        <div class="assess-aws-compute-110-cis-result-summary-copy">
                            <div class="assess-aws-compute-110-cis-result-kicker">Descriptive Summary</div>
                            <h3 class="assess-aws-compute-110-cis-result-title">Filtered CIS control set</h3>
                            <p class="assess-aws-compute-110-cis-result-copy">The table, section rollups, selected script body, JSON, and exports are all derived from the current filters.</p>
                        </div>
                    </div>
                    <span class="assess-aws-compute-110-cis-result-card-divider" aria-hidden="true"></span>
                    <div class="assess-aws-compute-110-cis-result-chip-grid" aria-label="Assessment state">
                        <span class="assess-aws-compute-110-cis-result-chip assess-aws-compute-110-cis-result-chip-ready"><span class="assess-aws-compute-110-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-shield-check"></i></span><span>Benchmark: ${escapeHtml(benchmark.name || 'AWS Compute CIS')}</span></span>
                        <span class="assess-aws-compute-110-cis-result-chip assess-aws-compute-110-cis-result-chip-baseline"><span class="assess-aws-compute-110-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-diagram-3"></i></span><span>Family: ${escapeHtml(familyLabel)}</span></span>
                        <span class="assess-aws-compute-110-cis-result-chip assess-aws-compute-110-cis-result-chip-baseline"><span class="assess-aws-compute-110-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-folder2-open"></i></span><span>Section: ${escapeHtml(sectionLabel)}</span></span>
                        <span class="assess-aws-compute-110-cis-result-chip assess-aws-compute-110-cis-result-chip-baseline"><span class="assess-aws-compute-110-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-flag"></i></span><span>Criticality: ${escapeHtml(criticalityLabel)}</span></span>
                        <span class="assess-aws-compute-110-cis-result-chip assess-aws-compute-110-cis-result-chip-warning"><span class="assess-aws-compute-110-cis-result-chip-icon" aria-hidden="true"><i class="bi bi-funnel"></i></span><span>${escapeHtml(queryLabel)}</span></span>
                    </div>
                </article>
            </div>

            <div class="assess-aws-compute-110-cis-result-metric-grid" aria-label="Assessment metrics">
                <article class="assess-aws-compute-110-cis-result-metric-card assess-aws-compute-110-cis-result-metric-success">
                    <span class="assess-aws-compute-110-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-file-earmark-text"></i></span>
                    <span class="assess-aws-compute-110-cis-result-metric-label">Matched Scripts</span>
                    <strong class="assess-aws-compute-110-cis-result-metric-value">${escapeHtml(matchedScripts)}</strong>
                    <span class="assess-aws-compute-110-cis-result-metric-copy">Controls matching the selected filters.</span>
                    <span class="assess-aws-compute-110-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-aws-compute-110-cis-result-metric-card assess-aws-compute-110-cis-result-metric-info">
                    <span class="assess-aws-compute-110-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-eye"></i></span>
                    <span class="assess-aws-compute-110-cis-result-metric-label">Visible Rows</span>
                    <strong class="assess-aws-compute-110-cis-result-metric-value">${escapeHtml(visibleRows)}</strong>
                    <span class="assess-aws-compute-110-cis-result-metric-copy">Rows rendered in the controls table.</span>
                    <span class="assess-aws-compute-110-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-aws-compute-110-cis-result-metric-card assess-aws-compute-110-cis-result-metric-accent-tone">
                    <span class="assess-aws-compute-110-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-layers"></i></span>
                    <span class="assess-aws-compute-110-cis-result-metric-label">Sections</span>
                    <strong class="assess-aws-compute-110-cis-result-metric-value">${escapeHtml(visibleSections)}</strong>
                    <span class="assess-aws-compute-110-cis-result-metric-copy">Section rollups from the filtered set.</span>
                    <span class="assess-aws-compute-110-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="assess-aws-compute-110-cis-result-metric-card assess-aws-compute-110-cis-result-metric-warning">
                    <span class="assess-aws-compute-110-cis-result-metric-icon" aria-hidden="true"><i class="bi bi-code"></i></span>
                    <span class="assess-aws-compute-110-cis-result-metric-label">Selected Script</span>
                    <strong class="assess-aws-compute-110-cis-result-metric-value">${escapeHtml(selectedScriptLines)}</strong>
                    <span class="assess-aws-compute-110-cis-result-metric-copy">${escapeHtml(selectedScriptCopy)}</span>
                    <span class="assess-aws-compute-110-cis-result-metric-accent" aria-hidden="true"></span>
                </article>
            </div>
        `;
    ...
    function renderToolbarMeta(result) {
        ...
        toolbarMeta.innerHTML = `
            <span class="assess-aws-compute-110-cis-toolbar-pill">Family scope: ${result.familyScopedControls.length}</span>
            <span class="assess-aws-compute-110-cis-toolbar-pill">Section scope: ${result.sectionScopedControls.length}</span>
            <span class="assess-aws-compute-110-cis-toolbar-pill">Criticality scope: ${result.criticalityScopedControls.length}</span>
            <span class="assess-aws-compute-110-cis-toolbar-pill">${escapeHtml(rowLimitLabel)}</span>
            <span class="assess-aws-compute-110-cis-toolbar-pill">Selected: ${escapeHtml(selectedLabel)}</span>
        `;
    }
    ...
    function renderControlsTable(result) {
        if (result.visibleControls.length === 0) {
            controlsTableBody.innerHTML = `
                <tr class="assess-aws-compute-110-cis-empty-row">
        ...
        controlsTableBody.innerHTML = result.visibleControls.map(function (control, index) {
            ...
            return `
                <tr data-control-path="${escapeHtml(control.script_path)}" class="${isSelected ? 'assess-aws-compute-110-cis-control-row-selected' : ''}">
                  <td class="tool-generated-rownum-cell">${index + 1}</td>
                  <td><span class="assess-aws-compute-110-cis-control-id">${escapeHtml(control.id)}</span></td>
                  <td><span class="assess-aws-compute-110-cis-title-text">${escapeHtml(control.title)}</span></td>
                  <td>
                    <div class="assess-aws-compute-110-cis-section-cell">
                      <span class="assess-aws-compute-110-cis-path-text" title="${escapeHtml(control.section_path)}">${escapeHtml(sectionDisplay)}</span>
                      <span class="assess-aws-compute-110-cis-section-family">${escapeHtml(control.family_label)}</span>
                    </div>
                  </td>
                  <td><span class="assess-aws-compute-110-cis-criticality-badge" data-tone="${escapeHtml(criticalityTone(control.criticality))}">${escapeHtml(criticalityLabel)}</span></td>
                  <td><span class="assess-aws-compute-110-cis-script-text" title="${escapeHtml(control.script_path)}">${escapeHtml(scriptDisplay)}</span></td>
                  <td class="assess-aws-compute-110-cis-table-action-cell tool-table-action-cell">
                    <button type="button" class="assess-aws-compute-110-cis-row-copy" data-control-copy-row="${index}" aria-label="Copy control row ${index + 1}" title="Copy control row">
        ...
        sectionsTableBody.innerHTML = result.visibleSections.map(function (section, index) {
            ...
            return `
                <tr>
                  ...
                  <td>
                    <div class="assess-aws-compute-110-cis-section-cell">
                      <span class="assess-aws-compute-110-cis-path-text">${escapeHtml(section.path)}</span>
                      <span class="assess-aws-compute-110-cis-section-family">${escapeHtml(section.full_label)}</span>
                    </div>
                  </td>
                  <td>${section.depth}</td>
                  <td>${section.visible_control_count}</td>
                  <td>${section.child_count}</td>
                  <td><span class="assess-aws-compute-110-cis-range-text">${escapeHtml(rangeText)}</span></td>
                </tr>
            `;
        }).join('');
    }
    ...
    function renderScriptState(control, statusText, scriptContent) {
        if (!control) {
            scriptMeta.innerHTML = `
                <div class="assess-aws-compute-110-cis-script-chip">
                  <span class="assess-aws-compute-110-cis-script-chip-label">Selection</span>
                  <span class="assess-aws-compute-110-cis-script-chip-value">No matched script</span>
                </div>
            `;
        ...
        const statusMarkup = statusText
            ? `
            <div class="assess-aws-compute-110-cis-script-chip">
              <span class="assess-aws-compute-110-cis-script-chip-label">Status</span>
              <span class="assess-aws-compute-110-cis-script-chip-value">${escapeHtml(statusText)}</span>
            </div>
        `
            : '';
        ...
        scriptMeta.innerHTML = `
            <div class="assess-aws-compute-110-cis-script-chip">
              <span class="assess-aws-compute-110-cis-script-chip-label">Control ID</span>
              <span class="assess-aws-compute-110-cis-script-chip-value">${escapeHtml(control.id)}</span>
            </div>
            <div class="assess-aws-compute-110-cis-script-chip">
              <span class="assess-aws-compute-110-cis-script-chip-label">Title</span>
              <span class="assess-aws-compute-110-cis-script-chip-value">${escapeHtml(control.title)}</span>
            </div>
            <div class="assess-aws-compute-110-cis-script-chip">
              <span class="assess-aws-compute-110-cis-script-chip-label">Criticality</span>
              <span class="assess-aws-compute-110-cis-script-chip-value">${escapeHtml(criticalityLabel)}</span>
            </div>
            <div class="assess-aws-compute-110-cis-script-chip">
              <span class="assess-aws-compute-110-cis-script-chip-label">Section path</span>
              <span class="assess-aws-compute-110-cis-script-chip-value">${escapeHtml(control.section_path)}</span>
            </div>
            <div class="assess-aws-compute-110-cis-script-chip">
              <span class="assess-aws-compute-110-cis-script-chip-label">Script path</span>
              <span class="assess-aws-compute-110-cis-script-chip-value">${escapeHtml(control.script_path)}</span>
            </div>
            <div class="assess-aws-compute-110-cis-script-chip">
              <span class="assess-aws-compute-110-cis-script-chip-label">Lines</span>
              <span class="assess-aws-compute-110-cis-script-chip-value">${control.script_line_count}</span>
            </div>
            ${statusMarkup}
        `;
    ...
    async function requestSelectedScript(scriptPath) {
        const response = await fetch(assessAwsCompute110CisScriptEndpoint, {
            method: 'POST',
            headers: {
    ...
    function buildBundleRequest(result) {
        ...
        const benchmarkTitle = benchmark.label || benchmark.title || benchmark.name || 'CIS benchmark';
        const benchmarkVersion = benchmark.version || benchmark.name || 'CIS benchmark version';
        const selectedLabel = result.selectedControl
        ...
        return {
            ...
            subject: `Full script request: ${benchmarkTitle}`
        };
    }
    ...
    function renderBundleModal(result) {
        ...
        bundleDetails.innerHTML = bundleRequest.detailRows.map(function (row) {
            return `
                <div class="assess-aws-compute-110-cis-bundle-detail">
    ...
    function getCurrentBundleResult() {
        if (latestResult && Array.isArray(latestResult.filteredControls) && latestResult.filteredControls.length > 0) {
            return latestResult;
        }

        const currentResult = buildResult(buildState());

        if (Array.isArray(currentResult.filteredControls) && currentResult.filteredControls.length > 0) {
    ...
    function openBundleModal() {
        const bundleResult = getCurrentBundleResult();
...
// ns:end family._base.workspace.08_json-restore
    ...
    function renderResult(result) {
        ...
        showResultState();
        resultSummary.classList.add('assess-aws-compute-110-cis-result-summary');
    ...
    selectedControlOptions.addEventListener('click', function (event) {
        ...
        renderExplorer();
        if (latestResult) {
            activateTab('assessAwsCompute110CisScriptPanel');
        }
    });
    ...
    resetButton.addEventListener('click', function () {
        ...
        showEmptyState();
        activateTab('assessAwsCompute110CisControlsPanel');
    ...
    controlsTableBody.addEventListener('click', function (event) {
        const target = event.target instanceof HTMLElement ? event.target : null;
        const copyButton = target ? target.closest('.assess-aws-compute-110-cis-row-copy') : null;
    ...
    document.addEventListener('click', function (event) {
        const fullScriptTrigger = event.target instanceof HTMLElement ? event.target.closest('#assessAwsCompute110CisFullScript') : null;

        if (!fullScriptTrigger) {
            return;
        }
...
// ns:start family._base.workspace.06_output-toolbar
    exportPdfButton.addEventListener('click', function () {
        ...
        exportResultShellAsPdf('assess-aws-compute-110-cis', resultContent);
    ...
    downloadCsvButton.addEventListener('click', function () {
        ...
        downloadFile('assess-aws-compute-110-cis.csv', latestResult.csv, 'text/csv;charset=utf-8');
    ...
    downloadJsonButton.addEventListener('click', function () {
        ...
        downloadFile('assess-aws-compute-110-cis.json', JSON.stringify(latestResult.jsonPayload, null, 2), 'application/json;charset=utf-8');
...
(function setupAssessAwsCompute110CisTableOutputStandard() {
    const rootSelector = '.assess-aws-compute-110-cis-tool';
    const tableSelector = '.tool-result-table tbody tr, .assess-aws-compute-110-cis-table tbody tr';
    const tbodySelector = '.tool-result-table tbody, .assess-aws-compute-110-cis-table tbody';
    const clampClass = 'assess-aws-compute-110-cis-table-cell-text';
    const cellClampClass = 'assess-aws-compute-110-cis-cell-clamp';
    const statusColumnClass = 'assess-aws-compute-110-cis-table-status-cell';
    ...
    function applyStatusAlignment(root) {
        root.querySelectorAll('.tool-result-table, .assess-aws-compute-110-cis-table').forEach(function alignStatusTable(table) {
