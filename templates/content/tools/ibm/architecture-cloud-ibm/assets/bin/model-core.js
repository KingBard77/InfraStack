const ArchitectureCloudIbmModelCore = (function () {
    const toolId = 'architecture-cloud-ibm';
    const toolVersion = '1.0.0';
    const legacyToolIds = ['ibm-cloud-architecture'];
    ...
    const presetCatalog = [
        {
            ...
            description: 'Cloud Internet Services, public ingress, private app/data subnets, security groups, flow logs, and monitored operations.',
            defaults: {
                cloudSize: 'medium',
                accessBlocks: 3,
                vlans: [
        ...
        },
        {
            ...
            description: 'Three-zone VPC for Red Hat OpenShift or IKS with private workers, load balancing, service endpoints, secrets, and observability.',
            defaults: {
                cloudSize: 'large',
                accessBlocks: 3,
                vlans: [
        ...
        },
        {
            id: 'data-services',
            label: 'Data Services',
            description: 'Private-first VPC with app and data subnets, Object Storage, PostgreSQL, Secrets Manager, Key Protect, logging, and private endpoints.',
            defaults: {
                cloudSize: 'medium',
                accessBlocks: 2,
                vlans: [
        ...
        },
        {
            ...
            description: 'Hybrid VPC with Transit Gateway, VPN or Direct Link, private tiers, public gateway egress, flow logs, and monitored controls.',
            defaults: {
                cloudSize: 'large',
                accessBlocks: 3,
                vlans: [
    ...
    function buildBaseSpec(values) {
        ...
        return {
            ...
            ospfArea: normalizeShortText(source.ospfArea || source.resourceGroup, defaults.ospfArea),
    ...
    /**
     * Builds a normalized IBM Cloud architecture spec from controls and prompt text.
     *
     * @param {Object} values Raw control values and prompt text.
     * @returns {Object} Normalized IBM Cloud architecture spec.
     */
    function inferFromPrompt(values) {
        ...
        if (/\blarge\b|enterprise|production|multi-zone|openshift|kubernetes|iks/.test(prompt)) {
        ...
        spec.wireless = resolveBooleanFromPrompt(
            prompt,
            spec.wireless,
            [/openshift/, /kubernetes/, /\biks\b/, /worker nodes?/, /red hat/],
            [/no openshift/, /without openshift/, /no kubernetes/, /without kubernetes/]
        );
        ...
        spec.wan = resolveBooleanFromPrompt(
            prompt,
            spec.wan,
            [/transit gateway/, /\btgw\b/, /hub and spoke/, /shared services hub/],
            [/no transit gateway/, /without transit gateway/]
        );
        spec.monitoring = resolveBooleanFromPrompt(
            prompt,
            spec.monitoring,
            [/monitoring/, /log analysis/, /\blogs\b/, /activity tracker/, /observability/],
            [/no monitoring/, /without monitoring/, /no logs/]
        );
        spec.dhcpDns = resolveBooleanFromPrompt(
            prompt,
            spec.dhcpDns,
            [/secrets manager/, /key protect/, /kms/, /secrets/, /key management/],
            [/no secrets/, /without secrets/, /no key protect/]
        );
        spec.hsrp = resolveBooleanFromPrompt(
            prompt,
            spec.hsrp,
            [/cloud internet services/, /\bcis\b/, /\bcdn\b/, /\bwaf\b/, /edge protected/, /dns/],
            [/no cis/, /without cloud internet services/, /no edge/]
        );
        ...
        spec.acl = resolveBooleanFromPrompt(
            prompt,
            spec.acl,
            [/flow logs?/, /vpc logs?/, /logging/, /activity tracker/],
            [/no flow logs?/, /without flow logs?/]
        );
        spec.nat = resolveBooleanFromPrompt(
            prompt,
            spec.nat,
            [/public gateway/, /\bnat\b/, /egress/],
            [/no public gateway/, /without public gateway/, /no nat/]
        );
        spec.vpn = resolveBooleanFromPrompt(
            prompt,
            spec.vpn,
            [/direct link/, /\bvpn\b/, /site-to-site/, /on-prem/, /on premises/, /hybrid/],
            [/no vpn/, /without vpn/, /no direct link/]
        );

        if (/private[- ]only|private endpoint|no public|service endpoint/.test(prompt)) {
            spec.routingMode = 'private-only';
        } else if (/hybrid|direct link|\bvpn\b|transit gateway|on-prem|on premises/.test(prompt)) {
            spec.routingMode = 'hybrid';
        } else if (/cloud internet services|\bcis\b|\bwaf\b|edge protected|cdn/.test(prompt)) {
    ...
    /**
     * Builds stage nodes and connectors for the normalized IBM Cloud spec.
     *
     * @param {Object} values Normalized or raw IBM Cloud architecture spec.
    ...
    function buildArchitecture(values, layoutOverrides) {
        ...
        const nodes = [
            createNode({
                ...
                y: 172,
                width: 152,
                purpose: 'Represents application users, partner systems, or operators reaching the IBM Cloud environment.'
            }, safeOverrides),
            createNode({
                id: 'vpc',
                title: 'IBM Cloud VPC',
                ...
                width: 210,
                height: 86,
                purpose: 'Defines the regional IBM Cloud network boundary, address space, route tables, and subnet grouping.'
            }, safeOverrides)
        ];
        const connectors = [];
        ...
        if (spec.hsrp) {
            ...
            connectors.push({
                id: 'users-to-cis',
                from: 'users',
                to: 'cloud-internet-services',
                label: 'DNS / edge'
            });
            connectors.push({
                id: 'cis-to-ingress',
                from: 'cloud-internet-services',
                to: ingressTarget,
        ...
        if (spec.firewall) {
            nodes.push(createNode({
                ...
                y: 172,
                width: 196,
                purpose: 'Represents IBM Cloud security groups, network ACLs, and subnet policy boundaries.'
            }, safeOverrides));
            connectors.push({
                id: 'security-to-vpc',
        ...
        nodes.push(createNode({
            id: 'workload-runtime',
            title: spec.wireless ? 'Red Hat OpenShift / IKS' : 'Virtual Server Instances',
            ...
            y: 735,
            width: 190,
            purpose: 'Runs the application workload in private IBM Cloud network space.'
        }, safeOverrides));
        connectors.push({
        ...
        nodes.push(createNode({
            ...
            y: 735,
            width: 190,
            purpose: 'Represents IBM Cloud Databases, Object Storage, and private service endpoint consumption.'
        }, safeOverrides));
        connectors.push({
        ...
        if (spec.dhcpDns) {
            nodes.push(createNode({
                ...
                title: 'Secrets Manager / Key Protect',
        ...
        if (spec.vpn) {
            nodes.push(createNode({
                id: 'direct-link-vpn',
                title: 'Direct Link / VPN',
                subtitle: 'ASN ' + spec.bgpAsn,
                layer: 'On-premises edge',
                icon: 'transitGateway',
                x: 995,
                y: 172,
                purpose: 'Shows private hybrid connectivity from IBM Cloud to external networks.'
            }, safeOverrides));
            connectors.push({
                id: 'direct-link-to-transit-gateway',
                from: 'direct-link-vpn',
                to: spec.wan ? 'transit-gateway' : 'vpc',
                label: 'Hybrid path'
            });
        }
        ...
        return {
            ...
            groups: [
                ...
                },
                {
                    ...
                    subtitle: spec.hsrp ? 'Cloud Internet Services path' : 'Direct application path',
                    tone: 'edge',
                    x: 275,
                    y: 108,
                    width: 680,
                ...
                },
                {
                    ...
                    title: 'Ops / hybrid',
                    subtitle: spec.vpn || spec.wan ? 'Transit, Direct Link, VPN' : 'Keys and monitoring',
                    tone: 'services',
                    x: 970,
                    y: 108,
                    width: 240,
    ...
    /**
     * Builds technical inventory rows from an IBM Cloud architecture stage model.
     *
    ...
    /**
     * Builds prompt interpretation notes from a normalized IBM Cloud spec.
     *
     * @param {Object} values Normalized or raw IBM Cloud architecture spec.
    ...
    function buildPromptNotes(values) {
        ...
        const prompt = normalizeText(spec.prompt);
        const keywords = [
            /vpc/.test(prompt) ? 'IBM Cloud VPC' : '',
            /cloud internet services|\bcis\b|cdn|waf/.test(prompt) ? 'Cloud Internet Services' : '',
            /load balancer|alb|nlb|ingress/.test(prompt) ? 'Load Balancer' : '',
            /subnet|cidr/.test(prompt) ? 'Subnet tiers' : '',
            /openshift|kubernetes|iks/.test(prompt) ? 'OpenShift / IKS' : '',
            /virtual server|vsi|instances?/.test(prompt) ? 'Virtual Server Instances' : '',
            /object storage|postgres|database|data services/.test(prompt) ? 'Data services' : '',
            /transit gateway|direct link|\bvpn\b|on-prem/.test(prompt) ? 'Hybrid connectivity' : '',
            /secrets manager|key protect|kms/.test(prompt) ? 'Secrets and keys' : '',
            /monitoring|logs|flow logs|activity tracker/.test(prompt) ? 'Monitoring and logs' : ''
        ].filter(Boolean);
        const assumptions = [
            ...
            'Resource group ' + spec.ospfArea + ' is used as the workspace grouping label.'
        ];
        const model = [
            ...
            spec.hsrp ? 'Cloud Internet Services included' : 'Cloud Internet Services omitted',
            ...
            spec.nat ? 'Public Gateway egress included' : 'Public Gateway egress omitted',
            spec.vpn ? 'Direct Link or VPN included' : 'Direct Link or VPN omitted',
            spec.wan ? 'Transit Gateway included' : 'Transit Gateway omitted',
            spec.dhcpDns ? 'Secrets Manager or Key Protect included' : 'Secrets and key service omitted',
        ...
        if (spec.routingMode === 'hybrid' && !spec.vpn && !spec.wan) {
            cons.push('Hybrid pattern selected without Direct Link, VPN, or Transit Gateway in the model.');
        }

        return {
            summary: spec.presetLabel + ' using ' + routingLabel(spec.routingMode) + ' across ' + zoneCountLabel(spec.accessBlocks) + '.',
            keywords: keywords.length > 0 ? keywords : ['IBM Cloud architecture baseline'],
            assumptions: assumptions,
            model: model,
            pros: pros.length > 0 ? pros : ['Architecture has clear VPC, subnet, workload, data, and operations placement.'],
            cons: cons.length > 0 ? cons : ['Detailed IBM Cloud account, IAM, quota, and service-plan choices are outside this draft.']
        };
    }

    /**
     * Builds a heuristic readiness score for the current IBM Cloud model.
     *
     * @param {Object} values Normalized or raw IBM Cloud architecture spec.
    ...
    function diagramFromSpec(spec) {
        return {
            ...
            transit_gateway: spec.wan,
            monitoring_logs: spec.monitoring,
            secrets_manager_key_protect: spec.dhcpDns,
            connectivity_pattern: spec.routingMode,
            cloud_internet_services: spec.hsrp,
            load_balancer: spec.etherChannel,
            flow_logs: spec.acl,
            public_gateway: spec.nat,
            vpn_or_direct_link: spec.vpn,
    ...
    /**
     * Builds a restorable IBM Cloud JSON export payload.
     *
     * @param {Object} values Normalized or raw IBM Cloud architecture spec.
    ...
    function specFromDiagram(diagram) {
        return {
            ...
            wan: readDiagramValue(diagram, 'transit_gateway', 'wan'),
            monitoring: readDiagramValue(diagram, 'monitoring_logs', 'monitoring'),
            dhcpDns: readDiagramValue(diagram, 'secrets_manager_key_protect', 'dhcp_dns'),
            routingMode: readDiagramValue(diagram, 'connectivity_pattern', 'routing_mode'),
            hsrp: readDiagramValue(diagram, 'cloud_internet_services', 'hsrp'),
            etherChannel: readDiagramValue(diagram, 'load_balancer', 'ether_channel'),
            acl: readDiagramValue(diagram, 'flow_logs', 'acl'),
            nat: readDiagramValue(diagram, 'public_gateway', 'nat'),
            vpn: readDiagramValue(diagram, 'vpn_or_direct_link', 'vpn'),
            ...
            ospfArea: readDiagramValue(diagram, 'resource_group', 'ospf_area'),
    ...
    function validateImportedSpec(rawSpec) {
        ...
        if (!booleanKeys.every(function (key) {
            ...
            return {
                error: 'The imported JSON contains invalid IBM Cloud service toggle values.'
            };
        }
    ...
    /**
     * Validates and normalizes an imported IBM Cloud workspace payload.
     *
    ...
    function buildImportedPayloadState(payload) {
        if (!payload || typeof payload !== 'object' || (payload.tool !== toolId && !legacyToolIds.includes(payload.tool))) {
            return {
                error: 'Invalid IBM Cloud architecture JSON.'
            };
        }
...
if (typeof globalThis !== 'undefined') {
    globalThis.ArchitectureCloudIbmModelCore = ArchitectureCloudIbmModelCore;
}

if (typeof module !== 'undefined' && module.exports) {
    module.exports = ArchitectureCloudIbmModelCore;
}
