...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const previewNumeric = document.getElementById('generateChmodShellPreviewNumeric');
    const previewSymbolic = document.getElementById('generateChmodShellPreviewSymbolic');
    const previewLs = document.getElementById('generateChmodShellPreviewLs');
    ...
    function flashButton(button, text) {
        ...
        if (!label && button.classList.contains('generate-chmod-shell-row-copy')) {
    ...
    function enhanceNativeSelect(select) {
        if (!select || select.dataset.generateChmodShellEnhanced === '1') {
            return;
        }
        ...
        select.dataset.generateChmodShellEnhanced = '1';
        select.classList.add('generate-chmod-shell-native-select');
        wrapper.className = 'generate-chmod-shell-enhanced-select';
        toggle.type = 'button';
        toggle.className = 'generate-chmod-shell-enhanced-select-toggle';
        toggle.setAttribute('aria-haspopup', 'listbox');
        toggle.setAttribute('aria-expanded', 'false');
        menu.className = 'generate-chmod-shell-enhanced-select-menu';
        ...
        Array.from(select.options).forEach((option) => {
            ...
            optionButton.className = 'generate-chmod-shell-enhanced-select-option';
    ...
    function syncPreviewValues() {
        previewNumeric.textContent = buildNumericMode(permissionState);
        previewSymbolic.textContent = buildSymbolicMode(permissionState);
        previewLs.textContent = buildLsPreview(permissionState);
    }
...
// ns:end family._base.workspace.02_basic-settings
    ...
    Array.from(document.querySelectorAll('.generate-chmod-shell-form select')).forEach((select) => {
