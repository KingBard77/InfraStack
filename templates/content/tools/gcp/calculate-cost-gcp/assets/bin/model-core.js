(function (root, factory) {
    ...
    root.CalculateCostGcpModelCore = core;
}(typeof self !== 'undefined' ? self : this, function () {
    'use strict';

    const TOOL_ID = 'calculate-cost-gcp';
    const VERSION = '1.0.0';
    const HOURS_PER_MONTH = 730;

    const computeProfiles = {
        'b2s': {
            label: 'e2-medium - 2 vCPU - 4 GiB',
            hourly: 0.0416,
            vcpu: 2,
            memoryGiB: 4
        },
        'd2s-v5': {
            label: 'e2-standard-2 - 2 vCPU - 8 GiB',
            hourly: 0.096,
            vcpu: 2,
            memoryGiB: 8
        },
        'd4s-v5': {
            label: 'e2-standard-4 - 4 vCPU - 16 GiB',
            hourly: 0.192,
            vcpu: 4,
            memoryGiB: 16
        },
        'd8s-v5': {
            label: 'n2-standard-8 - 8 vCPU - 32 GiB',
            hourly: 0.384,
            vcpu: 8,
            memoryGiB: 32
        },
        'f4s-v2': {
            label: 'c3-standard-4 - 4 vCPU - 16 GiB',
            hourly: 0.169,
            vcpu: 4,
            memoryGiB: 8
        }
    ...
    const databaseTiers = {
        'sql-basic': {
            label: 'Google Cloud SQL Basic',
            monthly: 5
        },
        'sql-standard': {
            label: 'Cloud SQL standard starter',
            monthly: 15
        },
        'sql-business': {
            label: 'Cloud SQL enterprise starter',
            monthly: 280
        },
        'postgres-flex': {
            label: 'Cloud SQL for PostgreSQL starter',
            monthly: 160
        }
    };
    ...
    const presets = {
        'balanced-web': {
            label: 'Balanced web app',
            state: {
                label: 'Google Cloud balanced web app',
                preset: 'balanced-web',
                region: 'us-central1',
        ...
        'serverless-api': {
            ...
            state: {
                label: 'Google Cloud serverless API',
        ...
        'data-platform': {
            label: 'Data platform',
            state: {
                label: 'Google Cloud data platform',
        ...
        'blank': {
            ...
            state: {
                label: 'Google Cloud blank estimate',
                preset: 'blank',
                region: 'us-central1',
                hoursPerMonth: HOURS_PER_MONTH,
                services: {
    ...
    /**
     * Normalizes raw calculator input into the Google Cloud cost model.
     *
    ...
    function normalizeState(input) {
        ...
        return {
            ...
            region: chooseString(source.region, defaultState.region, ['us-central1', 'us-central1', 'us-west1', 'centralus', 'westeurope', 'southeastasia']),
    ...
    /**
     * Builds a full Google Cloud estimate from normalized or raw state.
     *
    ...
    function buildEstimate(input) {
        ...
        if (state.services.compute.include && state.services.compute.count > 0) {
            ...
            addLine(
                lines,
                'Compute',
                'Google Cloud Compute Engine - ' + profile.label,
                state.services.compute.count * state.services.compute.hours,
                'instance hours',
                hourly,
                monthly,
                state.services.compute.count + ' VM(s), ' + profile.vcpu + ' vCPU each'
            );
        }
        ...
        if (state.services.storage.include && state.services.storage.storageGb > 0) {
            ...
            addLine(lines, 'Storage', 'Cloud Storage capacity', state.services.storage.storageGb, 'GB-month', storageRate, storageMonthly, 'Cloud Storage starter assumption');
            addLine(lines, 'Storage', 'Cloud Storage operations', state.services.storage.transactionsMillion, 'million operations', opsRate, opsMonthly, 'Request-volume planning input');
        }
        ...
        if (state.services.serverless.include) {
            ...
            addLine(lines, 'Serverless', 'Cloud Functions requests', state.services.serverless.requestsMillion, 'million requests', requestRate, requestMonthly, 'Request-based starter estimate');
            addLine(lines, 'Serverless', 'Cloud Functions execution', state.services.serverless.gbSeconds, 'GB-seconds', gbSecondRate, executionMonthly, 'Runtime and memory estimate');
        }
        ...
        if (state.services.network.include) {
            ...
            if (state.services.network.edgeMonthly > 0) {
                addLine(lines, 'Network', 'Cloud Load Balancing or Cloud CDN allowance', 1, 'monthly allowance', state.services.network.edgeMonthly, state.services.network.edgeMonthly, 'Visible edge-service placeholder');
            }
        }
        ...
        if (discountAmount > 0) {
            addLine(overheadLines, 'Adjustment', 'Commitment discount', 1, 'discount', -discountAmount, -discountAmount, 'Applied to compute and database subtotal');
        }
    ...
    function buildAssumptions(state) {
        return [
            ...
            ['Support uplift', state.assumptions.supportPct + '%', 'Planning allowance, not a selected Google Cloud support plan.'],
    ...
    function buildRecommendations(state, lines, total) {
        ...
        if (!driver) {
            recommendations.push('Add at least one included Google Cloud service before using the estimate in review.');
        } else {
        ...
        if (state.assumptions.commitmentDiscountPct === 0 && state.services.compute.include) {
            recommendations.push('Compare pay-as-you-go with Google Cloud committed uses or committed use discounts when Compute runtime is steady.');
        }

        if (state.services.network.include && state.services.network.egressGb > 250) {
            recommendations.push('Review outbound data transfer separately; egress is where tidy estimates learn sarcasm.');
        }
        ...
        recommendations.push('Validate material values in Google Cloud Pricing Calculator, Cloud Billing exports, or agreement pricing before committing.');

        return recommendations;
    }

    /**
     * Builds an exportable JSON payload for the current Google Cloud estimate.
     *
     * @param {object} input Raw or normalized estimate state.
    ...
    /**
     * Builds restored state from an imported Google Cloud calculator payload.
     *
    ...
    function buildImportedPayloadState(payload) {
        if (!payload || payload.tool !== TOOL_ID || typeof payload.state !== 'object') {
            return {
                error: 'Invalid GCP cost calculator JSON.'
            };
        }
