# InfraStack Phase 2 Content Contract

This file governs shared metadata, card copy, support markdown, typography, examples, trust boundaries, and domain voice.

## Directory Roles

- `templates/content/factory/phase-2/_content/scaffold/`: reusable package skeleton source, including default asset placeholders under `scaffold/assets/`.
- `templates/content/factory/phase-2/_content/sections/`: reusable support-content section structure.

## Phase 2 Source Rules

- Keep phase-2 content rules generic and family-neutral.
- Do not place provider, product, or family-specific final copy here.
- Do not turn `_content/` into a runtime package.
- Do not duplicate family workspace sections under `_content/`.
...
Do not recreate the removed root-level shared `tool-post-visual.html.twig` fallback. The shared post visual fallback lives at `templates/content/factory/phase-2/_content/scaffold/assets/img/post.html.twig`.
...
Update `templates/content/factory/phase-2/_content/sections/manifest.yml` when section inventory or structured metadata changes.
...
7. `templates/content/factory/phase-1/_family/FAMILY.md` when reusable family baselines are involved
8. `codex/devops/AGENTS.md` before task record work
9. the matching family source under `templates/content/factory/phase-1/_family/<family>/` when the tool belongs to a known family
...
Audit note: complete runtime tools keep their post/listing visual in `assets/img/post.html.twig` and use `card_image: null` for template-driven imagery. Do not document or generate a root-level `post.html.twig`, and do not recreate the removed shared `tool-post-visual.html.twig` fallback.
...
Use these phase-2 sections for repeated support markdown blocks, article sections, example structures, FAQ blocks, acronym or glossary sections, and markdown card patterns before writing a new tool page. Use `templates/content/factory/phase-1/_family/<family>/workspace/` for family workspace grammar and `templates/content/factory/phase-1/_family/<family>/` for family-specific content behavior and adaptation rules. The scaffold and family pattern can guide structure and consistency, but the final `content.md`, `meta.yml`, and `card.yml` must still live in `templates/content/tools/<category>/<tool-slug>/`.
...
Before writing content for a new or substantially revised tool, name the dominant tool family, read `templates/content/factory/phase-1/_family/<family>/workspace/` when it exists, read `templates/content/factory/phase-2/_content/sections/`, and check whether a matching family source exists under `templates/content/factory/phase-1/_family/`. Use phase-2 content sections for shared rhythm and family content rules for adaptation. If no family source applies, keep the content tool-local but still align it to the selected family.
...
Architecture-family content should align with `templates/content/factory/phase-1/_family/architecture/FAMILY.md` and the matching manifests. Do not duplicate detailed architecture baseline rules in this file.
...
When `card_image` is `null`, the visual should come from tool-local `assets/img/post.html.twig` or, during scaffold fallback, `templates/content/factory/phase-2/_content/scaffold/assets/img/post.html.twig`. The card file should not point to a missing asset, and the tool package should not create a root-level `post.html.twig`.
...
Template parity is part of the typography standard. When a phase-2 content section is adapted into a final package, validate the rendered contract in the final files: card frame, section title icon, heading divider line, Technical Details subsection separators, accordion row icons, section spacing, paragraph rhythm, list inheritance, inline code size, table layout, details/FAQ behavior, copy-button controls, citation links, terminal strip title treatment, and responsive selectors used by the section.

Automated audits are support tools, not acceptance evidence by themselves. Do not mark a phase-2 section as applied because `standard-package.sh`, `tool-package.sh`, a selector scan, or a namespace audit passed. The final runtime page must be compared against the selected section reference and checked in the browser for the user-visible details the audit cannot prove.

For prompt and command sections, final rendered verification must confirm the intro paragraph, terminal chrome, centered title-case strip title, prompt or command text, `Show prompt use` or `Show command use` control, matching `Hide` state, note box, copy button, copy feedback, and copy-click behavior. The copy button must not toggle the parent details row. Prompt-specific or command-specific labels must not appear as loose text outside the terminal chrome.

For icon-bearing support sections, final rendered verification must confirm that each icon chip contains a visible glyph. A syntactically valid icon class is not enough when the glyph is missing from the loaded icon font.
...
- `templates/content/factory/phase-2/_content/sections/*/section.css`
- `templates/content/factory/phase-1/_family/<family>/**/section.css`
- `templates/content/factory/phase-1/_family/<family>/baseline/source/custom.css`
...
- phase-2 content sources updated or `not needed`
