...
// ns:start family._base.workspace.01_input-brief
    const form = document.getElementById('__DOM_ID_PREFIX__Form');
    const labelInput = document.getElementById('__DOM_ID_PREFIX__Label');
...
// ns:start family._base.workspace.02_basic-settings
    ...
    const applyPresetButton = document.getElementById('__DOM_ID_PREFIX__ApplyPreset');
// ns:end family._base.workspace.02_basic-settings
    const submitButton = document.getElementById('__DOM_ID_PREFIX__Submit');
    const resetButton = document.getElementById('__DOM_ID_PREFIX__Reset');

// ns:start family._base.workspace.03_custom-settings
    const includeEc2Input = document.getElementById('__DOM_ID_PREFIX__IncludeEc2');
    const includeEbsInput = document.getElementById('__DOM_ID_PREFIX__IncludeEbs');
    const includeS3Input = document.getElementById('__DOM_ID_PREFIX__IncludeS3');
    const includeLambdaInput = document.getElementById('__DOM_ID_PREFIX__IncludeLambda');
    const includeApiInput = document.getElementById('__DOM_ID_PREFIX__IncludeApi');

    const ec2InstanceInput = document.getElementById('__DOM_ID_PREFIX__Ec2Instance');
    const ec2CustomOptionLabel = document.getElementById('__DOM_ID_PREFIX__Ec2CustomOptionLabel');
    const ec2PurchaseInput = document.getElementById('__DOM_ID_PREFIX__Ec2Purchase');
    const ec2CustomFields = document.getElementById('__DOM_ID_PREFIX__Ec2CustomFields');
    const ec2CustomHint = document.getElementById('__DOM_ID_PREFIX__Ec2CustomHint');
    const ec2CustomLabelInput = document.getElementById('__DOM_ID_PREFIX__Ec2CustomLabel');
    const ec2CustomVcpuInput = document.getElementById('__DOM_ID_PREFIX__Ec2CustomVcpu');
    const ec2CustomMemoryGiBInput = document.getElementById('__DOM_ID_PREFIX__Ec2CustomMemoryGiB');
    const ec2CountInput = document.getElementById('__DOM_ID_PREFIX__Ec2Count');
    const ec2HoursInput = document.getElementById('__DOM_ID_PREFIX__Ec2Hours');

    const ebsStorageGbInput = document.getElementById('__DOM_ID_PREFIX__EbsStorageGb');
    const ebsIopsInput = document.getElementById('__DOM_ID_PREFIX__EbsIops');
    const ebsThroughputInput = document.getElementById('__DOM_ID_PREFIX__EbsThroughput');

    const s3StorageGbInput = document.getElementById('__DOM_ID_PREFIX__S3StorageGb');
    const s3EgressGbInput = document.getElementById('__DOM_ID_PREFIX__S3EgressGb');
    const s3GetRequestsKInput = document.getElementById('__DOM_ID_PREFIX__S3GetRequestsK');
    const s3PutRequestsKInput = document.getElementById('__DOM_ID_PREFIX__S3PutRequestsK');

    const lambdaRequestsMillionInput = document.getElementById('__DOM_ID_PREFIX__LambdaRequestsMillion');
    const lambdaDurationMsInput = document.getElementById('__DOM_ID_PREFIX__LambdaDurationMs');
    const lambdaMemoryMbInput = document.getElementById('__DOM_ID_PREFIX__LambdaMemoryMb');

    const apiTypeInput = document.getElementById('__DOM_ID_PREFIX__ApiType');
    const apiRequestsMillionInput = document.getElementById('__DOM_ID_PREFIX__ApiRequestsMillion');
    const apiResponseKbInput = document.getElementById('__DOM_ID_PREFIX__ApiResponseKb');

    const sharedEgressGbInput = document.getElementById('__DOM_ID_PREFIX__SharedEgressGb');
    const supportPctInput = document.getElementById('__DOM_ID_PREFIX__SupportPct');
    const contingencyPctInput = document.getElementById('__DOM_ID_PREFIX__ContingencyPct');
    const manualAdjustmentInput = document.getElementById('__DOM_ID_PREFIX__ManualAdjustment');

    const freeTierLambdaInput = document.getElementById('__DOM_ID_PREFIX__FreeTierLambda');
    const freeTierApiInput = document.getElementById('__DOM_ID_PREFIX__FreeTierApi');
    const freeTierEgressInput = document.getElementById('__DOM_ID_PREFIX__FreeTierEgress');
    const regionalUpliftPctInput = document.getElementById('__DOM_ID_PREFIX__RegionalUpliftPct');

    const ec2HourlyOverrideInput = document.getElementById('__DOM_ID_PREFIX__Ec2HourlyOverride');
    const ebsStorageRateInput = document.getElementById('__DOM_ID_PREFIX__EbsStorageRate');
    const ebsIopsRateInput = document.getElementById('__DOM_ID_PREFIX__EbsIopsRate');
    const ebsThroughputRateInput = document.getElementById('__DOM_ID_PREFIX__EbsThroughputRate');
    const s3StorageRateInput = document.getElementById('__DOM_ID_PREFIX__S3StorageRate');
    const s3GetRateInput = document.getElementById('__DOM_ID_PREFIX__S3GetRate');
    const s3PutRateInput = document.getElementById('__DOM_ID_PREFIX__S3PutRate');
    const lambdaRequestRateInput = document.getElementById('__DOM_ID_PREFIX__LambdaRequestRate');
    const lambdaGbSecondRateInput = document.getElementById('__DOM_ID_PREFIX__LambdaGbSecondRate');
    const apiHttpFirstRateInput = document.getElementById('__DOM_ID_PREFIX__ApiHttpFirstRate');
    const apiHttpNextRateInput = document.getElementById('__DOM_ID_PREFIX__ApiHttpNextRate');
    const apiRestRateInput = document.getElementById('__DOM_ID_PREFIX__ApiRestRate');
    const egressRateInput = document.getElementById('__DOM_ID_PREFIX__EgressRate');

// ns:end family._base.workspace.03_custom-settings
    const resultEmpty = document.getElementById('__DOM_ID_PREFIX__ResultEmpty');
    const resultContent = document.getElementById('__DOM_ID_PREFIX__ResultContent');
    const resultError = document.getElementById('__DOM_ID_PREFIX__ResultError');
    const resultSummary = document.getElementById('__DOM_ID_PREFIX__ResultSummary');
    const breakdownTableBody = document.getElementById('__DOM_ID_PREFIX__BreakdownTableBody');
    const serviceTableBody = document.getElementById('__DOM_ID_PREFIX__ServiceTableBody');
    const assumptionTableBody = document.getElementById('__DOM_ID_PREFIX__AssumptionTableBody');
    const recommendationsWrap = document.getElementById('__DOM_ID_PREFIX__Recommendations');
    const methodologyWrap = document.getElementById('__DOM_ID_PREFIX__Methodology');
    const jsonOutput = document.getElementById('__DOM_ID_PREFIX__JsonOutput');

    const sortInput = document.getElementById('__DOM_ID_PREFIX__Sort');
    const sortSummary = document.getElementById('__DOM_ID_PREFIX__SortSummary');
    const sortOptionButtons = Array.from(document.querySelectorAll('.__PREFIX__-sort-option'));
    const sortSelect = document.getElementById('__DOM_ID_PREFIX__SortSelect');
    const exportPdfButton = document.getElementById('__DOM_ID_PREFIX__ExportPdf');
    const downloadCsvButton = document.getElementById('__DOM_ID_PREFIX__DownloadCsv');
    ...
    const NUMERIC_INPUT_DECORATORS = {
        __DOM_ID_PREFIX__Ec2CustomVcpu: {
            suffix: 'vCPU'
        },
        __DOM_ID_PREFIX__Ec2CustomMemoryGiB: {
            suffix: 'GiB'
        },
        __DOM_ID_PREFIX__Ec2Count: {
            suffix: 'instances'
        },
        __DOM_ID_PREFIX__Ec2Hours: {
            suffix: 'hours'
        },
        __DOM_ID_PREFIX__EbsStorageGb: {
            suffix: 'GB'
        },
        __DOM_ID_PREFIX__EbsIops: {
            suffix: 'IOPS'
        },
        __DOM_ID_PREFIX__EbsThroughput: {
            suffix: 'MB/s'
        },
        __DOM_ID_PREFIX__S3StorageGb: {
            suffix: 'GB'
        },
        __DOM_ID_PREFIX__S3EgressGb: {
            suffix: 'GB'
        },
        __DOM_ID_PREFIX__S3GetRequestsK: {
            suffix: '1k'
        },
        __DOM_ID_PREFIX__S3PutRequestsK: {
            suffix: '1k'
        },
        __DOM_ID_PREFIX__LambdaRequestsMillion: {
            suffix: '1M'
        },
        __DOM_ID_PREFIX__LambdaDurationMs: {
            suffix: 'ms'
        },
        __DOM_ID_PREFIX__LambdaMemoryMb: {
            suffix: 'MB'
        },
        __DOM_ID_PREFIX__ApiRequestsMillion: {
            suffix: '1M'
        },
        __DOM_ID_PREFIX__ApiResponseKb: {
            suffix: 'KB'
        },
        __DOM_ID_PREFIX__SharedEgressGb: {
            suffix: 'GB'
        },
        __DOM_ID_PREFIX__SupportPct: {
            suffix: '%'
        },
        __DOM_ID_PREFIX__ContingencyPct: {
            suffix: '%'
        },
        __DOM_ID_PREFIX__ManualAdjustment: {
            prefix: '$'
        },
        __DOM_ID_PREFIX__RegionalUpliftPct: {
            suffix: '%'
        },
        __DOM_ID_PREFIX__Ec2HourlyOverride: {
            prefix: '$',
            suffix: 'instance-hour'
        },
        __DOM_ID_PREFIX__EbsStorageRate: {
            prefix: '$',
            suffix: 'GB-month'
        },
        __DOM_ID_PREFIX__EbsIopsRate: {
            prefix: '$',
            suffix: 'IOPS-month'
        },
        __DOM_ID_PREFIX__EbsThroughputRate: {
            prefix: '$',
            suffix: 'MB/s-month'
        },
        __DOM_ID_PREFIX__S3StorageRate: {
            prefix: '$',
            suffix: 'GB-month'
        },
        __DOM_ID_PREFIX__S3GetRate: {
            prefix: '$',
            suffix: '1k GET'
        },
        __DOM_ID_PREFIX__S3PutRate: {
            prefix: '$',
            suffix: '1k PUT'
        },
        __DOM_ID_PREFIX__LambdaRequestRate: {
            prefix: '$',
            suffix: '1M req'
        },
        __DOM_ID_PREFIX__LambdaGbSecondRate: {
            prefix: '$',
            suffix: 'GB-s'
        },
        __DOM_ID_PREFIX__ApiHttpFirstRate: {
            prefix: '$',
            suffix: '1M'
        },
        __DOM_ID_PREFIX__ApiHttpNextRate: {
            prefix: '$',
            suffix: '1M'
        },
        __DOM_ID_PREFIX__ApiRestRate: {
            prefix: '$',
            suffix: '1M'
        },
        __DOM_ID_PREFIX__EgressRate: {
            prefix: '$',
            suffix: 'GB'
        }
    };
    ...
    function buildInputAddon(text, position) {
        ...
        addon.className = `__PREFIX__-input-addon __PREFIX__-input-addon--${position}`;
    ...
    function decorateNumericInput(input, decorator) {
        if (!input || input.dataset.__DOM_ID_PREFIX__Decorated === '1') {
            return;
        }
        ...
        wrapper.className = '__PREFIX__-input-group';
        input.classList.add('__PREFIX__-input-group-control');
        input.dataset.__DOM_ID_PREFIX__Decorated = '1';
    ...
    function initMarkdownCopyButtons() {
        ...
        promptCopyButtons.forEach((button) => {
            ...
            if (button.dataset.__DOM_ID_PREFIX__CopyBound === '1') {
                return;
            }

            button.dataset.__DOM_ID_PREFIX__CopyBound = '1';
        ...
        codeBlocks.forEach((pre) => {
            ...
            if (button && button.dataset.__DOM_ID_PREFIX__CopyBound === '1') {
                return;
            }
    ...
    function syncServiceState() {
        Object.values(serviceRefs).forEach((serviceRef) => {
            ...
            Array.from(serviceRef.body.querySelectorAll('.__PREFIX__-select')).forEach((selectElement) => {
    ...
    function groupServiceRows(lineItems, monthlyTotal) {
        ...
        return Array.from(totalsByService.values())
            ...
            .map((row) => {
                ...
                let signal = 'Light';
                let signalClass = '__PREFIX__-signal-light';

                if (sharePct >= 40) {
                    signal = 'Heavy';
                    signalClass = '__PREFIX__-signal-heavy';
                } else if (sharePct >= 15) {
                    signal = 'Medium';
                    signalClass = '__PREFIX__-signal-medium';
                }
    ...
    function renderSpendRingChart(result) {
        const chartElement = document.getElementById('__DOM_ID_PREFIX__SpendChart');
        const ringElement = document.getElementById('__DOM_ID_PREFIX__SpendRing');
    ...
    function buildSummaryHtml(result) {
        ...
        return `
            <header class="__PREFIX__-result-header" aria-label="Result summary header">
                <div class="__PREFIX__-result-header-main">
                    <span class="__PREFIX__-result-header-icon" aria-hidden="true"><i class="bi bi-calculator"></i></span>
                    <div class="__PREFIX__-result-header-copy">
                        <h2 class="__PREFIX__-result-header-title">Result Summary</h2>
                        <p>Overview of the current AWS estimate result and key metrics</p>
                    </div>
                </div>
                <div class="__PREFIX__-result-header-meta" aria-label="Result summary status">
                    <span class="__PREFIX__-result-header-chip __PREFIX__-result-chip-ready"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-circle-fill"></i></span><span>Estimated</span></span>
                    <span class="__PREFIX__-result-header-chip __PREFIX__-result-chip-updated"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-calendar3"></i></span><span>${escapeHtml(generatedAtText)}</span></span>
                </div>
            </header>

            <div class="__PREFIX__-result-hero-grid" aria-live="polite">
                <article class="__PREFIX__-result-card __PREFIX__-result-card-primary" data-result-visual="ring" aria-label="Primary estimate result">
                    <div class="__PREFIX__-result-primary-heading __PREFIX__-result-visual-copy __PREFIX__-result-visual-copy-top"><div class="__PREFIX__-result-kicker">Primary Result</div><h3 class="__PREFIX__-result-title __PREFIX__-result-title-center">Estimated run rate</h3></div>
                    <div class="__PREFIX__-result-primary-visual" id="__DOM_ID_PREFIX__ResultVisual" aria-label="Primary estimate result visual">
                        <span class="__PREFIX__-result-card-icon __PREFIX__-result-card-icon-primary" aria-hidden="true"><i class="bi bi-cash-coin"></i></span>
                        <div class="__PREFIX__-result-ring __PREFIX__-spend-ring" id="__DOM_ID_PREFIX__SpendRing" style="--__PREFIX__-result-progress: ${escapeHtml(String(progressAngle))}deg; --progress-angle: ${escapeHtml(String(progressAngle))}deg; --__PREFIX__-result-value-chars: ${escapeHtml(String(primaryValueChars))};" aria-label="Estimated monthly AWS run rate ${escapeHtml(monthlyTotal)}">
                            <div class="__PREFIX__-spend-chart" id="__DOM_ID_PREFIX__SpendChart" aria-hidden="true"></div>
                            <div class="__PREFIX__-result-ring-center __PREFIX__-spend-value">
                                <strong class="__PREFIX__-result-ring-value">${escapeHtml(monthlyTotal)}</strong>
                                <span class="__PREFIX__-result-ring-unit">Monthly</span>
                            </div>
                        </div>
                    </div>
                    <div class="__PREFIX__-result-visual-copy">
                        <p class="__PREFIX__-result-copy __PREFIX__-result-copy-center">Modeled monthly total from the current assumptions.</p>
                    </div>
                    <span class="__PREFIX__-result-card-divider" aria-hidden="true"></span>
                    <div class="__PREFIX__-result-chip-row __PREFIX__-result-chip-row-center" aria-label="Primary result outcome">
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-outcome __PREFIX__-result-chip-ready"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-cash-coin"></i></span><span>Monthly Estimate</span></span>
                    </div>
                </article>

                <article class="__PREFIX__-result-card __PREFIX__-result-card-summary" aria-label="Estimate summary">
                    <div class="__PREFIX__-result-summary-intro">
                        <span class="__PREFIX__-result-card-icon __PREFIX__-result-card-icon-summary" aria-hidden="true"><i class="bi bi-receipt"></i></span>
                        <div class="__PREFIX__-result-summary-copy">
                            <div class="__PREFIX__-result-kicker">Descriptive Summary</div>
                            <h3 class="__PREFIX__-result-title">AWS run-rate estimate</h3>
                            <p class="__PREFIX__-result-copy">Top driver: ${escapeHtml(topDriver)}. Totals are derived from the current assumptions, visible overrides, and generated service mix.</p>
                        </div>
                    </div>
                    <span class="__PREFIX__-result-card-divider" aria-hidden="true"></span>
                    <div class="__PREFIX__-result-chip-grid" aria-label="Estimate state">
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-ready"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-check2-circle"></i></span><span>${escapeHtml(presetLabel)}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-baseline"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-grid-3x3-gap"></i></span><span>${escapeHtml(String(activeServices))} services</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-${overrideTone}"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-sliders"></i></span><span>${escapeHtml(String(overrideCount))} overrides</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-baseline"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-graph-up-arrow"></i></span><span>Top driver: ${escapeHtml(topDriver)}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-warning"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-hdd-network"></i></span><span>Egress: ${escapeHtml(combinedEgress)}</span></span>
                    </div>
                </article>
            </div>

            <div class="__PREFIX__-result-metric-grid" aria-label="Estimate metrics">
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-success">
                    <span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-calendar2-check"></i></span>
                    <span class="__PREFIX__-result-metric-label">Annual</span>
                    <strong class="__PREFIX__-result-metric-value">${escapeHtml(formatCurrency(result.totals.annualTotal))}</strong>
                    <span class="__PREFIX__-result-metric-copy">Projected from the current monthly run rate.</span>
                    <span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-info">
                    <span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-sun"></i></span>
                    <span class="__PREFIX__-result-metric-label">Daily</span>
                    <strong class="__PREFIX__-result-metric-value">${escapeHtml(formatCurrency(result.totals.dailyTotal))}</strong>
                    <span class="__PREFIX__-result-metric-copy">Thirty-day planning average.</span>
                    <span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-accent-tone">
                    <span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-clock"></i></span>
                    <span class="__PREFIX__-result-metric-label">Hourly</span>
                    <strong class="__PREFIX__-result-metric-value">${escapeHtml(formatCurrency(result.totals.hourlyTotal))}</strong>
                    <span class="__PREFIX__-result-metric-copy">Useful for long-running workload checks.</span>
                    <span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-warning">
                    <span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-hdd-network"></i></span>
                    <span class="__PREFIX__-result-metric-label">Egress</span>
                    <strong class="__PREFIX__-result-metric-value">${escapeHtml(combinedEgress)}</strong>
                    <span class="__PREFIX__-result-metric-copy">Combined modeled outbound traffic.</span>
                    <span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span>
                </article>
            </div>
        `;
    ...
    function buildBreakdownRow(item, index) {
        return `
            <tr>
                ...
                <td class="__PREFIX__-currency">${escapeHtml(formatCurrency(item.monthly))}</td>
                <td class="__PREFIX__-currency">${escapeHtml(formatCurrency(item.annual))}</td>
                <td class="__PREFIX__-copy-cell tool-table-action-cell">
                    <button type="button" class="__PREFIX__-copy-btn" data-copy="${escapeHtml(item.copyValue)}" aria-label="Copy breakdown row ${index + 1}" title="Copy row">
    ...
    function buildServiceRow(row, index) {
        return `
            <tr>
                ...
                <td class="__PREFIX__-currency">${escapeHtml(formatCurrency(row.monthly))}</td>
                <td class="__PREFIX__-currency">${escapeHtml(formatCurrency(row.annual))}</td>
                <td>${escapeHtml(formatPercent(row.sharePct))}</td>
                <td><span class="__PREFIX__-signal ${escapeHtml(row.signalClass)}">${escapeHtml(row.signal)}</span></td>
                <td class="__PREFIX__-copy-cell tool-table-action-cell">
                    <button type="button" class="__PREFIX__-copy-btn" data-copy="${escapeHtml(row.copyValue)}" aria-label="Copy service mix row ${index + 1}" title="Copy row">
                        <i class="bi bi-clipboard" aria-hidden="true"></i>
                    </button>
                </td>
            </tr>
    ...
    function buildAssumptionRow(row, index) {
        return `
            <tr>
                ...
                <td class="__PREFIX__-copy-cell tool-table-action-cell">
                    <button type="button" class="__PREFIX__-copy-btn" data-copy="${escapeHtml(`${row.name}: ${row.value}`)}" aria-label="Copy assumption row ${index + 1}" title="Copy row">
                        <i class="bi bi-clipboard" aria-hidden="true"></i>
                    </button>
                </td>
            </tr>
    ...
    function buildRecommendationsHtml(recommendations) {
        return recommendations.map((recommendation) => `
            <section class="__PREFIX__-recommendation-card">
                <span class="__PREFIX__-basis-label">Recommendation</span>
                <strong>${escapeHtml(recommendation.title)}</strong>
                <span class="__PREFIX__-recommendation-copy">${escapeHtml(recommendation.copy)}</span>
            </section>
        `).join('');
    }
    ...
    function buildMethodologyHtml(result) {
        return `
            <section class="__PREFIX__-method-card">
                <span class="__PREFIX__-method-label">Model Scope</span>
                <span class="__PREFIX__-method-title">Included spend drivers</span>
                <span class="__PREFIX__-method-copy">This model covers EC2, EBS, S3, Lambda, API Gateway, combined internet egress, support uplift, contingency, and any manual monthly adjustment you applied.</span>
            </section>
            <section class="__PREFIX__-method-card">
                <span class="__PREFIX__-method-label">Model Gaps</span>
                <span class="__PREFIX__-method-title">What is not priced automatically</span>
                <span class="__PREFIX__-method-copy">Taxes, CloudFront, NAT gateways, load balancers, RDS, observability, backups, support plan minimums, and private discount agreements are not automatically derived. Add them as explicit overrides or manual adjustments when they matter.</span>
            </section>
        `;
    }
