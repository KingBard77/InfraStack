...
- `source/calculate-cost-aws/`: full copied AWS Cost Calculator source snapshot for traceability and freshness checks.
...
- `family.<family>.workspace.04_visual-contract`: optional calculate visual and model contract for metric cards, driver cards, formula rows, rings, charts, and normalized visual result primitives.
- `family.<family>.workspace.04_selected-item`: optional selected line item or component inspector.
...
templates/content/factory/phase-1/_family/calculate/source/calculate-cost-aws/<section>/page.html.twig
templates/content/factory/phase-1/_family/calculate/source/calculate-cost-aws/<section>/section.css
templates/content/factory/phase-1/_family/calculate/source/calculate-cost-aws/<section>/section.js
templates/content/factory/phase-1/_family/calculate/source/calculate-cost-aws/<section>/demo.html
```

Common workspace shape is composed from `_base/workspace` by `family manifest source_namespaces` `source_namespaces`. Do not recreate duplicate calculate copies of `_base` sections.

The full copied source snapshot is reference-only and must compare cleanly against `templates/content/tools/aws/calculate-cost-aws/` before new calculate tools are created. The current reference set remains:
...
Do not keep loose root source `custom.css`, `custom.js`, or `demo.html.twig` files for the calculate family baseline. For new provider or domain calculators, adapt section sources using the three-tool reference set and verify that copied provider terms and category/provider lineage tokens are replaced.
...
Final calculate validation must fail stale Basic/Custom dropdown regressions: native-select `display: none`, `custom-dropdown`, `custom-native-select`, `select-control` on converted native selects, `enhanced-select`, or JavaScript that builds custom option menus for Basic/Custom settings.
