...
const __DOM_ID_PREFIX__Data = {{ include('content/tools/__CATEGORY__/__TOOL_SLUG__/assets/custom.json.twig')|raw }};
const __DOM_ID_PREFIX__ScriptEndpoint = '{{ path('app___CATEGORY_____TOOL_ID___script') }}';

// ns:start family._base.workspace.00_shell
document.addEventListener('DOMContentLoaded', function () {
    const benchmark = __DOM_ID_PREFIX__Data.benchmark || {};
    const families = Array.isArray(__DOM_ID_PREFIX__Data.families) ? __DOM_ID_PREFIX__Data.families.slice() : [];
    const sections = Array.isArray(__DOM_ID_PREFIX__Data.sections) ? __DOM_ID_PREFIX__Data.sections.slice() : [];
    const controls = Array.isArray(__DOM_ID_PREFIX__Data.controls) ? __DOM_ID_PREFIX__Data.controls.slice() : [];
// ns:start family._base.workspace.01_input-brief
    const form = document.getElementById('__DOM_ID_PREFIX__Form');
    const queryInput = document.getElementById('__DOM_ID_PREFIX__Query');
...
// ns:start family._base.workspace.02_basic-settings
    const familyInput = document.getElementById('__DOM_ID_PREFIX__Family');
    const sectionPathInput = document.getElementById('__DOM_ID_PREFIX__SectionPath');
    const criticalityInput = document.getElementById('__DOM_ID_PREFIX__Criticality');
    const selectedControlInput = document.getElementById('__DOM_ID_PREFIX__SelectedControl');
    const selectedControlTrigger = document.getElementById('__DOM_ID_PREFIX__SelectedControlTrigger');
    const selectedControlLabel = document.getElementById('__DOM_ID_PREFIX__SelectedControlLabel');
    const selectedControlPanel = document.getElementById('__DOM_ID_PREFIX__ControlPickerPanel');
    const selectedControlSearch = document.getElementById('__DOM_ID_PREFIX__ControlPickerSearch');
    const selectedControlOptions = document.getElementById('__DOM_ID_PREFIX__ControlPickerOptions');
// ns:end family._base.workspace.02_basic-settings
    ...
    const sortOptionsContainer = document.getElementById('__DOM_ID_PREFIX__SortOptions');
    const rowLimitInput = document.getElementById('__DOM_ID_PREFIX__RowLimit');
    const criticalityRow = document.getElementById('__DOM_ID_PREFIX__CriticalityRow');
    ...
    const toolbarMeta = document.getElementById('__DOM_ID_PREFIX__ToolbarMeta');
    const controlsTableBody = document.getElementById('__DOM_ID_PREFIX__ControlsTableBody');
    const sectionsTableBody = document.getElementById('__DOM_ID_PREFIX__SectionsTableBody');
    const scriptMeta = document.getElementById('__DOM_ID_PREFIX__ScriptMeta');
    const scriptOutput = document.getElementById('__DOM_ID_PREFIX__ScriptOutput');
    const jsonOutput = document.getElementById('__DOM_ID_PREFIX__JsonOutput');
    const copyScriptButton = document.getElementById('__DOM_ID_PREFIX__CopyScript');
    const downloadScriptButton = document.getElementById('__DOM_ID_PREFIX__DownloadScript');
    const fullScriptButton = document.getElementById('__DOM_ID_PREFIX__FullScript');
    const bundleModal = document.getElementById('__DOM_ID_PREFIX__BundleModal');
    const bundleCloseButton = document.getElementById('__DOM_ID_PREFIX__BundleClose');
    const bundleCancelButton = document.getElementById('__DOM_ID_PREFIX__BundleCancel');
    const bundleDetails = document.getElementById('__DOM_ID_PREFIX__BundleDetails');
    const bundleRequestText = document.getElementById('__DOM_ID_PREFIX__BundleRequestText');
    const bundleEmailLink = document.getElementById('__DOM_ID_PREFIX__BundleEmail');
    const bundleCopyButton = document.getElementById('__DOM_ID_PREFIX__BundleCopy');
...
// ns:end family._base.workspace.03_custom-settings
    ...
    function initMarkdownCopyButtons() {
        ...
        const commandBlocks = Array.from(document.querySelectorAll('.markdown-content pre.__PREFIX__-command-example-pre'));
        const commandCopyButtons = document.querySelectorAll('.__PREFIX__-command-copy-btn');
        ...
        const codeBlocks = document.querySelectorAll('.markdown-content pre:not(.__PREFIX__-prompt-pre):not(.__PREFIX__-command-example-pre)');
    ...
    function renderSortOptions(container, options, selectedValue) {
        container.innerHTML = options.map(function (option) {
            ...
            return `
                <button type="button" class="__PREFIX__-sort-option${isActive ? ' is-active' : ''}" data-sort-value="${escapeHtml(option.value)}" aria-pressed="${isActive ? 'true' : 'false'}"${metaAttribute}>
    ...
    function renderControlPickerButtons(options, selectedValue) {
        if (!options.length) {
            selectedControlOptions.innerHTML = '<div class="__PREFIX__-control-picker-empty">No matched scripts.</div>';
            return;
        }
        ...
        selectedControlOptions.innerHTML = options.map(function (option) {
            ...
            return '<button type="button" class="__PREFIX__-control-picker-option' + (isActive ? ' is-active' : '') + '" data-control-value="' + escapeHtml(option.value) + '" role="option" aria-selected="' + (isActive ? 'true' : 'false') + '"><strong>' + escapeHtml(option.title) + '</strong>' + meta + '</button>';
        }).join('');
    }
    ...
    function renderCriticalityOverview() {
        ...
        criticalityRow.innerHTML = `
            <span class="__PREFIX__-criticality-pill" data-tone="low"><strong>${counts.low || 0}</strong><span>Low</span></span>
            <span class="__PREFIX__-criticality-pill" data-tone="unspecified"><strong>${counts.unspecified || 0}</strong><span>Unspecified</span></span>
        `;
    }
        ...
        return `
            <header class="__PREFIX__-result-header" aria-label="Result summary header">
                <div class="__PREFIX__-result-header-main">
                    <span class="__PREFIX__-result-header-icon" aria-hidden="true"><i class="bi bi-bar-chart-line"></i></span>
                ...
                <div class="__PREFIX__-result-header-meta" aria-label="Result summary status">
                    <span class="__PREFIX__-result-header-chip __PREFIX__-result-chip-ready"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-circle-fill"></i></span><span>Generated</span></span>
                    <span class="__PREFIX__-result-header-chip __PREFIX__-result-chip-updated"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-calendar3"></i></span><span>${escapeHtml(updatedText)}</span></span>
                </div>
            </header>

            <div class="__PREFIX__-result-hero-grid" aria-live="polite">
                <article class="__PREFIX__-result-card __PREFIX__-result-card-primary" data-result-visual="number" aria-label="Primary result">
                    <div class="__PREFIX__-result-primary-heading __PREFIX__-result-visual-copy __PREFIX__-result-visual-copy-top"><div class="__PREFIX__-result-kicker">Primary Result</div><h3 class="__PREFIX__-result-title __PREFIX__-result-title-center">${escapeHtml(visibleRows)} visible rows</h3></div>
                    <div class="__PREFIX__-result-primary-visual" id="__DOM_ID_PREFIX__ResultVisual" aria-label="Primary assessment result">
                        <span class="__PREFIX__-result-card-icon __PREFIX__-result-card-icon-primary" aria-hidden="true"><i class="bi bi-clipboard-check"></i></span>
                        <div class="__PREFIX__-result-primary-number">
                            <strong class="__PREFIX__-result-primary-number-value">${escapeHtml(visibleRows)}</strong>
                            <span class="__PREFIX__-result-primary-number-unit">visible rows</span>
                        </div>
                    </div>
                    <div class="__PREFIX__-result-visual-copy">
                        <p class="__PREFIX__-result-copy __PREFIX__-result-copy-center">Selected: ${escapeHtml(selectedMeta)}</p>
                    </div>
                    <span class="__PREFIX__-result-card-divider" aria-hidden="true"></span>
                    <div class="__PREFIX__-result-chip-row __PREFIX__-result-chip-row-center" aria-label="Primary result source">
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-outcome __PREFIX__-result-chip-ready"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-shield-check"></i></span><span>Benchmark: ${escapeHtml(benchmark.name || 'Ubuntu 22.04 CIS')}</span></span>
                    </div>
                </article>

                <article class="__PREFIX__-result-card __PREFIX__-result-card-summary" aria-label="Assessment summary">
                    <div class="__PREFIX__-result-summary-intro">
                        <span class="__PREFIX__-result-card-icon __PREFIX__-result-card-icon-summary" aria-hidden="true"><i class="bi bi-clipboard-data"></i></span>
                        <div class="__PREFIX__-result-summary-copy">
                            <div class="__PREFIX__-result-kicker">Descriptive Summary</div>
                            <h3 class="__PREFIX__-result-title">Filtered CIS control set</h3>
                            <p class="__PREFIX__-result-copy">The table, section rollups, selected script body, JSON, and exports are all derived from the current filters.</p>
                        </div>
                    </div>
                    <span class="__PREFIX__-result-card-divider" aria-hidden="true"></span>
                    <div class="__PREFIX__-result-chip-grid" aria-label="Assessment state">
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-ready"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-shield-check"></i></span><span>Benchmark: ${escapeHtml(benchmark.name || 'Ubuntu 22.04 CIS')}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-baseline"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-diagram-3"></i></span><span>Family: ${escapeHtml(familyLabel)}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-baseline"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-folder2-open"></i></span><span>Section: ${escapeHtml(sectionLabel)}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-baseline"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-flag"></i></span><span>Criticality: ${escapeHtml(criticalityLabel)}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-warning"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-funnel"></i></span><span>${escapeHtml(queryLabel)}</span></span>
                    </div>
                </article>
            </div>

            <div class="__PREFIX__-result-metric-grid" aria-label="Assessment metrics">
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-success">
                    <span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-file-earmark-text"></i></span>
                    <span class="__PREFIX__-result-metric-label">Matched Scripts</span>
                    <strong class="__PREFIX__-result-metric-value">${escapeHtml(matchedScripts)}</strong>
                    <span class="__PREFIX__-result-metric-copy">Controls matching the selected filters.</span>
                ...
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-info">
                    <span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-eye"></i></span>
                    <span class="__PREFIX__-result-metric-label">Visible Rows</span>
                    <strong class="__PREFIX__-result-metric-value">${escapeHtml(visibleRows)}</strong>
                    <span class="__PREFIX__-result-metric-copy">Rows rendered in the controls table.</span>
                    <span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-accent-tone">
                    <span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-layers"></i></span>
                    <span class="__PREFIX__-result-metric-label">Sections</span>
                    <strong class="__PREFIX__-result-metric-value">${escapeHtml(visibleSections)}</strong>
                    <span class="__PREFIX__-result-metric-copy">Section rollups from the filtered set.</span>
                    <span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span>
                </article>
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-warning">
                    <span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-code"></i></span>
                    <span class="__PREFIX__-result-metric-label">Selected Script</span>
                    <strong class="__PREFIX__-result-metric-value">${escapeHtml(selectedScriptLines)}</strong>
                    <span class="__PREFIX__-result-metric-copy">${escapeHtml(selectedScriptCopy)}</span>
                    <span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span>
                </article>
            </div>
        `;
    ...
    function renderToolbarMeta(result) {
        ...
        toolbarMeta.innerHTML = `
            <span class="__TOOL_CLASS__bar-pill">Family scope: ${result.familyScopedControls.length}</span>
            <span class="__TOOL_CLASS__bar-pill">Section scope: ${result.sectionScopedControls.length}</span>
            <span class="__TOOL_CLASS__bar-pill">Criticality scope: ${result.criticalityScopedControls.length}</span>
            <span class="__TOOL_CLASS__bar-pill">${escapeHtml(rowLimitLabel)}</span>
            <span class="__TOOL_CLASS__bar-pill">Selected: ${escapeHtml(selectedLabel)}</span>
        `;
    }
    ...
    function renderControlsTable(result) {
        ...
        controlsTableBody.innerHTML = result.visibleControls.map(function (control, index) {
            ...
            return `
                <tr data-control-path="${escapeHtml(control.script_path)}" class="${isSelected ? '__PREFIX__-control-row-selected' : ''}">
                  <td class="tool-generated-rownum-cell">${index + 1}</td>
                  <td><span class="__PREFIX__-control-id">${escapeHtml(control.id)}</span></td>
                  <td><span class="__PREFIX__-title-text">${escapeHtml(control.title)}</span></td>
                  <td>
                    <div class="__PREFIX__-section-cell">
                      <span class="__PREFIX__-path-text" title="${escapeHtml(control.section_path)}">${escapeHtml(sectionDisplay)}</span>
                      <span class="__PREFIX__-section-family">${escapeHtml(control.family_label)}</span>
                    </div>
                  </td>
                  <td><span class="__PREFIX__-criticality-badge" data-tone="${escapeHtml(criticalityTone(control.criticality))}">${escapeHtml(criticalityLabel)}</span></td>
                  <td><span class="__PREFIX__-script-text" title="${escapeHtml(control.script_path)}">${escapeHtml(scriptDisplay)}</span></td>
                  <td class="__PREFIX__-table-action-cell tool-table-action-cell">
                    <button type="button" class="__PREFIX__-row-copy" data-control-copy-row="${index}" aria-label="Copy control row ${index + 1}" title="Copy control row">
        ...
        sectionsTableBody.innerHTML = result.visibleSections.map(function (section, index) {
            ...
            return `
                <tr>
                  ...
                  <td>
                    <div class="__PREFIX__-section-cell">
                      <span class="__PREFIX__-path-text">${escapeHtml(section.path)}</span>
                      <span class="__PREFIX__-section-family">${escapeHtml(section.full_label)}</span>
                    </div>
                  </td>
                  <td>${section.depth}</td>
                  <td>${section.visible_control_count}</td>
                  <td>${section.child_count}</td>
                  <td><span class="__PREFIX__-range-text">${escapeHtml(rangeText)}</span></td>
                </tr>
            `;
        }).join('');
    }
    ...
    function renderScriptState(control, statusText, scriptContent) {
        if (!control) {
            scriptMeta.innerHTML = `
                <div class="__PREFIX__-script-chip">
                  <span class="__PREFIX__-script-chip-label">Selection</span>
                  <span class="__PREFIX__-script-chip-value">No matched script</span>
                </div>
            `;
        ...
        const statusMarkup = statusText
            ? `
            <div class="__PREFIX__-script-chip">
              <span class="__PREFIX__-script-chip-label">Status</span>
              <span class="__PREFIX__-script-chip-value">${escapeHtml(statusText)}</span>
            </div>
        `
            : '';
        ...
        scriptMeta.innerHTML = `
            <div class="__PREFIX__-script-chip">
              <span class="__PREFIX__-script-chip-label">Control ID</span>
              <span class="__PREFIX__-script-chip-value">${escapeHtml(control.id)}</span>
            </div>
            <div class="__PREFIX__-script-chip">
              <span class="__PREFIX__-script-chip-label">Title</span>
              <span class="__PREFIX__-script-chip-value">${escapeHtml(control.title)}</span>
            </div>
            <div class="__PREFIX__-script-chip">
              <span class="__PREFIX__-script-chip-label">Criticality</span>
              <span class="__PREFIX__-script-chip-value">${escapeHtml(criticalityLabel)}</span>
            </div>
            <div class="__PREFIX__-script-chip">
              <span class="__PREFIX__-script-chip-label">Section path</span>
              <span class="__PREFIX__-script-chip-value">${escapeHtml(control.section_path)}</span>
            </div>
            <div class="__PREFIX__-script-chip">
              <span class="__PREFIX__-script-chip-label">Script path</span>
              <span class="__PREFIX__-script-chip-value">${escapeHtml(control.script_path)}</span>
            </div>
            <div class="__PREFIX__-script-chip">
              <span class="__PREFIX__-script-chip-label">Lines</span>
              <span class="__PREFIX__-script-chip-value">${control.script_line_count}</span>
            </div>
            ${statusMarkup}
        `;
    ...
    async function requestSelectedScript(scriptPath) {
        const response = await fetch(__DOM_ID_PREFIX__ScriptEndpoint, {
            method: 'POST',
            headers: {
    ...
    function buildBundleRequest(result) {
        ...
        const detailRows = [
            ['Benchmark', benchmark.title || '__TOOL_TITLE__'],
        ...
        return {
            ...
            subject: `Full script request: ${benchmark.title || '__TOOL_TITLE__'}`
        };
    }
    ...
    function renderBundleModal(result) {
        ...
        bundleDetails.innerHTML = bundleRequest.detailRows.map(function (row) {
            return `
                <div class="__PREFIX__-bundle-detail">
    ...
    function openBundleModal() {
        ...
        document.body.classList.add('__PREFIX__-modal-open');
        bundleCloseButton.focus();
    }

    function closeBundleModal() {
        bundleModal.hidden = true;
        document.body.classList.remove('__PREFIX__-modal-open');
...
// ns:end family._base.workspace.08_json-restore
    ...
    selectedControlOptions.addEventListener('click', function (event) {
        ...
        renderExplorer();
        if (latestResult) {
            activateTab('__DOM_ID_PREFIX__ScriptPanel');
        }
    });
    ...
    resetButton.addEventListener('click', function () {
        ...
        showEmptyState();
        activateTab('__DOM_ID_PREFIX__ControlsPanel');
    ...
    controlsTableBody.addEventListener('click', function (event) {
        const target = event.target instanceof HTMLElement ? event.target : null;
        const copyButton = target ? target.closest('.__PREFIX__-row-copy') : null;
...
// ns:start family._base.workspace.06_output-toolbar
    ...
    downloadCsvButton.addEventListener('click', function () {
        ...
        downloadFile('__TOOL_SLUG__.csv', latestResult.csv, 'text/csv;charset=utf-8');
    ...
    downloadJsonButton.addEventListener('click', function () {
        ...
        downloadFile('__TOOL_SLUG__.json', JSON.stringify(latestResult.jsonPayload, null, 2), 'application/json;charset=utf-8');
