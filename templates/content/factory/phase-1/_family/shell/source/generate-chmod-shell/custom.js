...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const targetInput = document.getElementById('__DOM_ID_PREFIX__Target');
    ...
    const targetTypeInput = document.getElementById('__DOM_ID_PREFIX__TargetType');
    const recursiveInput = document.getElementById('__DOM_ID_PREFIX__Recursive');
    const verboseInput = document.getElementById('__DOM_ID_PREFIX__Verbose');
    const scopeOctalBadges = Array.from(document.querySelectorAll('[data-scope-octal]'));
    const permissionInputs = Array.from(document.querySelectorAll('.__PREFIX__-perm-toggle input[type="checkbox"]'));
    const setuidInput = document.getElementById('__DOM_ID_PREFIX__Setuid');
    const setgidInput = document.getElementById('__DOM_ID_PREFIX__Setgid');
    const stickyInput = document.getElementById('__DOM_ID_PREFIX__Sticky');
    const numericInput = document.getElementById('__DOM_ID_PREFIX__NumericMode');
    const numericFeedback = document.getElementById('__DOM_ID_PREFIX__NumericFeedback');
    const symbolicInput = document.getElementById('__DOM_ID_PREFIX__SymbolicMode');
    const symbolicFeedback = document.getElementById('__DOM_ID_PREFIX__SymbolicFeedback');
    const operationInput = document.getElementById('__DOM_ID_PREFIX__Operation');
    const applyOperationButton = document.getElementById('__DOM_ID_PREFIX__ApplyOperation');
    const operationFeedback = document.getElementById('__DOM_ID_PREFIX__OperationFeedback');
    const quickOperationButtons = Array.from(document.querySelectorAll('[data-operation]'));
    const umaskInput = document.getElementById('__DOM_ID_PREFIX__Umask');
    const umaskFileBaseInput = document.getElementById('__DOM_ID_PREFIX__UmaskFileBase');
    const umaskDirBaseInput = document.getElementById('__DOM_ID_PREFIX__UmaskDirBase');
    const umaskFileNumeric = document.getElementById('__DOM_ID_PREFIX__UmaskFileNumeric');
    const umaskFileSymbolic = document.getElementById('__DOM_ID_PREFIX__UmaskFileSymbolic');
    const umaskDirNumeric = document.getElementById('__DOM_ID_PREFIX__UmaskDirNumeric');
    const umaskDirSymbolic = document.getElementById('__DOM_ID_PREFIX__UmaskDirSymbolic');
    const umaskFeedback = document.getElementById('__DOM_ID_PREFIX__UmaskFeedback');
    ...
    const commandOutput = document.getElementById('__DOM_ID_PREFIX__CommandOutput');
    const lsOutput = document.getElementById('__DOM_ID_PREFIX__LsOutput');
    const summaryTableBody = document.getElementById('__DOM_ID_PREFIX__SummaryTableBody');
    const permissionTableBody = document.getElementById('__DOM_ID_PREFIX__PermissionTableBody');
    const umaskTableBody = document.getElementById('__DOM_ID_PREFIX__UmaskTableBody');
    const operationsTableBody = document.getElementById('__DOM_ID_PREFIX__OperationsTableBody');
    const warningsList = document.getElementById('__DOM_ID_PREFIX__WarningsList');
    const errorsList = document.getElementById('__DOM_ID_PREFIX__ErrorsList');
    ...
    const sortOptionButtons = Array.from(document.querySelectorAll('.__PREFIX__-sort-option[data-sort-value]'));
    const sortSelect = document.getElementById('__DOM_ID_PREFIX__SortSelect');
    const copyCommandButton = document.getElementById('__DOM_ID_PREFIX__CopyCommand');
    ...
    function initMarkdownCopyButtons() {
        ...
        codeBlocks.forEach((pre) => {
            ...
            const commandSummary = commandNote && commandNote.classList.contains('__PREFIX__-command-note')
                ? commandNote.querySelector('summary')
                : null;

            if (pre.querySelector('.markdown-copy-btn') || (commandSummary && commandSummary.querySelector('.__PREFIX__-command-copy-btn'))) {
                return;
            }
            ...
            button.type = 'button';
            button.innerHTML = '<i class="bi bi-clipboard"></i><span class="__PREFIX__-command-copy-label">Copy</span>';
            ...
            if (commandSummary) {
                ...
                labelGroup.className = '__PREFIX__-command-note-summary-labels';

                while (commandSummary.firstChild) {
                    labelGroup.appendChild(commandSummary.firstChild);
                }

                button.className = '__PREFIX__-command-copy-btn';
                ...
                pre.classList.add('__PREFIX__-command-pre');

                return;
            }

            button.className = 'markdown-copy-btn';
            button.querySelector('.__PREFIX__-command-copy-label').textContent = 'Copy';
            pre.appendChild(button);
        });
    }

    function flashButton(button, text) {
        const label = button.querySelector('[data-button-label]') || button.querySelector('.__PREFIX__-command-copy-label');

        if (!label && (button.classList.contains('__PREFIX__-row-copy') || (button.closest && button.closest('.tool-table-action-cell')))) {
...
function createBlankState(targetType) {
    ...
    function clearFeedback(element) {
        ...
        element.classList.remove('__PREFIX__-feedback-success', '__PREFIX__-feedback-error');
    }

    function showFeedback(element, type, message) {
        element.textContent = message;
        element.classList.remove('d-none', '__PREFIX__-feedback-success', '__PREFIX__-feedback-error');
        element.classList.add(type === 'error' ? '__PREFIX__-feedback-error' : '__PREFIX__-feedback-success');
    }
    ...
    function exportResultShellAsPdf(filenameStem, container) {
        ...
        const shell = container.querySelector('.__PREFIX__-result-shell') || container;
...
<body>
  ${shell.outerHTML}
    ...
    function renderMessageList(element, messages, emptyLabel) {
        if (!messages.length) {
            element.className = '__PREFIX__-message-list __PREFIX__-message-list-empty';
            element.innerHTML = `<li>${escapeHtml(emptyLabel)}</li>`;
            return;
        }

        element.className = '__PREFIX__-message-list';
    ...
    function renderEmptyTableRow(tableBody, colspan, label) {
        tableBody.innerHTML = `<tr class="__PREFIX__-empty-row"><td colspan="${colspan}">${escapeHtml(label)}</td></tr>`;
    }
        ...
        resultSummary.innerHTML = `
            <header class="__PREFIX__-result-header" aria-label="Result summary header">
                <div class="__PREFIX__-result-header-main">
                    <span class="__PREFIX__-result-header-icon" aria-hidden="true"><i class="bi bi-terminal"></i></span>
            ...
            <div class="__PREFIX__-result-hero-grid" aria-live="polite">
                <article class="__PREFIX__-result-card __PREFIX__-result-card-primary" data-result-visual="command" aria-label="Primary command result">
                    <div class="__PREFIX__-result-primary-heading __PREFIX__-result-visual-copy __PREFIX__-result-visual-copy-top"><div class="__PREFIX__-result-kicker">Primary Result</div><h3 class="__PREFIX__-result-title __PREFIX__-result-title-center">Mode result</h3></div>
                    <div class="__PREFIX__-result-primary-visual" id="__DOM_ID_PREFIX__ResultVisual" aria-label="Primary chmod result">
                        <div class="__PREFIX__-result-command-output"><code class="__PREFIX__-result-command-value">${escapeHtml(`Mode ${result.numeric}`)}</code></div>
                    </div>
                    <div class="__PREFIX__-result-visual-copy"><p class="__PREFIX__-result-copy __PREFIX__-result-copy-center">Compact permission preview for the generated chmod command.</p></div>
                    <span class="__PREFIX__-result-card-divider" aria-hidden="true"></span>
                    <div class="__PREFIX__-result-chip-row __PREFIX__-result-chip-row-center" aria-label="Primary result outcome"><span class="__PREFIX__-result-chip __PREFIX__-result-chip-outcome __PREFIX__-result-chip-ready"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-terminal"></i></span><span>Command Generated</span></span></div>
                </article>
                <article class="__PREFIX__-result-card __PREFIX__-result-card-summary" aria-label="Command summary">
                    <div class="__PREFIX__-result-summary-intro"><span class="__PREFIX__-result-card-icon __PREFIX__-result-card-icon-summary" aria-hidden="true"><i class="bi bi-terminal"></i></span><div class="__PREFIX__-result-summary-copy"><div class="__PREFIX__-result-kicker">Descriptive Summary</div><h3 class="__PREFIX__-result-title">chmod mode preview</h3><p class="__PREFIX__-result-copy">Numeric, symbolic, and ls-style output stay aligned with the current target and permission controls.</p></div></div>
                    <span class="__PREFIX__-result-card-divider" aria-hidden="true"></span>
                    <div class="__PREFIX__-result-chip-grid" aria-label="Command state">
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-baseline"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-folder2"></i></span><span>${escapeHtml(result.targetType === 'directory' ? 'Directory' : 'File')}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-ready"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-check2-circle"></i></span><span>${escapeHtml(result.flagsLabel)}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-${warningTone}"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-exclamation-triangle"></i></span><span>${escapeHtml(`${result.warnings.length} warning${result.warnings.length === 1 ? '' : 's'}`)}</span></span>
                        <span class="__PREFIX__-result-chip __PREFIX__-result-chip-baseline"><span class="__PREFIX__-result-chip-icon" aria-hidden="true"><i class="bi bi-terminal"></i></span><span>${escapeHtml(`${result.commandMode} command mode`)}</span></span>
                    </div>
                </article>
            </div>
            <div class="__PREFIX__-result-metric-grid" aria-label="Command metrics">
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-success"><span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-folder2"></i></span><span class="__PREFIX__-result-metric-label">Target</span><strong class="__PREFIX__-result-metric-value">${escapeHtml(result.target || 'No path')}</strong><span class="__PREFIX__-result-metric-copy">Path used in the command.</span><span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span></article>
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-info"><span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-person-check"></i></span><span class="__PREFIX__-result-metric-label">Triplets</span><strong class="__PREFIX__-result-metric-value">${escapeHtml(buildTripletSummary(result.state))}</strong><span class="__PREFIX__-result-metric-copy">Owner, group, and others.</span><span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span></article>
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-accent-tone"><span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-shield-lock"></i></span><span class="__PREFIX__-result-metric-label">Special</span><strong class="__PREFIX__-result-metric-value">${escapeHtml(buildSpecialSummary(result.state))}</strong><span class="__PREFIX__-result-metric-copy">Setuid, setgid, or sticky bits.</span><span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span></article>
                <article class="__PREFIX__-result-metric-card __PREFIX__-result-metric-warning"><span class="__PREFIX__-result-metric-icon" aria-hidden="true"><i class="bi bi-file-code"></i></span><span class="__PREFIX__-result-metric-label">ls -l</span><strong class="__PREFIX__-result-metric-value">${escapeHtml(result.lsPreview)}</strong><span class="__PREFIX__-result-metric-copy">Permission string preview.</span><span class="__PREFIX__-result-metric-accent" aria-hidden="true"></span></article>
            </div>
        `;
    }
        ...
        operationsTableBody.innerHTML = getSortedOperationRows(result)
            .map((row, index) => `
                <tr>
                    ...
                    <td>${escapeHtml(row.result)}</td>
                    <td class="__PREFIX__-table-copy-cell tool-table-action-cell">
                        <button type="button" class="__PREFIX__-row-copy __PREFIX__-row-copy-btn" data-operation-copy="${escapeHtml(row.result)}" aria-label="Copy operation row ${index + 1}" title="Copy operation row">
...
// ns:start family.shell.workspace.04_result-text
    function renderResult(result) {
        ...
        activateTab('__DOM_ID_PREFIX__OperationsPanel');
    }
...
// ns:end family._base.workspace.02_basic-settings
    ...
    operationsTableBody.addEventListener('click', function (event) {
        ...
        const copyButton = target.closest('.__PREFIX__-row-copy, .__PREFIX__-row-copy-btn');
    ...
    downloadJsonButton.addEventListener('click', function () {
        ...
        downloadFile('__TOOL_SLUG__.json', `${JSON.stringify(latestResult.jsonPayload, null, 2)}\n`, 'application/json;charset=utf-8');
