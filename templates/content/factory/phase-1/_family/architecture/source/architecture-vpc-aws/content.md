...
<div class="content-card __PREFIX__-markdown-card __PREFIX__-markdown-card-overview">
  ...
  <p><strong>__TOOL_TITLE__</strong> is an InfraStack prompt-driven workspace for turning a short AWS VPC brief into a first-pass architecture diagram. The workspace reads explicit terms such as <span class="__PREFIX__-term-accent">region</span>, <span class="__PREFIX__-term-accent">availability zone</span>, <span class="__PREFIX__-term-accent">VPC CIDR</span>, ingress services, workload tier, data tier, egress mode, observability controls, and hybrid links, then uses that normalized model to render the stage, technical inventory, prompt notes, and export state.</p>
  <p><a id="__PREFIX__-cite-primary" class="__PREFIX__-citation-link" href="#__PREFIX__-ref-primary"><span class="__PREFIX__-citation-inline">Amazon Web Services (n.d.-a)</span></a> describes Amazon VPC as a logically isolated virtual network for AWS resources. The workspace uses that source-backed boundary as the anchor for placement, review, and restore behavior.</p>
...
<h3 class="__PREFIX__-technical-step-heading">2. Boundary and placement layers</h3>
...
<h3 class="__PREFIX__-technical-step-heading">3. Ingress and edge path</h3>
...
<h3 class="__PREFIX__-technical-step-heading">4. Workload and data tiers</h3>
...
<h3 class="__PREFIX__-technical-step-heading">5. Egress, private access, and hybrid connectivity</h3>
...
<h3 class="__PREFIX__-technical-step-heading">6. Operations, observability, and support surfaces</h3>
...
<h3 class="__PREFIX__-technical-step-heading">7. Editable stage and normalized state</h3>
...
<h3 class="__PREFIX__-technical-step-heading">10. Handoff checklist</h3>
<p>A clean handoff includes the diagram, the inventory rows, the prompt notes, and the JSON state. The diagram explains the visible model, but the notes explain how the workspace interpreted the prompt. The inventory gives reviewers a compact list of components, placements, and purposes. The JSON keeps the model editable so another reviewer can reopen the same state instead of recreating it from memory. For __TOOL_TITLE__, the handoff should also state which choices came from the prompt, which choices came from the preset, and which choices were corrected after generation. That distinction matters because a diagram that looks polished can still hide a weak assumption if the team does not know where a value came from.</p>
...
<h3 class="__PREFIX__-technical-step-heading">11. What the export does not prove</h3>
...
<pre class="__PREFIX__-prompt-pre"><code>Build an AWS VPC architecture in eu-west-1 across 2 availability zones for an ECS Fargate platform. Put CloudFront and AWS WAF in front of an Application Load Balancer, keep ECS services in private app subnets, use Aurora in private data subnets, add ElastiCache Redis, VPC endpoints, CloudWatch, flow logs, and one NAT gateway per AZ.</code></pre>
...
<pre class="__PREFIX__-prompt-pre"><code>Draft a private AWS VPC in ap-southeast-1 with no public application tier, no NAT, VPC endpoints, Systems Manager access, Transit Gateway connectivity to shared services, RDS in private data subnets, CloudWatch, and VPC Flow Logs.</code></pre>
...
<div class="content-card __PREFIX__-markdown-card __PREFIX__-markdown-card-citations">
  ...
  <table class="__PREFIX__-citation-table">
    ...
    <tbody>
      <tr id="__PREFIX__-ref-primary"><td>Website</td><td><a class="__PREFIX__-citation-backlink" href="#__PREFIX__-cite-primary"><span class="__PREFIX__-citation-inline">(Amazon Web Services, n.d.-a)</span></a></td><td>Amazon Web Services. (n.d.-a). <em>What is Amazon VPC?</em> Amazon Virtual Private Cloud User Guide. Retrieved May 17, 2026, from <a href="https://docs.aws.amazon.com/vpc/latest/userguide/what-is-amazon-vpc.html">https://docs.aws.amazon.com/vpc/latest/userguide/what-is-amazon-vpc.html</a></td></tr>
      <tr id="__PREFIX__-ref-method"><td>Website</td><td><a class="__PREFIX__-citation-backlink" href="#__PREFIX__-cite-method"><span class="__PREFIX__-citation-inline">(Amazon Web Services, n.d.-b)</span></a></td><td>Amazon Web Services. (n.d.-b). <em>NAT gateways</em>. Amazon Virtual Private Cloud User Guide. Retrieved May 17, 2026, from <a href="https://docs.aws.amazon.com/vpc/latest/userguide/vpc-nat-gateway.html">https://docs.aws.amazon.com/vpc/latest/userguide/vpc-nat-gateway.html</a></td></tr>
      <tr id="__PREFIX__-ref-review"><td>Framework website</td><td><a class="__PREFIX__-citation-backlink" href="#__PREFIX__-cite-review"><span class="__PREFIX__-citation-inline">(Amazon Web Services, 2024)</span></a></td><td>Amazon Web Services. (2024). <em>AWS Well-Architected Framework</em>. Retrieved May 17, 2026, from <a href="https://docs.aws.amazon.com/wellarchitected/latest/framework/welcome.html">https://docs.aws.amazon.com/wellarchitected/latest/framework/welcome.html</a></td></tr>
    </tbody>
  </table>
</div>
