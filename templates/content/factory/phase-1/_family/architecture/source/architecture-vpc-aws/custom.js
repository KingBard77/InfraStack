...
{{ include('content/tools/__CATEGORY__/__TOOL_SLUG__/assets/bin/engine-runtime.js')|raw }}
// ns:end family.architecture.workspace.04_visual-contract
{{ include('content/tools/__CATEGORY__/__TOOL_SLUG__/assets/bin/model-core.js')|raw }}

const __DOM_ID_PREFIX__PresetCatalog = [
    {
        id: 'three-tier-web',
        label: '3-Tier Web',
        description: 'Public ALB, private EC2 app tier, Multi-AZ RDS, single NAT.',
        prompt: 'Create an AWS VPC architecture in us-east-1 with 2 availability zones. Use public subnets for an internet-facing Application Load Balancer, private app subnets for EC2 application servers, and private data subnets for a Multi-AZ RDS PostgreSQL database. Add a single NAT gateway, VPC endpoints for S3 and Systems Manager, CloudWatch, and VPC flow logs.',
        defaults: {
            region: 'us-east-1',
            cidr: '10.0.0.0/16',
            azCount: 2,
    ...
    },
    {
        id: 'ecs-fargate',
        label: 'ECS Fargate',
        description: 'CloudFront, WAF, ECS services, Aurora, Redis, and per-AZ NAT.',
        prompt: 'Build an AWS VPC architecture in eu-west-1 across 2 availability zones for an internet-facing ECS Fargate platform. Put CloudFront and AWS WAF in front of an Application Load Balancer, keep ECS services in private app subnets, use Aurora in private data subnets, add ElastiCache Redis, VPC endpoints, CloudWatch, flow logs, and one NAT gateway per AZ.',
        defaults: {
            region: 'eu-west-1',
            cidr: '10.20.0.0/16',
            azCount: 2,
    ...
    },
    {
        id: 'eks-platform',
        label: 'Private EKS',
        description: 'Three AZ platform with private nodes, endpoints, bastion, and observability.',
        prompt: 'Generate an AWS VPC architecture in ap-southeast-1 across 3 availability zones for a private EKS platform. Keep an Application Load Balancer in public subnets, worker nodes in private app subnets, Aurora in private data subnets, a bastion host for controlled admin access, VPC endpoints, CloudWatch, and VPC flow logs. Use one NAT gateway per AZ.',
        defaults: {
    ...
    },
    {
        ...
        description: 'VPN or transit gateway connectivity with shared service layers.',
        prompt: 'Design an AWS VPC architecture in us-west-2 across 2 availability zones for a hybrid environment. Use Route 53, an internet-facing Application Load Balancer, EC2 application servers in private app subnets, RDS in private data subnets, a bastion host, VPC endpoints, CloudWatch, flow logs, site-to-site VPN, and a transit gateway for shared services. Use a single NAT gateway.',
        defaults: {
            region: 'us-west-2',
            cidr: '10.40.0.0/16',
            azCount: 2,
...
const ARCHITECTURE_VPC_AWS_TOOL_ID = __PASCAL_PREFIX__ModelCore.toolId;
const ARCHITECTURE_VPC_AWS_TOOL_VERSION = __PASCAL_PREFIX__ModelCore.toolVersion;
const __DOM_ID_PREFIX__RegionCatalog = __PASCAL_PREFIX__ModelCore.regionCatalog;
const __DOM_ID_PREFIX__SupportedRegions = __PASCAL_PREFIX__ModelCore.supportedRegions;
const __DOM_ID_PREFIX__AllowedNatModes = __PASCAL_PREFIX__ModelCore.allowedNatModes;
const __DOM_ID_PREFIX__AllowedAppTiers = __PASCAL_PREFIX__ModelCore.allowedAppTiers;
const __DOM_ID_PREFIX__AllowedDatabases = __PASCAL_PREFIX__ModelCore.allowedDatabases;
const __DOM_ID_PREFIX__EngineRuntime = window.InfraStackArchitectureEngineRuntime || null;

const __DOM_ID_PREFIX__IconUrlMap = {
    amazonVpc: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'amazon-vpc' })|json_encode|raw }},
    route53: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'route-53' })|json_encode|raw }},
    cloudFront: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'cloud-front' })|json_encode|raw }},
    waf: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'waf' })|json_encode|raw }},
    internetGateway: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'internet-gateway' })|json_encode|raw }},
    applicationLoadBalancer: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'application-load-balancer' })|json_encode|raw }},
    vpcRouter: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'vpc-router' })|json_encode|raw }},
    natGateway: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'nat-gateway' })|json_encode|raw }},
    ec2: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'ec2' })|json_encode|raw }},
    ec2AutoScaling: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'ec2-auto-scaling' })|json_encode|raw }},
    ecs: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'ecs' })|json_encode|raw }},
    eks: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'eks' })|json_encode|raw }},
    fargate: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'fargate' })|json_encode|raw }},
    lambda: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'lambda' })|json_encode|raw }},
    rds: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'rds' })|json_encode|raw }},
    aurora: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'aurora' })|json_encode|raw }},
    dynamodb: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'dynamodb' })|json_encode|raw }},
    elasticache: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'elasticache' })|json_encode|raw }},
    vpcEndpoints: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'vpc-endpoints' })|json_encode|raw }},
    vpcFlowLogs: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'vpc-flow-logs' })|json_encode|raw }},
    cloudWatch: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'cloud-watch' })|json_encode|raw }},
    systemsManager: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'systems-manager' })|json_encode|raw }},
    siteToSiteVpn: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'site-to-site-vpn' })|json_encode|raw }},
    transitGateway: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'transit-gateway' })|json_encode|raw }},
    bastion: {{ path('app___CATEGORY_____TOOL_ID___icon', { iconKey: 'bastion' })|json_encode|raw }}
};

function init__PASCAL_PREFIX__Tool() {
    const toolRoot = document.querySelector('.__TOOL_CLASS__');

    if (!toolRoot || toolRoot.dataset.runtimeReady === 'true') {
        return;
    }

    toolRoot.dataset.runtimeReady = 'true';

// ns:start family._base.workspace.01_input-brief
    const promptInput = document.getElementById('__DOM_ID_PREFIX__Prompt');
    const generateButton = document.getElementById('__DOM_ID_PREFIX__Generate');
    const resetButton = document.getElementById('__DOM_ID_PREFIX__Reset');
    const errorState = document.getElementById('__DOM_ID_PREFIX__ErrorState');
// ns:end family._base.workspace.01_input-brief
// ns:start family._base.workspace.02_basic-settings
    const presetInput = document.getElementById('__DOM_ID_PREFIX__Preset');
    const presetDescription = document.getElementById('__DOM_ID_PREFIX__PresetDescription');
// ns:end family._base.workspace.02_basic-settings
// ns:start family.architecture.workspace.04_visual-contract
    const stageTitle = document.getElementById('__DOM_ID_PREFIX__StageTitle');
    const stageSubtitle = document.getElementById('__DOM_ID_PREFIX__StageSubtitle');
    const stageMeta = document.getElementById('__DOM_ID_PREFIX__StageMeta');
// ns:end family.architecture.workspace.04_visual-contract
// ns:start family.architecture.workspace.04_visual-contract
    const stageEmpty = document.getElementById('__DOM_ID_PREFIX__StageEmpty');
    const stageCanvas = document.getElementById('__DOM_ID_PREFIX__StageCanvas');
    const stageShell = document.getElementById('__DOM_ID_PREFIX__StageShell');
    const zoomControl = document.getElementById('__DOM_ID_PREFIX__ZoomControl');
    const zoomLabel = document.getElementById('__DOM_ID_PREFIX__ZoomLabel');
    const zoomInput = document.getElementById('__DOM_ID_PREFIX__ZoomInput');
    const zoomOutButton = document.getElementById('__DOM_ID_PREFIX__ZoomOut');
    const zoomInButton = document.getElementById('__DOM_ID_PREFIX__ZoomIn');
    const zoomFitButton = document.getElementById('__DOM_ID_PREFIX__ZoomFit');
    const zoomActualButton = document.getElementById('__DOM_ID_PREFIX__ZoomActual');
    const undoStageEditButton = document.getElementById('__DOM_ID_PREFIX__UndoStageEdit');
    const highlightAllButton = document.getElementById('__DOM_ID_PREFIX__HighlightAll');
    const zoomHideUiButton = document.getElementById('__DOM_ID_PREFIX__ZoomHideUi');
    const usageHelpButton = document.getElementById('__DOM_ID_PREFIX__UsageHelpButton');
    const usageHelpPopup = document.getElementById('__DOM_ID_PREFIX__UsageHelpPopup');
    const usageHelpCloseButton = document.getElementById('__DOM_ID_PREFIX__UsageHelpClose');
    const fullscreenButton = document.getElementById('__DOM_ID_PREFIX__Fullscreen');
    const resetLayoutButton = document.getElementById('__DOM_ID_PREFIX__ResetLayout');
// ns:end family.architecture.workspace.04_visual-contract
// ns:start family.architecture.workspace.04_visual-contract
    const outputEmpty = document.getElementById('__DOM_ID_PREFIX__OutputEmpty');
    const outputContent = document.getElementById('__DOM_ID_PREFIX__OutputContent');
// ns:end family.architecture.workspace.04_visual-contract
// ns:start family._base.workspace.05_result-summary
    const outputStatus = document.getElementById('__DOM_ID_PREFIX__OutputStatus');
// ns:end family._base.workspace.05_result-summary
    const pillarBreakdownOutput = document.getElementById('__DOM_ID_PREFIX__PillarBreakdown');
    const riskLevelOutput = document.getElementById('__DOM_ID_PREFIX__RiskLevel');
    const inventoryTableBody = document.getElementById('__DOM_ID_PREFIX__InventoryTableBody');
// ns:start family._base.workspace.06_output-toolbar
    const inventorySortInput = document.getElementById('__DOM_ID_PREFIX__InventorySort');
    const inventorySortSelect = document.getElementById('__DOM_ID_PREFIX__InventorySortSelect');
    const inventorySortSummary = document.getElementById('__DOM_ID_PREFIX__InventorySortSummary');
    const inventorySortOptions = Array.from(document.querySelectorAll('.__PREFIX__-sort-option'));
// ns:end family._base.workspace.06_output-toolbar
    const jsonOutput = document.getElementById('__DOM_ID_PREFIX__JsonOutput');
// ns:start family.architecture.workspace.04_visual-contract
    const promptSummary = document.getElementById('__DOM_ID_PREFIX__PromptSummary');
    const keywordList = document.getElementById('__DOM_ID_PREFIX__KeywordList');
    const assumptionList = document.getElementById('__DOM_ID_PREFIX__AssumptionList');
    const modelList = document.getElementById('__DOM_ID_PREFIX__ModelList');
    const prosList = document.getElementById('__DOM_ID_PREFIX__ProsList');
    const consList = document.getElementById('__DOM_ID_PREFIX__ConsList');
// ns:end family.architecture.workspace.04_visual-contract
// ns:start family.architecture.workspace.04_selected-item
    const selectedShell = document.getElementById('__DOM_ID_PREFIX__SelectedShell');
    const selectedEmpty = document.getElementById('__DOM_ID_PREFIX__SelectedEmpty');
    const selectedEditor = document.getElementById('__DOM_ID_PREFIX__SelectedEditor');
    const selectedName = document.getElementById('__DOM_ID_PREFIX__SelectedName');
    const selectedXInput = document.getElementById('__DOM_ID_PREFIX__SelectedX');
    const selectedYInput = document.getElementById('__DOM_ID_PREFIX__SelectedY');
    const selectedWidthInput = document.getElementById('__DOM_ID_PREFIX__SelectedWidth');
    const selectedHeightInput = document.getElementById('__DOM_ID_PREFIX__SelectedHeight');
    const highlightCardButton = document.getElementById('__DOM_ID_PREFIX__HighlightCard');
    const applyCardSizeButton = document.getElementById('__DOM_ID_PREFIX__ApplyCardSize');
    const resetCardSizeButton = document.getElementById('__DOM_ID_PREFIX__ResetCardSize');
// ns:end family.architecture.workspace.04_selected-item
// ns:start family._base.workspace.06_output-toolbar
    const exportPngButton = document.getElementById('__DOM_ID_PREFIX__ExportPng');
    const downloadSvgButton = document.getElementById('__DOM_ID_PREFIX__DownloadSvg');
    const copyJsonButton = document.getElementById('__DOM_ID_PREFIX__CopyJson');
    const downloadJsonButton = document.getElementById('__DOM_ID_PREFIX__DownloadJson');
    const importJsonButton = document.getElementById('__DOM_ID_PREFIX__ImportJsonButton');
    const importJsonInput = document.getElementById('__DOM_ID_PREFIX__ImportJson');
// ns:end family._base.workspace.06_output-toolbar
    const tabButtons = Array.from(document.querySelectorAll('.__PREFIX__-tab-btn'));
    const tabPanels = Array.from(document.querySelectorAll('.__PREFIX__-tab-panel'));
    const configTabButtons = Array.from(document.querySelectorAll('.__PREFIX__-config-tab'));
    const configTabPanels = Array.from(document.querySelectorAll('[data-config-panel]'));
// ns:start family._base.workspace.02_basic-settings
    const regionInput = document.getElementById('__DOM_ID_PREFIX__Region');
    const cidrInput = document.getElementById('__DOM_ID_PREFIX__Cidr');
    const azCountInput = document.getElementById('__DOM_ID_PREFIX__AzCount');
// ns:end family._base.workspace.02_basic-settings
// ns:start family._base.workspace.03_custom-settings
    const natModeInput = document.getElementById('__DOM_ID_PREFIX__NatMode');
    const appTierInput = document.getElementById('__DOM_ID_PREFIX__AppTier');
    const databaseInput = document.getElementById('__DOM_ID_PREFIX__Database');
    const route53Input = document.getElementById('__DOM_ID_PREFIX__Route53');
    const cloudFrontInput = document.getElementById('__DOM_ID_PREFIX__CloudFront');
    const wafInput = document.getElementById('__DOM_ID_PREFIX__Waf');
    const albInput = document.getElementById('__DOM_ID_PREFIX__Alb');
    const bastionInput = document.getElementById('__DOM_ID_PREFIX__Bastion');
    const endpointsInput = document.getElementById('__DOM_ID_PREFIX__Endpoints');
    const flowLogsInput = document.getElementById('__DOM_ID_PREFIX__FlowLogs');
    const cloudWatchInput = document.getElementById('__DOM_ID_PREFIX__CloudWatch');
    const siteToSiteVpnInput = document.getElementById('__DOM_ID_PREFIX__SiteToSiteVpn');
    const transitGatewayInput = document.getElementById('__DOM_ID_PREFIX__TransitGateway');
    const cacheInput = document.getElementById('__DOM_ID_PREFIX__Cache');
// ns:end family._base.workspace.03_custom-settings
    ...
    let selectedPresetId = __DOM_ID_PREFIX__PresetCatalog[0].id;
    let latestResult = null;
    let echartsPromise = null;
    let scoreRingChart = null;
    let scoreRingResizeHandler = null;
    let scoreRingRenderToken = 0;
    let selectedCardId = '';
    let selectedCardIds = [];
    ...
    const engineRuntimeConfig = {
        ...
        classes: {
            ...
            diagramHighlighted: '__PREFIX__-stage-highlight-all',
            dragging: '__PREFIX__-stage-dragging',
            resizing: '__PREFIX__-stage-resizing',
            uiHidden: '__PREFIX__-stage-ui-hidden',
            expanded: '__PREFIX__-stage-expanded',
            bodyLock: '__PREFIX__-stage-expanded-lock',
            hidden: 'd-none'
        }
    };
    const diagramShellCardId = 'amazon-vpc-shell';
    const baseStageMinWidth = 1120;
    const stageHeadingTitle = '__TOOL_TITLE__';
...
// ns:start family.architecture.workspace.04_selected-item
    function renderSelectedEmptyMessage(message) {
        ...
        selectedEmpty.classList.toggle('__PREFIX__-selected-empty-chips', messageText === selectedCardHintText);
        ...
        copy.className = '__PREFIX__-selected-empty-copy';
        ...
        selectedCardHintChips.forEach(function renderHintChip(chip) {
            ...
            chipElement.className = '__PREFIX__-selected-hint-chip __PREFIX__-selected-hint-chip-' + chip.tone;
    ...
    function showSelectedShell() {
        selectedShell.classList.remove('d-none');
        selectedShell.hidden = false;
    }

// ns:end family.architecture.workspace.04_selected-item
    function initMarkdownCopyButtons() {
        const promptBlocks = Array.from(document.querySelectorAll('.markdown-content pre.__PREFIX__-prompt-pre'));
        const promptCopyButtons = document.querySelectorAll('.__PREFIX__-prompt-copy-btn');
        ...
        const codeBlocks = document.querySelectorAll('.markdown-content pre:not(.__PREFIX__-prompt-pre)');
    ...
    function applyWorkspaceInfoMarkers() {
        ...
        const markerClass = '__PREFIX__-info-marker';
        ...
        Array.from(root.querySelectorAll([
            ...
            if (!marker) {
                ...
                marker.tabIndex = 0;
                marker.innerHTML = '<span class="__PREFIX__-info-glyph" aria-hidden="true">i</span>';
                element.appendChild(marker);
            }

            let popover = marker.querySelector('.__PREFIX__-info-popover');

            if (!popover) {
                popover = document.createElement('span');
                popover.className = '__PREFIX__-info-popover';
...
// ns:start family._base.workspace.01_input-brief
    function normalizePrompt(value) {
        return __PASCAL_PREFIX__ModelCore.normalizePrompt(value);
    }

    function normalizeCidrInput(value) {
        return __PASCAL_PREFIX__ModelCore.normalizeCidrInput(value);
    }
    ...
    function isValidIpv4Cidr(value) {
        return __PASCAL_PREFIX__ModelCore.isValidIpv4Cidr(value);
    }

    function parseCidrValue(value) {
        return __PASCAL_PREFIX__ModelCore.parseCidrValue(value);
    }
...
// ns:end family._base.workspace.01_input-brief
    ...
    function isSupportedImportVersion(version) {
        ...
        return version.split('.')[0] === ARCHITECTURE_VPC_AWS_TOOL_VERSION.split('.')[0];
    }
    ...
    function buildImportedPayloadState(payload) {
        return __PASCAL_PREFIX__ModelCore.buildImportedPayloadState(payload, __DOM_ID_PREFIX__PresetCatalog);
    }
    ...
    function natModeLabel(value) {
        return __PASCAL_PREFIX__ModelCore.natModeLabel(value);
    }

    function appTierLabel(value) {
        return __PASCAL_PREFIX__ModelCore.appTierLabel(value);
    }

    function databaseLabel(value) {
        return __PASCAL_PREFIX__ModelCore.databaseLabel(value);
    }

// ns:start family._base.workspace.02_basic-settings
// ns:start family._base.workspace.02_basic-settings
    function findPresetById(presetId) {
        return __DOM_ID_PREFIX__PresetCatalog.find(function (preset) {
            return preset.id === presetId;
        }) || __DOM_ID_PREFIX__PresetCatalog[0];
    }

    function populateRegionOptions() {
        const currentValue = regionInput.value;

        regionInput.innerHTML = __DOM_ID_PREFIX__RegionCatalog.map(function (region) {
            return '<option value="' + escapeHtml(region.value) + '">' + escapeHtml(region.label) + '</option>';
        }).join('');

        regionInput.value = __DOM_ID_PREFIX__SupportedRegions.includes(currentValue)
            ? currentValue
            : __DOM_ID_PREFIX__RegionCatalog[0].value;
    }
...
// ns:start family.architecture.workspace.04_visual-contract
    ...
    function setStageUiHidden(isHidden) {
        ...
        stageShell.classList.toggle('__PREFIX__-stage-ui-hidden', stageUiHidden);
    ...
    function syncStageDiagramHighlight() {
        ...
        stageShell.classList.toggle('__PREFIX__-stage-highlight-all', stageDiagramHighlighted);
    ...
    function updateFullscreenButton() {
        const isFullscreen = document.fullscreenElement === stageShell || stageShell.classList.contains('__PREFIX__-stage-expanded');
    ...
    function setStageExpanded(isExpanded) {
        stageShell.classList.toggle('__PREFIX__-stage-expanded', Boolean(isExpanded));
        document.body.classList.toggle('__PREFIX__-stage-expanded-lock', Boolean(isExpanded));
    ...
    function resetStageViewport() {
        const svgElement = stageCanvas.querySelector('svg');
        const vpcCard = getRenderedCardById('amazon-vpc-shell');
        ...
        const left = Math.max(0, ((vpcCard.x + (vpcCard.width / 2)) * scale) - (stageCanvas.clientWidth / 2));
        const top = Math.max(0, ((vpcCard.y + (vpcCard.height / 2)) * scale) - (stageCanvas.clientHeight / 2));
    ...
    async function toggleFullscreen() {
        try {
            if (stageShell.classList.contains('__PREFIX__-stage-expanded')) {
...
// ns:end family._base.workspace.01_input-brief
    ...
    function createToneChip(iconClass, label, tone) {
        return [
            '<span class="__PREFIX__-score-tag __PREFIX__-score-tag-' + escapeHtml(tone) + '">',
...
// ns:start family._base.workspace.01_input-brief
    function extractRegion(prompt, fallbackRegion, assumptions, matchedKeywords) {
        ...
        const matched = __DOM_ID_PREFIX__SupportedRegions.find(function (region) {
    ...
    function extractAppTier(prompt, fallbackAppTier, assumptions, matchedKeywords) {
        ...
        const options = [
            ...
            { value: 'ecs', tests: [/\becs\b/, /fargate/, /ecs services?/, /containers?/] },
            { value: 'lambda', tests: [/\blambda\b/, /\bfunctions\b/, /serverless/] },
            { value: 'ec2', tests: [/\bec2\b/, /auto scaling/, /application servers?/, /instances?/] }
        ];
...
// ns:end family._base.workspace.01_input-brief
        return __PASCAL_PREFIX__ModelCore.buildPromptTitle(prompt, fallbackLabel);
    }

    function inferFromPrompt(prompt, preset) {
        return __PASCAL_PREFIX__ModelCore.inferFromPrompt(prompt, preset);
    }
...
// ns:end family._base.workspace.03_custom-settings
    ...
    function describeInventoryItem(item, spec) {
        ...
        if (component === 'aws vpc') {
            return 'Isolated regional network boundary where subnets, route tables, gateways, VPC endpoints, and security controls attach to the CIDR range.';
        }

        if (component === 'route 53') {
            return 'DNS entry point for the application hostname, resolving users toward CloudFront when present or directly toward the ingress tier.';
        }

        if (component === 'cloudfront') {
        ...
        if (component === 'application load balancer') {
        ...
        if (component.indexOf('public subnet ') === 0) {
            return 'Subnet tier with an internet gateway route for public ingress components, bastion access, or managed NAT gateway egress.';
        }
        ...
        if (component.indexOf('rds subnet ') === 0 || component.indexOf('aurora subnet ') === 0) {
            return 'Private database subnet group member used to place the data tier across availability zones without public ingress.';
        }

        if (component === 'nat gateway') {
        ...
        if (component === 'eks') {
            return 'Kubernetes workload tier with private worker nodes, keeping cluster workloads off direct public subnet placement.';
        }

        if (component === 'lambda in vpc') {
        ...
        if (component === 'dynamodb') {
            return 'Regional managed data tier outside the VPC boundary, normally reached privately through VPC endpoints when enabled.';
        }

        if (component === 'elasticache redis') {
            return 'Private in-memory cache tier used by application services for low-latency state, sessions, or query acceleration.';
        }

        if (component === 'vpc endpoints') {
            return 'Private AWS service access path for services such as S3 and Systems Manager without depending on public internet routing.';
        }

        if (component === 'bastion host') {
            return 'Hardened administrative jump host in the public tier for controlled access into private resources.';
        }

        if (component === 'site-to-site vpn') {
    ...
    function buildInventory(spec) {
        ...
        azLabels.forEach(function (azLabel, index) {
            inventory.push({
                ...
                placement: 'AZ ' + azLabel.toUpperCase(),
        ...
        inventory.push({
            ...
            placement: spec.azCount + ' AZ layout',
        ...
        if (spec.endpoints) {
            inventory.push({
                ...
                purpose: 'Private connectivity to AWS managed services'
            });
        }
    ...
    function buildModelSummary(spec) {
        ...
        if (spec.route53) {
            summary.push('Route 53');
        }

        if (spec.cloudFront) {
            summary.push('CloudFront');
        }

        if (spec.waf) {
            summary.push('AWS WAF');
        }

        if (spec.endpoints) {
            summary.push('VPC Endpoints');
        }
...
// ns:start family._base.workspace.05_result-summary
    ...
    function buildArchitectureProsCons(spec) {
        ...
        if (spec.azCount >= 2) {
            pros.push('Multi-AZ placement improves workload and database resilience.');
        } else {
            pros.push('Single-AZ placement keeps a lab or proof-of-concept topology simple.');
            cons.push('Single-AZ placement limits failure tolerance.');
        }

        if (spec.alb) {
            pros.push('Application Load Balancer separates public ingress from private workloads.');
        } else {
        ...
        if (spec.database === 'rds' || spec.database === 'aurora') {
            ...
            pros.push('DynamoDB removes database subnet and instance management from the VPC path.');
        } else {
            cons.push('No managed database tier is represented in the architecture.');
        }

        if (spec.endpoints) {
            pros.push('VPC Endpoints reduce dependency on public internet paths for AWS service access.');
        } else {
            cons.push('Private workloads may rely on NAT or public paths for AWS service APIs.');
        }

        if (spec.cloudWatch && spec.flowLogs) {
            pros.push('CloudWatch and VPC flow logs give the design baseline observability.');
        } else if (spec.cloudWatch || spec.flowLogs) {
            cons.push('Observability is partial; pair CloudWatch with flow logs for better operations coverage.');
        } else {
            cons.push('No observability layer is modeled for metrics, logs, or network traffic inspection.');
        }

        if (spec.natMode === 'per-az') {
            pros.push('NAT per AZ avoids a single outbound egress dependency.');
        } else if (spec.natMode === 'single' && spec.azCount >= 2) {
            pros.push('A single NAT gateway keeps cost lower than per-AZ NAT.');
            cons.push('A single NAT gateway is an availability-zone dependency for private subnet egress.');
        } else if (spec.natMode === 'none') {
            cons.push('No NAT gateway is modeled, so private subnet outbound internet access may be blocked.');
        }

        if (spec.route53 && spec.cloudFront && spec.waf) {
            pros.push('Route 53, CloudFront, and AWS WAF create a stronger edge entry path.');
        } else if (!spec.waf && (spec.alb || spec.cloudFront)) {
            cons.push('No AWS WAF layer is modeled in front of public HTTP ingress.');
        }

        if (spec.bastion) {
            cons.push('A bastion host adds an administrative access surface that needs strict hardening.');
        }
        ...
        if (cons.length === 0) {
            cons.push('No major diagram-level gaps detected; still review IAM, security groups, and route tables before production.');
        }
    ...
    function buildArchitectureScore(spec) {
        ...
        return {
            ...
            tags: [
                ...
                { icon: 'bi bi-grid-3x3-gap', label: spec.azCount + ' AZ', tone: 'az' },
...
// ns:end family._base.workspace.05_result-summary
    ...
    function buildRiskLevel(spec) {
        ...
        let detail = 'Review IAM, security groups, route tables, and data protection settings before production change approval.';
        ...
        if (scorePayload.score < 65 || consCount >= 5) {
            ...
            detail = 'Prioritize availability, ingress protection, private routing, and observability before using this as a delivery baseline.';
            ...
            detail = 'Validate NAT placement, service controls, and operational ownership before production rollout.';
        }
...
// ns:start family.architecture.workspace.04_visual-contract
    function inferEdgeCards(spec) {
        ...
        if (spec.route53) {
            cards.push({ id: 'route53', label: 'Route 53', short: 'DNS', tone: 'service' });
        }

        if (spec.cloudFront) {
            cards.push({ id: 'cloudfront', label: 'CloudFront', short: 'CF', tone: 'service' });
        }

        if (spec.waf) {
            cards.push({ id: 'waf', label: 'AWS WAF', short: 'WAF', tone: 'security' });
        }
    ...
    function computeStageGeometry(spec) {
        ...
        const cacheLaneHeight = spec.cache ? 120 : 0;
        const dataTierGap = hasDataTier ? interRowGap + dataHeight : 0;
        const cacheTierGap = spec.cache ? interRowGap + cacheLaneHeight : 0;
        const azHeight = azHeaderHeight + publicHeight + interRowGap + appHeight + cacheTierGap + dataTierGap + azBottomPadding;
        const sharedServicesLaneHeight = spec.cache ? 126 : 0;
        const sharedServicesY = azY + azHeight + 32;
        const vpcHeight = azHeight + topOffset + 58;
        ...
        return {
            ...
            appCardTopInset: 74,
            cacheLaneHeight: cacheLaneHeight,
    ...
    function resolveCardIconHref(card, options) {
        ...
        if (!card.iconKey || !Object.prototype.hasOwnProperty.call(__DOM_ID_PREFIX__IconUrlMap, card.iconKey)) {
            return '';
        }

        return __DOM_ID_PREFIX__IconUrlMap[card.iconKey];
    }
    ...
    function getDependentCardIds(cardId, spec) {
        ...
        azLabels.forEach(function (azLabel, index) {
            ...
            amazonVpcChildren.push('availability-zone-' + azLabel);
            Array.prototype.push.apply(amazonVpcChildren, zoneChildren);
        });

        if (spec.alb) {
            amazonVpcChildren.push('application-load-balancer');
        }

        amazonVpcChildren.push('internet-gateway');

        if (spec.cache) {
            amazonVpcChildren.push('elasticache');
        }

        if (spec.endpoints) {
            amazonVpcChildren.push('vpc-endpoints');
        }

        const dependencyMap = Object.assign({}, zoneDependencies, subnetDependencies, {
            'amazon-vpc-shell': amazonVpcChildren
        });
    ...
    function cloneLayoutOverrides(layoutOverrides) {
        if (__DOM_ID_PREFIX__EngineRuntime && typeof __DOM_ID_PREFIX__EngineRuntime.cloneLayoutOverrides === 'function') {
            return __DOM_ID_PREFIX__EngineRuntime.cloneLayoutOverrides(layoutOverrides);
        }

        return __PASCAL_PREFIX__ModelCore.cloneLayoutOverrides(layoutOverrides);
    }

    function cloneConnectorOverrides(connectorOverrides) {
        if (__DOM_ID_PREFIX__EngineRuntime && typeof __DOM_ID_PREFIX__EngineRuntime.cloneConnectorOverrides === 'function') {
            return __DOM_ID_PREFIX__EngineRuntime.cloneConnectorOverrides(connectorOverrides);
        }

        return __PASCAL_PREFIX__ModelCore.cloneConnectorOverrides(connectorOverrides);
    }
    ...
    function currentViewportState() {
        return {
            ...
            fullscreen: Boolean(stageShell && (document.fullscreenElement === stageShell || stageShell.classList.contains('__PREFIX__-stage-expanded'))),
    ...
    function createEngineState(value) {
        ...
        if (__DOM_ID_PREFIX__EngineRuntime && typeof __DOM_ID_PREFIX__EngineRuntime.createState === 'function') {
            return __DOM_ID_PREFIX__EngineRuntime.createState(stateValue, engineRuntimeConfig);
        }
    ...
    function toPersistedEngineState(value) {
        ...
        if (__DOM_ID_PREFIX__EngineRuntime && typeof __DOM_ID_PREFIX__EngineRuntime.toPersistedState === 'function') {
            return __DOM_ID_PREFIX__EngineRuntime.toPersistedState(state, engineRuntimeConfig);
        }
    ...
    function renderSvgCard(card, options) {
        ...
        return [
            ...
            '<rect x="' + card.x + '" y="' + card.y + '" width="' + card.width + '" height="' + card.height + '" rx="18" class="diagram-card diagram-card-' + card.tone + '"></rect>',
            ...
            '<rect x="' + card.x + '" y="' + card.y + '" width="' + card.width + '" height="' + card.height + '" rx="18" class="diagram-card-hitbox"></rect>',
    ...
    function renderCardConnector(sourceCard, targetCard, options) {
        ...
        const markerId = resolvedOptions.active ? '__DOM_ID_PREFIX__ArrowActive' : '__DOM_ID_PREFIX__Arrow';
        ...
        const connectorPath = buildConnectorPathFromCards(sourceCard, targetCard, sides.sourceSide, sides.targetSide, sourceRatio, targetRatio, bend);
        ...
        return [
            ...
            ' data-connector-id="' + escapeHtml(connectorId) + '"></path>',
            '<path d="' + connectorPath + '"',
    ...
    function buildSvgMarkup(spec, layoutOverrides, connectorOverrides, options) {
        ...
        const vpcShellOverride = safeLayoutOverrides['amazon-vpc-shell'] || {};
        const vpcShell = {
            id: 'amazon-vpc-shell',
        ...
        if (previousEdgeCard !== null) {
            connectors.push(renderCardConnector(previousEdgeCard, igwCard, {
                id: previousEdgeCard.id + '-to-internet-gateway',
                sourceSide: 'bottom',
                targetSide: 'top',
                active: true
            }));
        ...
        if (spec.alb) {
            ...
            connectors.push(renderCardConnector(igwCard, albCard, {
                id: 'internet-gateway-to-application-load-balancer',
                sourceSide: 'bottom',
                targetSide: 'top',
                active: true
            }));
        ...
        azLabels.forEach(function (azLabel, index) {
            ...
            const dataY = appY + geometry.appHeight + (spec.cache ? geometry.interRowGap + geometry.cacheLaneHeight : 0) + geometry.interRowGap;
            ...
            const appCard = addCard({
                ...
                subtitle: ['AZ ' + azLabel.toUpperCase() + ' workload'],
                tone: 'service',
            ...
            if (geometry.hasDataTier) {
                ...
                databaseCards.push(addCard({
                    ...
                    short: spec.database === 'aurora' ? 'AUR' : 'RDS',
        ...
        if (albCard) {
            appCards.forEach(function (card) {
                connectors.push(renderCardConnector(albCard, card, buildIngressToAppConnectorOptions(
                    card,
                    'application-load-balancer-to-' + card.id
                )));
            });
        } else {
            appCards.forEach(function (card) {
        ...
        if (spec.cache) {
            const cacheCard = addCard({
                id: 'elasticache',
                x: vpcCenterX - 140,
                y: geometry.azY + geometry.azHeaderHeight + geometry.publicHeight + geometry.interRowGap + geometry.appHeight + geometry.interRowGap + ((geometry.cacheLaneHeight - 64) / 2),
                width: 280,
                height: 64,
        ...
        if (spec.siteToSiteVpn) {
            ...
            connectors.push(renderCardConnector(vpnCard, vpcShell, {
                id: 'site-to-site-vpn-to-amazon-vpc',
                sourceSide: 'right',
                targetSide: 'left',
        ...
        if (spec.transitGateway) {
            ...
            connectors.push(renderCardConnector(transitGatewayCard, vpcShell, {
                id: 'transit-gateway-to-amazon-vpc',
                sourceSide: 'right',
                targetSide: 'left',
        ...
        if (spec.endpoints) {
            addRightRailCard({
                ...
                label: 'VPC Endpoints',
                short: 'EP',
                subtitle: ['Private S3 and Systems Manager access'],
                tone: 'service',
        ...
        return {
            svgMarkup: [
                ...
                '<defs>',
                '<style><![CDATA[',
                '.diagram-root{font-family:Roboto,system-ui,-apple-system,"Segoe UI","Helvetica Neue",Arial,"Noto Sans","Liberation Sans",sans-serif;--diagram-font-title:26px;--diagram-font-meta:15px;--diagram-font-pill:13px;--diagram-font-card-title:16px;--diagram-font-card-subtitle:12px;--diagram-font-zone-label:14px;--diagram-ink:#16191f;--diagram-muted:#5f6b7a;--diagram-network:#111827;--diagram-network-active:#111827;--diagram-border:#b9d7ee;--diagram-border-soft:#d7e7f4;--diagram-surface:#ffffff;--diagram-surface-soft:#fbfdff;--diagram-aws:#F97316;--diagram-aws-deep:#C2410C;--diagram-select:#7C3AED;}',
                '.diagram-vpc{fill:var(--diagram-surface);stroke:#9cc9e8;stroke-width:1.55;filter:drop-shadow(0 18px 28px rgba(20,110,180,0.08));}',
                '.diagram-card-group.is-selected .diagram-vpc{stroke:var(--diagram-select);stroke-width:2.8;filter:drop-shadow(0 14px 22px rgba(249,115,22,0.18));}',
                '.diagram-vpc-title{fill:#14395b;font-size:var(--diagram-font-title);font-weight:800;}',
                '.diagram-vpc-meta{fill:#4a637c;font-size:var(--diagram-font-meta);font-weight:600;}',
                '.diagram-card{fill:var(--diagram-card-fill,var(--diagram-surface));stroke:var(--diagram-card-border,var(--diagram-border));stroke-width:1.25;filter:drop-shadow(0 10px 18px rgba(20,110,180,0.075));}',
                '.diagram-card-zone{--diagram-card-fill:var(--diagram-surface);--diagram-card-border:#c6ddec;}',
                '.diagram-card-public{--diagram-card-fill:#fafdff;--diagram-card-border:#b9d7ee;}',
                '.diagram-card-private{--diagram-card-fill:#ffffff;--diagram-card-border:#c5ddec;}',
                '.diagram-card-service{--diagram-card-fill:#ffffff;--diagram-card-border:#d4e1ec;}',
                '.diagram-card-edge{--diagram-card-fill:#ffffff;--diagram-card-border:#d4e1ec;}',
                '.diagram-card-security{--diagram-card-fill:#ffffff;--diagram-card-border:#d4e1ec;}',
                '.diagram-card-database{--diagram-card-fill:#ffffff;--diagram-card-border:#c7dfd4;}',
                '.diagram-card-hitbox{fill:transparent;pointer-events:all;}',
                '.diagram-card-highlight{display:none;fill:rgba(124,58,237,0.07);stroke:var(--diagram-select);stroke-width:3;stroke-dasharray:10 7;pointer-events:none;filter:drop-shadow(0 10px 18px rgba(124,58,237,0.18));}',
                '.diagram-card-group.is-selected .diagram-card-highlight{display:block;}',
                '.diagram-card-group.is-highlighted .diagram-card-highlight{display:block;stroke:#6D28D9;stroke-width:4;filter:drop-shadow(0 12px 22px rgba(124,58,237,0.22));}',
                '.diagram-content-highlight{display:none;fill:rgba(124,58,237,0.045);stroke:#7C3AED;stroke-width:5;stroke-dasharray:18 10;pointer-events:none;filter:drop-shadow(0 16px 28px rgba(124,58,237,0.16));}',
                '.diagram-root.is-highlight-all .diagram-content-highlight{display:block;}',
                '.diagram-card-group.is-marquee-target .diagram-card-highlight{display:block;stroke:#F97316;stroke-width:3;filter:drop-shadow(0 12px 22px rgba(249,115,22,0.24));}',
                '.diagram-card-group.is-selected .diagram-card{stroke:var(--diagram-select);stroke-width:2.8;filter:drop-shadow(0 14px 22px rgba(124,58,237,0.16));}',
                '.diagram-resize-handle{display:none;fill:#ffffff;stroke:var(--diagram-select);stroke-width:2;cursor:nwse-resize;pointer-events:all;filter:drop-shadow(0 4px 8px rgba(124,58,237,0.22));}',
                ...
                '.diagram-resize-preview{fill:rgba(124,58,237,0.08);stroke:var(--diagram-select);stroke-width:2;stroke-dasharray:8 6;pointer-events:none;}',
                '.diagram-pill{fill:#ffffff;stroke:#d7e7f4;stroke-width:1;}',
                '.diagram-pill-zone{fill:#ffffff;}',
                '.diagram-pill-public{fill:#ffffff;}',
                '.diagram-pill-private{fill:#ffffff;}',
                '.diagram-pill-service{fill:#ffffff;}',
                '.diagram-pill-edge{fill:#ffffff;}',
                '.diagram-pill-security{fill:#ffffff;}',
                '.diagram-pill-database{fill:#ffffff;}',
                '.diagram-pill-text{fill:var(--diagram-ink);font-size:var(--diagram-font-pill);font-weight:800;text-anchor:middle;}',
                '.diagram-card-title{fill:var(--diagram-ink);font-size:var(--diagram-font-card-title);font-weight:800;}',
                '.diagram-card-subtitle{fill:var(--diagram-muted);font-size:var(--diagram-font-card-subtitle);font-weight:500;}',
                '.diagram-zone-label{fill:#243b53;font-size:var(--diagram-font-zone-label);font-weight:800;}',
                '.diagram-connector{fill:none;stroke:var(--diagram-network);stroke-width:1.65;stroke-linecap:round;stroke-linejoin:round;opacity:0.88;pointer-events:stroke;vector-effect:non-scaling-stroke;}',
                '.diagram-connector-hit-target{fill:none;stroke:transparent;stroke-width:18;cursor:pointer;pointer-events:stroke;vector-effect:non-scaling-stroke;}',
                '.diagram-connector-active{stroke:var(--diagram-network-active);stroke-width:1.65;opacity:0.96;}',
                '.diagram-connector.is-selected{stroke:var(--diagram-select);stroke-width:2;opacity:1;}',
                '.diagram-connector-anchor-handle{fill:#ffffff;stroke:var(--diagram-select);stroke-width:2;cursor:grab;pointer-events:all;filter:drop-shadow(0 4px 8px rgba(124,58,237,0.22));}',
                '.diagram-connector-anchor-handle:active{cursor:grabbing;}',
                '.diagram-connector-bend-handle{fill:var(--diagram-select);stroke:#ffffff;stroke-width:2;cursor:move;pointer-events:all;filter:drop-shadow(0 4px 8px rgba(124,58,237,0.26));}',
                ...
                '.diagram-card-group-draggable:hover .diagram-card{stroke:var(--diagram-select);stroke-width:2.2;}',
                '.diagram-card-group-draggable:focus-visible{outline:none;}',
                '.diagram-card-group-draggable:focus-visible .diagram-card{stroke:var(--diagram-select);stroke-width:2.6;filter:drop-shadow(0 10px 18px rgba(124,58,237,0.18));}',
                '.diagram-card-group-draggable:focus-visible .diagram-vpc{stroke:var(--diagram-select);stroke-width:2.8;filter:drop-shadow(0 10px 18px rgba(124,58,237,0.18));}',
                ']]></style>',
                '<marker id="__DOM_ID_PREFIX__Arrow" markerWidth="11" markerHeight="11" refX="10" refY="5.5" orient="auto" markerUnits="userSpaceOnUse">',
                '<path d="M 0 0 L 11 5.5 L 0 15 z" fill="#111827"></path>',
                '</marker>',
                '<marker id="__DOM_ID_PREFIX__ArrowActive" markerWidth="11" markerHeight="11" refX="10" refY="5.5" orient="auto" markerUnits="userSpaceOnUse">',
...
// ns:end family.architecture.workspace.04_visual-contract
    ...
    function buildExportPayload(spec, inventory, layoutOverrides, connectorOverrides) {
        return applyVisualStateToExportPayload(__PASCAL_PREFIX__ModelCore.buildExportPayload(spec, inventory, layoutOverrides, connectorOverrides, buildNotePayload(spec)));
    }
    ...
    function renderInventory(inventory) {
        ...
        inventoryTableBody.innerHTML = sortedInventory.map(function (item, index) {
            return [
                ...
                '<td class="__PREFIX__-table-action-cell tool-table-action-cell">',
                '<button type="button" class="__PREFIX__-row-copy" data-inventory-copy-row="' + escapeHtml(index) + '" aria-label="Copy inventory row ' + escapeHtml(item.inventoryIndex || index + 1) + '" title="Copy inventory row">',
...
// ns:start family.architecture.workspace.04_visual-contract
    ...
    function renderNotes(spec) {
        ...
        keywordList.innerHTML = notePayload.matched_keywords.length > 0
            ...
            : '<li>No explicit AWS keywords were matched. The diagram relies on the selected preset and inspector controls.</li>';
...
// ns:start family._base.workspace.05_result-summary
    ...
    function loadECharts() {
        ...
        if (!echartsPromise) {
            echartsPromise = new Promise(function (resolve) {
                const existingScript = document.querySelector('script[data-__PREFIX__-echarts]');
    ...
    function toolCssValue(name, fallback) {
        const root = document.querySelector('.__TOOL_CLASS__');
        const value = root && window.getComputedStyle
    ...
    function renderScoreRingChart(scorePayload, renderToken) {
        const chartElement = document.getElementById('__DOM_ID_PREFIX__ScoreEchart');
        const scoreValue = document.getElementById('__DOM_ID_PREFIX__ScoreValue');
        const scoreRingColor = toolCssValue('--tool-score-ring', '#c2410c');
        const scoreRingRgb = toolCssValue('--tool-score-ring-rgb', '194, 65, 12');

        if (!chartElement || !scoreValue) {
            return;
        }

        loadECharts().then(function (echarts) {
            if (!echarts || renderToken !== scoreRingRenderToken || !chartElement.isConnected) {
                return;
            }
                ...
                series: [
                    {
                        ...
                        progress: {
                            ...
                            itemStyle: {
                                ...
                                shadowBlur: 8,
                                shadowColor: 'rgba(' + scoreRingRgb + ', 0.12)'
                            }
                        },
                        axisLine: {
                            roundCap: true,
            ...
            scoreValue.setAttribute('data-chart-ready', 'true');
            scoreRingResizeHandler = function () {
                if (scoreRingChart) {
...
// ns:end family._base.workspace.05_result-summary
    ...
    function renderAssessmentSections(spec) {
        ...
        pillarBreakdownOutput.className = '__PREFIX__-assessment-card __PREFIX__-pillar-card';
        riskLevelOutput.className = '__PREFIX__-assessment-card __PREFIX__-risk-card __PREFIX__-risk-card-' + risk.tone;

        pillarBreakdownOutput.innerHTML = [
            '<h3 class="__PREFIX__-result-section-title">Pillar Breakdown</h3>',
            '<div class="__PREFIX__-pillar-list">',
            pillars.map(function (pillar) {
                return [
                    '<div class="__PREFIX__-pillar-row __PREFIX__-pillar-row-' + escapeHtml(pillar.tone) + '">',
                    '<span class="__PREFIX__-pillar-icon" aria-hidden="true"><i class="' + escapeHtml(pillar.icon) + '"></i></span>',
                    '<span class="__PREFIX__-pillar-name">' + escapeHtml(pillar.label) + '</span>',
                    '<span class="__PREFIX__-pillar-meter" aria-hidden="true"><span style="--pillar-score: ' + escapeHtml(String(pillar.score)) + '%;"></span></span>',
                    '<span class="__PREFIX__-pillar-score"><strong>' + escapeHtml(String(pillar.score)) + '</strong> /100</span>',
                    '</div>'
                ].join('');
            }).join(''),
            '</div>',
            '<div class="__PREFIX__-pillar-legend" aria-label="Pillar score legend">',
            '<span><i class="__PREFIX__-legend-dot __PREFIX__-legend-dot-excellent"></i>Excellent (90-100)</span>',
            '<span><i class="__PREFIX__-legend-dot __PREFIX__-legend-dot-good"></i>Good (70-89)</span>',
            '<span><i class="__PREFIX__-legend-dot __PREFIX__-legend-dot-fair"></i>Fair (50-69)</span>',
            '<span><i class="__PREFIX__-legend-dot __PREFIX__-legend-dot-needs"></i>Needs improvement (&lt;50)</span>',
            '</div>'
        ].join('');

        riskLevelOutput.innerHTML = [
            '<h3 class="__PREFIX__-result-section-title">Risk Level</h3>',
            '<div class="__PREFIX__-risk-body">',
            '<div class="__PREFIX__-risk-icon" aria-hidden="true"><i class="' + escapeHtml(risk.icon) + '"></i></div>',
            '<div class="__PREFIX__-risk-copy">',
            '<div class="__PREFIX__-risk-level">' + escapeHtml(risk.level) + '</div>',
            '<p>' + escapeHtml(risk.summary) + '<br>' + escapeHtml(risk.detail) + '</p>',
            '</div>',
            '</div>',
            '<div class="__PREFIX__-risk-meta">',
            '<div>',
            '<span>Generated</span>',
...
// ns:start family._base.workspace.05_result-summary
    ...
    function renderOutputScore(spec) {
        ...
        const tagChips = scorePayload.tags.map(function (item) {
            return [
                '<span class="__PREFIX__-result-chip __PREFIX__-result-chip-' + chipTone(item.tone) + '">',
                '<span class="__PREFIX__-result-chip-icon"><i class="' + escapeHtml(item.icon) + '" aria-hidden="true"></i></span>',
                escapeHtml(item.label),
                '</span>'
            ].join('');
        }).join('');

        outputStatus.className = '__PREFIX__-score-card __PREFIX__-result-summary';
        ...
        outputStatus.innerHTML = [
            ...
            '<span class="__PREFIX__-result-kicker">Architecture score</span>',
            '<h3 class="__PREFIX__-result-title">' + escapeHtml(scorePayload.band) + '</h3>',
            '<p class="__PREFIX__-result-copy">' + escapeHtml(scorePayload.detail) + '</p>',
            '<div class="__PREFIX__-result-chip-row" aria-label="Architecture score state">',
            '<span class="__PREFIX__-result-chip __PREFIX__-result-chip-' + chipTone(scorePayload.badgeTone) + '"><span class="__PREFIX__-result-chip-icon"><i class="' + escapeHtml(scorePayload.badgeIcon) + '" aria-hidden="true"></i></span>' + escapeHtml(scorePayload.badgeLabel) + '</span>',
            tagChips,
            '</div>',
            '</article>',
            '<article class="__PREFIX__-result-card __PREFIX__-result-card-visual">',
            '<div class="__PREFIX__-result-ring __PREFIX__-score-value" id="__DOM_ID_PREFIX__ScoreValue" style="--__PREFIX__-result-progress: ' + escapeHtml(String(ringProgressAngle)) + 'deg; --progress-angle: ' + escapeHtml(String(ringProgressAngle)) + 'deg;" aria-label="Architecture score ' + escapeHtml(String(scorePayload.score)) + ' out of 100">',
            '<div class="__PREFIX__-score-echart" id="__DOM_ID_PREFIX__ScoreEchart" aria-hidden="true"></div>',
            '<div class="__PREFIX__-result-ring-center __PREFIX__-score-center">',
            '<span class="__PREFIX__-result-ring-value __PREFIX__-score-value-number">' + escapeHtml(String(scorePayload.score)) + '</span>',
            '<span class="__PREFIX__-result-ring-unit __PREFIX__-score-caption">/100</span>',
            '</div>',
            '</div>',
            '<div class="__PREFIX__-result-visual-copy">',
            '<span class="__PREFIX__-result-kicker">Primary result</span>',
            '<h3 class="__PREFIX__-result-title __PREFIX__-result-title-center">' + escapeHtml(scorePayload.ringLabel) + '</h3>',
            '</div>',
            '</article>',
            '</div>',
            '<div class="__PREFIX__-result-metric-grid" aria-label="Architecture metrics">',
            '<article class="__PREFIX__-result-metric-card"><span class="__PREFIX__-result-metric-label">Region</span><strong class="__PREFIX__-result-metric-value">' + escapeHtml(spec.region) + '</strong><span class="__PREFIX__-result-metric-copy">Selected deployment geography.</span></article>',
            '<article class="__PREFIX__-result-metric-card"><span class="__PREFIX__-result-metric-label">Zones</span><strong class="__PREFIX__-result-metric-value">' + escapeHtml(String(spec.azCount) + ' AZ') + '</strong><span class="__PREFIX__-result-metric-copy">Availability zone spread.</span></article>',
            '<article class="__PREFIX__-result-metric-card"><span class="__PREFIX__-result-metric-label">Egress</span><strong class="__PREFIX__-result-metric-value">' + escapeHtml(natModeLabel(spec.natMode)) + '</strong><span class="__PREFIX__-result-metric-copy">Private tier outbound pattern.</span></article>',
            '<article class="__PREFIX__-result-metric-card"><span class="__PREFIX__-result-metric-label">Data tier</span><strong class="__PREFIX__-result-metric-value">' + escapeHtml(databaseLabel(spec.database)) + '</strong><span class="__PREFIX__-result-metric-copy">Modeled persistence layer.</span></article>',
            '</div>'
        ].join('');

        renderScoreRingChart(scorePayload, scoreRingRenderToken);
    }

    function renderStageMeta(spec) {
        stageMeta.innerHTML = [
            createToneChip('bi bi-globe2', spec.region, 'region'),
            createToneChip('bi bi-grid-3x3-gap', String(spec.azCount) + ' AZ', 'az'),
...
// ns:start family.architecture.workspace.04_selected-item
        ...
        stageSubtitle.innerHTML = '<span class="__PREFIX__-stage-preset-chip" title="' + escapeHtml(presetChipLabel) + '">' + escapeHtml(presetChipLabel) + '</span>';
    }
    ...
    function updateSelectedCardEditor() {
        ...
        if (!selectedCard) {
            const hasConnectorSelection = selectedConnectorId !== '';
            ...
            selectedWidthInput.value = '';
            selectedHeightInput.value = '';
            renderSelectedEmptyMessage(hasConnectorSelection
...
// ns:start family.architecture.workspace.04_visual-contract
    ...
    function isMarqueeBlockedTarget(target) {
        ...
        if (target.closest('.diagram-connector, .diagram-connector-hit-target, .diagram-connector-anchor-handle, .diagram-connector-bend-handle, .diagram-resize-handle')) {
            return true;
        }
...
// ns:start family.architecture.workspace.04_selected-item
    ...
    function createResizePreview(svgElement, card) {
        ...
        preview.setAttribute('rx', card.id === 'amazon-vpc-shell' ? '24' : '16');
    ...
    function bindStageDragging() {
        ...
        Array.from(svgElement.querySelectorAll('.diagram-connector[data-connector-id], .diagram-connector-hit-target[data-connector-id]')).forEach(function (path) {
...
// ns:start family.architecture.workspace.04_visual-contract
        ...
        stageCanvas.classList.remove('__PREFIX__-stage-preview');
        ...
        normalizeInfraStackResultSummary('__TOOL_SLUG__');
...
// ns:end family.architecture.workspace.04_visual-contract
    ...
    function createPresetPreviewOverlay() {
        return [
// ns:start family.architecture.workspace.04_visual-contract
            '<div class="__PREFIX__-stage-preview-overlay" role="status">',
            '<div class="__PREFIX__-stage-preview-panel">',
            '<span class="__PREFIX__-stage-preview-icon"><i class="bi bi-stars" aria-hidden="true"></i></span>',
    ...
    function renderPresetPreview(preset, options) {
        ...
        stageCanvas.classList.add('__PREFIX__-stage-preview');
...
// ns:start family._base.workspace.01_input-brief
    ...
    function generateFromPrompt() {
        ...
        if (prompt === '') {
            showError('Add an AWS VPC brief before generating the diagram.');
            return;
        }
...
// ns:start family._base.workspace.02_basic-settings
    ...
    function resetToDefault() {
        applyPreset(__DOM_ID_PREFIX__PresetCatalog[0].id, false);
// ns:end family._base.workspace.02_basic-settings
// ns:start family._base.workspace.01_input-brief
    }
...
// ns:start family._base.workspace.06_output-toolbar
    ...
    function getSvgDimensions(svgMarkup) {
        const viewBoxMatch = String(svgMarkup || '').match(/viewBox=["']\s*0\s+0\s+([\d.]+)\s+([\d.]+)\s*["']/);
        const width = viewBoxMatch ? Number.parseFloat(viewBoxMatch[1]) : 1520;
        const height = viewBoxMatch ? Number.parseFloat(viewBoxMatch[2]) : 860;

        return {
            width: Number.isFinite(width) && width > 0 ? width : 1520,
            height: Number.isFinite(height) && height > 0 ? height : 860
        };
    }
    ...
    async function createStagePngBlob(svgMarkup) {
        const dimensions = getSvgDimensions(svgMarkup);
        const loadedImage = await loadSvgImage(svgMarkup);
        const canvas = document.createElement('canvas');
        const context = canvas.getContext('2d');

        if (!context) {
            URL.revokeObjectURL(loadedImage.svgUrl);
    ...
    function getShareTitle() {
        return getPageMetaContent('meta[property="og:title"]') || document.title || '__TOOL_TITLE__';
    }

    function getShareSubtitle() {
        if (!latestResult || !latestResult.spec) {
            return '';
        }
    ...
    function getShareDescription() {
        return getPageMetaContent('meta[property="og:description"]') || 'Generated AWS VPC architecture snapshot from InfraStack.';
    }

    window.InfraStackShareAdapters = window.InfraStackShareAdapters || {};
    window.InfraStackShareAdapters['aws/__TOOL_SLUG__'] = {
        fileName: '__PREFIX__-share.png',
        ensureReady: async function () {
            ...
            if (!latestResult) {
                throw new Error('Generate a diagram before opening the share preview.');
            }
        },
        getTitle: getShareTitle,
        getSubtitle: getShareSubtitle,
        getSummary: getShareDescription
    };
    ...
    function downloadSvg() {
        ...
        downloadFile('__TOOL_SLUG__.svg', latestResult.svgMarkup, 'image/svg+xml;charset=utf-8');
    }
    ...
    async function exportPng() {
        ...
        try {
            const pngBlob = await createStagePngBlob(latestResult.svgMarkup);

            downloadFile('__TOOL_SLUG__.png', pngBlob, 'image/png');
    ...
    function downloadJson() {
        ...
        downloadFile('__TOOL_SLUG__.json', JSON.stringify(latestResult.exportPayload, null, 2), 'application/json;charset=utf-8');
    }
...
// ns:start family._base.workspace.01_input-brief
    ...
    presetInput.addEventListener('change', function () {
        applyPreset(String(presetInput.value || __DOM_ID_PREFIX__PresetCatalog[0].id), true);
// ns:end family._base.workspace.01_input-brief
// ns:start family._base.workspace.02_basic-settings
    });
...
// ns:end family._base.workspace.02_basic-settings
            activateTab(String(button.dataset.tabTarget || '__DOM_ID_PREFIX__InventoryPanel'));
        });
    });
    ...
    configTabButtons.forEach(function (button) {
        button.addEventListener('click', function () {
// ns:start family._base.workspace.03_custom-settings
            activateConfigTab(String(button.dataset.configTabTarget || '__DOM_ID_PREFIX__NetworkConfigPanel'));
        });
    });
...
// ns:start family.architecture.workspace.04_visual-contract
    ...
    document.addEventListener('keydown', function (event) {
        if (event.key === 'Escape' && stageShell.classList.contains('__PREFIX__-stage-expanded')) {
            setStageExpanded(false);
        }
    });
...
// ns:start family._base.workspace.06_output-toolbar
    ...
    inventoryTableBody.addEventListener('click', function (event) {
        const button = event.target.closest('.__PREFIX__-row-copy');
// ns:end family._base.workspace.06_output-toolbar
...
// ns:start family._base.workspace.03_custom-settings
    ...
    activateTab('__DOM_ID_PREFIX__InventoryPanel');
    activateConfigTab('__DOM_ID_PREFIX__NetworkConfigPanel');
    updateFullscreenButton();
// ns:end family._base.workspace.03_custom-settings
    applyStageZoom();
    updatePresetSelection();
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', init__PASCAL_PREFIX__Tool);
} else {
    init__PASCAL_PREFIX__Tool();
}
