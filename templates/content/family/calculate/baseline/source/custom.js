...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const presetSummary = document.getElementById('calculateCostAwsPresetSummary');
    const presetOptionInputs = Array.from(document.querySelectorAll('input[name="calculateCostAwsPresetOption"]'));
    const presetSelect = document.getElementById('calculateCostAwsPresetSelect');
    ...
    const ec2InstanceSummary = document.getElementById('calculateCostAwsEc2InstanceSummary');
    const ec2InstanceOptionInputs = Array.from(document.querySelectorAll('input[name="calculateCostAwsEc2InstanceOption"]'));
    const ec2InstanceSelect = document.getElementById('calculateCostAwsEc2InstanceSelect');
    ...
    const ec2PurchaseSummary = document.getElementById('calculateCostAwsEc2PurchaseSummary');
    const ec2PurchaseOptionInputs = Array.from(document.querySelectorAll('input[name="calculateCostAwsEc2PurchaseOption"]'));
    const ec2PurchaseSelect = document.getElementById('calculateCostAwsEc2PurchaseSelect');
    ...
    const apiTypeSummary = document.getElementById('calculateCostAwsApiTypeSummary');
    const apiTypeOptionInputs = Array.from(document.querySelectorAll('input[name="calculateCostAwsApiTypeOption"]'));
    const apiTypeSelect = document.getElementById('calculateCostAwsApiTypeSelect');
    ...
    const sortOptionInputs = Array.from(document.querySelectorAll('input[name="calculateCostAwsSortOption"]'));
    ...
    const dropdownSelects = Array.from(document.querySelectorAll('.calculate-cost-aws-select'));
    ...
    function getSelectSpaceContainer(selectElement) {
        return selectElement.closest('.calculate-cost-aws-field-stack, .calculate-cost-aws-toolbar-left, .calculate-cost-aws-setting-field') || selectElement.parentElement;
    }

    function getDropdownBoundaryRect(element) {
        const boundaryElement = element
            ? element.closest('.calculate-cost-aws-service-card, .calculate-cost-aws-inner-panel, .calculate-cost-aws-section-card, .tool-box')
            : null;
    ...
    function getSelectedSingleLabel(inputs, fallbackLabel) {
        ...
        const labelElement = selectedInput.closest('.calculate-cost-aws-select-card').querySelector('.calculate-cost-aws-select-title');
        ...
        if (query.includeEc2) {
            if (query.ec2Instance === 'custom') {
                ...
                if (query.overrides.ec2Hourly === null) {
                    return 'Set EC2 hourly override in Advanced when using Custom size.';
                }
    ...
    function buildSummaryHtml(result) {
        ...
        return `
            <section class="calculate-cost-aws-overview" aria-live="polite">
                <article class="calculate-cost-aws-spend-card">
                    <div class="calculate-cost-aws-spend-ring" id="calculateCostAwsSpendRing" style="--progress-angle: ${escapeHtml(String(progressAngle))}deg;" aria-label="Estimated monthly AWS run rate ${escapeHtml(formatCurrency(result.totals.monthlyTotal))}">
                        <div class="calculate-cost-aws-spend-chart" id="calculateCostAwsSpendChart" aria-hidden="true"></div>
                        <div class="calculate-cost-aws-spend-value">
                            <strong>${escapeHtml(formatCurrency(result.totals.monthlyTotal))}</strong>
                            <span>Monthly</span>
                        </div>
                    </div>
                    <h3 class="calculate-cost-aws-spend-label">Estimated AWS run rate</h3>
                    <p class="calculate-cost-aws-spend-copy">Built from current workload assumptions, not from your billing account. The math is visible so the argument can be too.</p>
                </article>

                <section class="calculate-cost-aws-summary-panel" aria-label="AWS estimate summary">
                    <div class="calculate-cost-aws-badge-row">
                        <span class="calculate-cost-aws-badge">${escapeHtml(presetLabel)}</span>
                        <span class="calculate-cost-aws-badge">Services ${escapeHtml(String(result.totals.activeServices))}</span>
                        <span class="calculate-cost-aws-badge">Overrides ${escapeHtml(String(overrideCount))}</span>
                        <span class="calculate-cost-aws-badge calculate-cost-aws-badge-muted">Updated ${escapeHtml(generatedAtText)}</span>
                    </div>

                    <div class="calculate-cost-aws-summary-route">
                        <span class="calculate-cost-aws-summary-route-label">Top driver</span>
                        <span class="calculate-cost-aws-summary-route-value">${escapeHtml(result.totals.topDriver)}</span>
                    </div>

                    <div class="calculate-cost-aws-stat-grid">
                        <div class="calculate-cost-aws-stat-card">
                            <span class="calculate-cost-aws-stat-label">Annual</span>
                            <span class="calculate-cost-aws-stat-value">${escapeHtml(formatCurrency(result.totals.annualTotal))}</span>
                            <span class="calculate-cost-aws-stat-note">Projected from the current monthly run rate.</span>
                        </div>

                        <div class="calculate-cost-aws-stat-card">
                            <span class="calculate-cost-aws-stat-label">Daily</span>
                            <span class="calculate-cost-aws-stat-value">${escapeHtml(formatCurrency(result.totals.dailyTotal))}</span>
                            <span class="calculate-cost-aws-stat-note">Thirty-day planning average.</span>
                        </div>

                        <div class="calculate-cost-aws-stat-card">
                            <span class="calculate-cost-aws-stat-label">Hourly</span>
                            <span class="calculate-cost-aws-stat-value">${escapeHtml(formatCurrency(result.totals.hourlyTotal))}</span>
                            <span class="calculate-cost-aws-stat-note">Useful for sanity-checking long-running workloads.</span>
                        </div>

                        <div class="calculate-cost-aws-stat-card">
                            <span class="calculate-cost-aws-stat-label">Egress</span>
                            <span class="calculate-cost-aws-stat-value">${escapeHtml(formatNumber(result.totals.combinedEgressGb, 1))} GB</span>
                            <span class="calculate-cost-aws-stat-note">Combined S3, API response, and shared outbound traffic.</span>
                        </div>
                    </div>
                </section>
            </section>
    ...
    function buildBreakdownRow(item, index) {
        return `
            <tr>
                ...
                <td class="calculate-cost-aws-copy-cell">
